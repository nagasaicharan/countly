...
* @module "api/lib/userovo.model"
*/

/** @lends module:api/lib/userovo.model */
var userovoModel = {};

/**
* Loads userovo model for provided data if it already exists in api/lib folder or in plugins, or creates new one from default model if it does not exist
* @param {string} segment - data segment name to process
* @returns {module:api/lib/userovo.model~userovoMetric} userovo metric model for provided name
* @example
* var userovoModel = require("api/lib/userovo.mode.js");
* var userovoDensity = userovoModel.load("densities");
*/
userovoModel.load = function(segment) {
    var _name = (segment.name) ? segment.name : segment;
    var model;
    try {
        //try loading model from core
        model = require("./userovo." + _name + ".js")();
    }
    catch (ex) {
        try {
...
/**
* Create Userovo data model to process data segment from fetched from server
* @param {function=} fetchValue - default function to fetch and transform if needed value from standard data model
* @returns {module:api/lib/userovo.model~userovoMetric} new userovo metric model
* @example
* var userovoModel = require("api/lib/userovo.mode.js");
* var userovoDensity = userovoModel.create(function(val, data, separate){
*      if(separate){
...
* });
*/
userovoModel.create = function(fetchValue) {
    /**
    ...
    * @memberof module:api/lib/userovo.model~userovoMetric
    * @return {module:api/lib/userovo.common.periodObj} period object
    */
    userovoMetric.getPeriod = function() {
        return _period;
    };
    ...
    * @memberof module:api/lib/userovo.model~userovoMetric
    * @param {module:api/lib/userovo.common.periodObj} period - set period Object used by the model
    */
    userovoMetric.setPeriod = function(period) {
        _period = period;
    };
    ...
    */
    userovoMetric.setTotalUsersObj = function(totalUsersObj, prevTotalUserObj) {
    ...
    */
    userovoMetric.getTotalUsersObj = function(prev) {
        if (prev) {
    ...
    */
    userovoMetric.setUniqueMetrics = function(uniques) {
        _uniques = uniques;
    };
    ...
    */
    userovoMetric.getUniqueMetrics = function() {
        return _uniques;
    };
    ...
    */
    userovoMetric.setMetrics = function(metrics) {
        _metrics = metrics;
    };
    ...
    */
    userovoMetric.getMetrics = function() {
        return _metrics;
    };
    ...
    */
    userovoMetric.getMeta = function(metric) {
    ...
    */
    userovoMetric.getData = function(segment, clean, join) {
        if (segment) {
            let dataProps = [
                {
                    name: segment,
            ...
            let chartData = userovoCommon.extractTwoLevelData(_Db, this.getMeta(segment), this.clearObject, dataProps, _totalUsersObj);
            chartData.chartData = userovoCommon.mergeMetricsByName(chartData.chartData, segment);
            ...
            return userovoCommon.extractChartData(this.getDb(), this.clearObject, chartData, dataProps);
        }
    };
    ...
    */
    userovoMetric.getBars = function(segment, maxItems, metric) {
        var periodObject = null;
        if (this.getPeriod()) { // only set custom period if it was explicitly set on the model object
            periodObject = userovoCommon.getPeriodObj({qstring: {}}, this.getPeriod());
        }
        metric = metric || _metrics[0];
        if (segment) {
            return userovoCommon.extractBarData(_Db, this.getMeta(segment), this.clearObject, fetchValue, maxItems, metric, this.getTotalUsersObj(), this.fixBarSegmentData ? this.fixBarSegmentData.bind(null, segment) : undefined, periodObject);
        }
        else {
            var barData = [],
                sum = 0,
            ...
            var totalUserData = userovoCommon.extractChartData(this.getDb(), this.clearObject, chartData, dataProps),
            ...
            for (var i = topUsers.length - 1; i >= 0; i--) {
                var percent = userovoCommon.round((topUsers[i][metric] / sum) * 100, 1);
    ...
    *}
    */
    userovoMetric.getTableData = function(segment, maxItems) {
    ...
    */
    userovoMetric.getNumber = function(metric, isSparklineNotRequired) {
        var periodObject = null;
        if (this.getPeriod()) { // only set custom period if it was explicitly set on the model object
            periodObject = userovoCommon.getPeriodObj({qstring: {}}, this.getPeriod());
        }
        ...
        var data = userovoCommon.getDashboardData(this.getDb(), metrics, _uniques, { u: this.getTotalUsersObj().users }, { u: this.getTotalUsersObj(true).users }, periodObject);
        if (isSparklineNotRequired) {
            return data[metric];
        }
        var ob = {};
        ob[metric] = metric;
        var sparkLines = userovoCommon.getSparklineData(this.getDb(), ob, function(obj) {
            if (obj) {
                if (!obj[metric]) {
                    obj[metric] = 0;
                }
    ...
    * @returns {array} object to use when displaying number {value: 123, change: 12, sparkline: [1,2,3,4,5,6,7]}
    */
    userovoMetric.getTimelineData = function() {
        var dataProps = [];
        var periodObject = null;
        for (let i = 0; i < _metrics.length; i++) {
            dataProps.push({ name: _metrics[i] });
        ...
        var data = userovoCommon.extractData(this.getDb(), this.clearObject, dataProps, periodObject);
        var ret = {};
   ...
   * @memberof module:api/lib/userovo.model~userovoMetric
    ...
    * @returns {array} object to use when displaying bars as [{"name":"English","percent":44},{"name":"Italian","percent":29},{"name":"German","percent":27}]
    */
    userovoMetric.getStackedBarData = function(segment, maxItems, metric, displayType) {
        metric = metric || _metrics[0];
        if (!metric) {
            metric = "u";
        }
        ...
        var dd = userovoCommon.extractStackedBarData(this.getDb(), this.clearObject, chartData, dataProps, "", true);
    ...
    *  }
    **/
    userovoMetric.getRangeData = function(metric, meta, explain) {
        ...
        chartData.chartData = userovoCommon.extractRangeData(_Db, metric, this.getMeta(meta), explain);
    ...
    **/
    function extendMeta() {
        if (_Db.meta) {
            for (let i in _Db.meta) {
                _metas[i] = userovoCommon.union(_metas[i], _Db.meta[i]);
            }
        }
    }

    return userovoMetric;
};

module.exports = userovoModel;