/* global userovoCommon, userovoGlobal, jQuery */
...
(function(userovoCMS, $) {
    ...
    userovoCMS.requestFromCMS = function(params) {
        var pageSize = 100;
    ...
    userovoCMS.saveEntries = function(entryID, entries) {
        var formData = new FormData();
        formData.append('app_id', userovoCommon.ACTIVE_APP_ID);
        formData.append('api_key', userovoGlobal.member.api_key);
        formData.append('_id', entryID);
        formData.append('entries', JSON.stringify(entries));

        return new Promise(function(resolve, reject) {
            $.ajax({
                url: userovoCommon.API_PARTS.data.w + "/cms/save_entries",
                method: 'POST',
                data: formData,
                processData: false,
                contentType: false,
    ...
    userovoCMS.requestFromBackend = function(params) {
        return new Promise(function(resolve, reject) {
            $.ajax({
                url: userovoCommon.API_PARTS.data.r + "/cms/entries",
                data: params,
                success: function(response) {
                    resolve(response);
                },
    ...
    userovoCMS.requestFromCMSAndSave = function(params) {
        return new Promise(function(resolve, reject) {
            userovoCMS.requestFromCMS(params)
                .then(function(response) {
                    var transformedResponse = transformCMSResponse(response, params);

                    userovoCMS.saveEntries(params._id, transformedResponse);
    ...
    userovoCMS.fetchEntry = function(entryID, options) {
        ...
        if (options && options.CMSFirst) {
            ...
            return userovoCMS.requestFromCMSAndSave(params);
        }
        else {
            // Request from backend first
            return new Promise(function(resolve, reject) {
                userovoCMS.requestFromBackend(params)
                    .then(function(response) {
    ...
    userovoCMS.clearCache = function(entryID) {
        return new Promise(function(resolve, reject) {
            $.ajax({
                url: userovoCommon.API_PARTS.data.r + "/cms/clear",
                data: { _id: entryID },
                success: function(response) {
                    resolve(response);
                },
...
}(window.userovoCMS = window.userovoCMS || {}, jQuery));
