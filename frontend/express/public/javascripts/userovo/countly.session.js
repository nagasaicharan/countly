/* global UserovoHelpers, userovoSession, userovoLocation, userovoCommon, _, jQuery*/
(function() {

    window.userovoSession = window.userovoSession || {};
    UserovoHelpers.createMetricModel(window.userovoSession, {name: "users", estOverrideMetric: "users"}, jQuery);

    userovoSession.callback = function(isRefresh, data) {
        if (isRefresh) {
            userovoLocation.refresh(data);
        }
        else {
            userovoLocation.initialize();
        }
    };
    userovoSession.getSessionData = function(sessionModel) {
        if (!sessionModel) {
            sessionModel = userovoSession.getDb();
        }
        var map = {t: "total-sessions", n: "new-users", u: "total-users", d: "total-duration", e: "events", p: "paying-users", m: "messaging-users"};
        var ret = {};
        var data = userovoCommon.getDashboardData(sessionModel, ["t", "n", "u", "d", "e", "p", "m"], ["u", "p", "m"], {u: "users"}, userovoSession.clearObject);
        ...
        var changeReturning = userovoCommon.getPercentChange(
        ...
        ret["total-duration"].total = userovoCommon.timeString(ret["total-duration"].total);
        ret["total-duration"]["prev-total"] = userovoCommon.timeString(ret["total-duration"]["prev-total"]);
        ret["avg-duration-per-session"].total = userovoCommon.timeString(ret["avg-duration-per-session"].total);
        ret["avg-duration-per-session"]["prev-total"] = userovoCommon.timeString(ret["avg-duration-per-session"]["prev-total"]);
        ...
        var sparkLines = userovoCommon.getSparklineData(sessionModel, {
            "total-sessions": "t",
            "new-users": "n",
            "total-users": "u",
            "total-duration": "d",
    ...
    userovoSession.getSessionDP = function() {
        ...
        return userovoCommon.extractChartData(userovoSession.getDb(), userovoSession.clearObject, chartData, dataProps);
    };

    userovoSession.getSessionDPTotal = function() {
            ...
            dataProps = [
                {
                    ...
                    period: userovoCommon.getPeriod() === 'day' ? "previousThisMonth" : "previous"
                },
                { name: "t" }
            ];
    ...
    userovoSession.getUserDP = function() {
    ...
    userovoSession.getUserDPActive = function() {
    ...
    userovoSession.getUserDPNew = function() {
    ...
    userovoSession.getMsgUserDPActive = function() {

        var chartData = [
                { data: [], label: jQuery.i18n.map["common.table.total-users"], color: userovoCommon.GRAPH_COLORS[0]},
                { data: [], label: jQuery.i18n.map["common.table.messaging-users"], color: userovoCommon.GRAPH_COLORS[1] }
            ],
            dataProps = [
                {
                    name: "u"
    ...
    userovoSession.getDurationDP = function(use_seconds) {
    ...
    userovoSession.getDurationDPAvg = function(use_seconds) {
    ...
    userovoSession.getEventsDP = function() {
    ...
    userovoSession.getEventsDPAvg = function() {
    ...
    */
    ...
    userovoSession.getDurationRange = function() {
        return durationRange();
    };
    userovoSession.explainDurationRange = function(index) {
        return durationRange()[index];
    };

    userovoSession.getDurationIndex = function(duration) {
    ...
    */
    ...
    userovoSession.getFrequencyRange = function() {
        return frequencyRange();
    };
    userovoSession.explainFrequencyRange = function(index) {
        return frequencyRange()[index];
    };

    userovoSession.getFrequencyIndex = function(frequency) {
    ...
    */
    ...
    userovoSession.getLoyalityRange = function() {
        return loyaltyRange();
    };
    userovoSession.explainLoyaltyRange = function(index) {
        return loyaltyRange()[index];
    };

    userovoSession.getLoyaltyIndex = function(loyalty) {
        return loyaltyRange().indexOf(loyalty);
    };

    userovoSession.getTopUserBars = function() {
        ...
        var _db = userovoSession.getDb(),
            dashboardData = userovoCommon.getDashboardData(_db, ["t", "n", "u", "d", "e", "p", "m"], ["u", "p", "m"], {u: "users"}, userovoSession.clearObject),
            totalUserData = userovoCommon.extractChartData(_db, userovoSession.clearObject, chartData, dataProps),
    ...
    userovoSession.clearObject = function(obj) {
        if (obj) {
            if (!obj.t) {
                obj.t = 0;
            }
