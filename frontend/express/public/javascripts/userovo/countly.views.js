/* global userovoView, userovoCommon, app, UserovoHelpers, userovoGlobal, userovoTaskManager, userovoVersionHistoryManager, DownloadView, Backbone, jQuery, $*/
...
window.DownloadView = userovoView.extend({
    renderCommon: function() {
        var self = this;
        if (!this.task_id) {
        ...
        if (this.path) {
            ...
            self.link = userovoCommon.API_PARTS.data.r + self.path + self.task_id + "?auth_token=" + userovoGlobal.auth_token + "&app_id=" + userovoCommon.ACTIVE_APP_ID;
            ...
            userovoTaskManager.fetchResult(this.task_id, function(res) {
                myhtml = '<div id="no-app-type"><h1>' + jQuery.i18n.map["downloading-view.download-title"] + '</h1>';
                if (res && res.data) {
                    res.data = res.data.replace(new RegExp("&quot;", 'g'), "");
                    self.link = userovoCommon.API_PARTS.data.r + self.path + res.data + "?auth_token=" + userovoGlobal.auth_token + "&app_id=" + userovoCommon.ACTIVE_APP_ID;
                    window.location = self.link;
...
$.ajaxPrefilter(function(options, originalOptions, jqXHR) {
    ...
    if (userovoGlobal.auth_token) {
        var testurl = originalOptions.url;

        //if url is valid+auth_token and api_key not given
        if (testurl.indexOf(userovoCommon.API_PARTS.data.w) === 0 || testurl.indexOf(userovoCommon.API_PARTS.data.r) === 0) {
            //add token in header
            jqXHR.setRequestHeader('userovo-token', userovoGlobal.auth_token);
        }
...
app.addAppSwitchCallback(function() {
    $.when(userovoVersionHistoryManager.initialize()).then(function() {
        var versionsData = userovoVersionHistoryManager.getData(true) || {fs: [], db: [], pkg: ""};
        var dbVersion = userovoGlobal.userovoVersion;
        if (versionsData.db && versionsData.db.length > 0) {
            dbVersion = versionsData.db[versionsData.db.length - 1];
        }

        var fsVersion = userovoGlobal.userovoVersion;
...
function checkIfEventViewHaveNotUpdatedChanges() {
    if (app.eventsBlueprintView && app.eventsBlueprintView.preventHashChange === true) {
        ...
        if (movemeto !== "/analytics/events/blueprint") {
            UserovoHelpers.confirm(jQuery.i18n.map["events.general.want-to-discard"], "popStyleGreen", function(result) {
                if (!result) {
