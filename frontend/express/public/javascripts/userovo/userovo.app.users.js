/*global userovoCommon, userovoTaskManager, jQuery, $*/
(function(userovoAppUsers) {

    //export data for user based on passed id
    //callback(error, fileid(if exist), taskid(if exist))
    userovoAppUsers.exportUser = function(query, callback) {
        $.ajax({
            type: "POST",
    ...
    //callback(error, fileid(if exist), taskid(if exist))
    userovoAppUsers.deleteExport = function(eid, callback) {
        $.ajax({
            type: "POST",
    ...
    //callback(error, fileid(if exist), taskid(if exist))
    userovoAppUsers.deleteUserdata = function(query, force, callback) {
...
}(window.userovoAppUsers = window.userovoAppUsers || {}, jQuery));