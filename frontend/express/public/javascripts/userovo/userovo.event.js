/*global userovoCommon, _, userovoGlobal, jQuery*/
// eslint-disable-next-line no-shadow-restricted-names
(function(userovoEvent, $, undefined) {
    ...
    userovoEvent.hasLoadedData = function() {
    ...
    //Public Methods
    userovoEvent.initialize = function(forceReload) {

        if (!forceReload && _initialized && _period === userovoCommon.getPeriodForAjax() && _activeAppKey === userovoCommon.ACTIVE_APP_KEY) {
            return userovoEvent.refresh();
        }
        if (forceReload && userovoEvent.hasLoadedData()) {
            return true;
        }
                    ...
                    success: function(json) {
                        $.ajax({
                            ...
                            success: function(groups_json) {
                                ...
                                if (!_activeEvent && userovoEvent.getEvents()[0]) {
                                    _activeEvent = userovoEvent.getEvents()[0].key;
    ...
    userovoEvent.getEventGroups = function() {
        return _eventGroups;
    };

    userovoEvent.getEventGroupsTable = function(getStatus) {
    ...
    userovoEvent.getOverviewList = function() {
    ...
    userovoEvent.getOverviewData = function(callback) {
        var my_events = [];
        var _overviewData = [];
    ...
    userovoEvent.createEventGroup = function(data, callback) {
        _activeLoadedEvent = "";
        $.ajax({
            type: "POST",
    ...
    userovoEvent.deleteEventGroup = function(data, callback) {
        _activeLoadedEvent = "";
        $.ajax({
            type: "POST",
    ...
    userovoEvent.updateEventGroup = function(data, order, update_status, status, callback) {
        _activeLoadedEvent = "";
        $.ajax({
            type: "POST",
    ...
    userovoEvent.getEventGroupById = function(_id, callback) {
        $.ajax({
            type: "GET",
            url: userovoCommon.API_PARTS.data.r,
            data: {
    ...
    userovoEvent.getTopEventData30Day = function(callback) {
        return $.when($.ajax({
            type: "GET",
            url: userovoCommon.API_PARTS.data.r,
            data: {
    ...
    userovoEvent.getTopEventDataDaily = function(callback) {
        return $.when($.ajax({
            type: "GET",
            url: userovoCommon.API_PARTS.data.r,
            data: {
    ...
    userovoEvent.update_map = function(event_map, event_order, event_overview, omitted_segments, callback) {
        _activeLoadedEvent = "";
        $.ajax({
            type: "POST",
    ...
    userovoEvent.update_visibility = function(my_events, visibility, callback) {
        _activeLoadedEvent = "";
        $.ajax({
            type: "POST",
            url: userovoCommon.API_PARTS.data.w + "/events/change_visibility",
            data: {
                "app_id": userovoCommon.ACTIVE_APP_ID,
    ...
    //Deletes events
    userovoEvent.delete_events = function(my_events, callback) {
        _activeLoadedEvent = "";
        $.ajax({
            type: "POST",
            ...
            data: {
                "app_id": userovoCommon.ACTIVE_APP_ID,
                "events": userovoCommon.decodeHtml(JSON.stringify(my_events))
            },
            success: function() {
                callback(true);
            },
                        ...
                        $.ajax({
                            ...
                            success: function(json) {
                                if (currentActiveEvent === _activeEvent && currentActiveSegmentation === _activeSegmentation) {
                                    _activeLoadedEvent = _activeEvent;
                                    _activeLoadedSegmentation = _activeSegmentation;
                                    userovoCommon.extendDbObj(_activeEventDb, json);
                                    extendMeta();
                                }
                            }
                        })).then(
    ...
    userovoEvent.refreshEvents = function() {
        if (!userovoCommon.DEBUG) {
    ...
    userovoEvent.setActiveEvent = function(activeEvent, callback) {
        var persistData = {};
        persistData["activeEvent_" + userovoCommon.ACTIVE_APP_ID] = activeEvent;
        ...
        $.when(userovoEvent.initialize(true)).then(callback);
    };

    userovoEvent.setActiveSegmentation = function(activeSegmentation, callback) {
        _activeEventDb = {};
    ...
    userovoEvent.getEventData = function() {
        ...
        if (_activeSegmentation) {
            ...
            var tmpEventData = userovoCommon.extractTwoLevelData(_activeEventDb, _activeSegmentationValues, userovoEvent.clearEventsObject, [
                {
                    name: "curr_segment",
            ...
            var segments = _.pluck(eventData.chartData, "curr_segment").slice(0, 15),
                ...
                chartDP = [
                    {data: [], color: userovoCommon.GRAPH_COLORS[0]}
                ];
            ...
            chartDP[chartDP.length] = {data: [], color: userovoCommon.GRAPH_COLORS[1]};
            chartDP[1].data[0] = [-1, null];
            chartDP[1].data[segments.length + 1] = [segments.length, null];

            chartDP[chartDP.length] = {data: [], color: userovoCommon.GRAPH_COLORS[2]};
            ...
            if (_eventGroups[_activeEvent]) {
                ...
                eventData.eventName = userovoEvent.getEventLongName(_activeEvent);
            ...
            var chartData = [
                    { data: [], label: countString, color: userovoCommon.GRAPH_COLORS[0] },
                    { data: [], label: sumString, color: userovoCommon.GRAPH_COLORS[1] },
                    { data: [], label: durString, color: userovoCommon.GRAPH_COLORS[2] }
                ],
                dataProps = [
                    { name: "c" },
                    { name: "s" },
            ...
            eventData = userovoCommon.extractChartData(_activeEventDb, userovoEvent.clearEventsObject, chartData, dataProps);
    ...
    userovoEvent.getEvents = function(get_hidden, with_groups) {
    ...
    userovoEvent.getEventsWithSegmentations = function() {
    ...
    userovoEvent.getEventMap = function(get_hidden, with_groups) {
        var events = userovoEvent.getEvents(get_hidden, with_groups),
            eventMap = {};
    ...
    userovoEvent.getEventLongName = function(eventKey) {
        var eventMap = (_activeEvents) ? ((_activeEvents.map) ? _activeEvents.map : {}) : {};
        eventKey = "" + eventKey;
    ...
    userovoEvent.getEventSegmentations = function() {
        return _activeSegmentations;
    };

    userovoEvent.clearEventsObject = function(obj) {
        if (obj) {
            if (!obj.c) {
                obj.c = 0;
            }
    ...
    userovoEvent.getEventSummary = function() {
        ...
        if (_periodObj.isSpecialPeriod) {
            for (var i = 0; i < (_periodObj.currentPeriodArr.length); i++) {
                tmp_x = userovoCommon.getDescendantProp(_activeEventDb, _periodObj.currentPeriodArr[i]);
                tmp_y = userovoCommon.getDescendantProp(_activeEventDb, _periodObj.previousPeriodArr[i]);
                tmp_x = userovoEvent.clearEventsObject(tmp_x);
                tmp_y = userovoEvent.clearEventsObject(tmp_y);
            ...
            tmp_x = userovoCommon.getDescendantProp(_activeEventDb, _periodObj.activePeriod);
            tmp_y = userovoCommon.getDescendantProp(_activeEventDb, _periodObj.previousPeriod);
            tmp_x = userovoEvent.clearEventsObject(tmp_x);
            tmp_y = userovoEvent.clearEventsObject(tmp_y);
        ...
        var	changeTotal = userovoCommon.getPercentChange(previousTotal, currentTotal),
            changeSum = userovoCommon.getPercentChange(previousSum, currentSum),
            changeDur = userovoCommon.getPercentChange(previousDur, currentDur);
        ...
        dataArr =
        {
            usage: {
                ...
                "event-dur": {
                    "total": userovoCommon.formatSecond(currentDur),
    ...
    userovoEvent.getMultiEventData = function(eventKeysArr, callback) {
        $.ajax({
            type: "GET",
            url: userovoCommon.API_PARTS.data.r,
            data: {
        ...
        */
        function extractDataForGraphAndChart(dataFromDb) {
            ...
            eventData = userovoCommon.extractChartData(dataFromDb, userovoEvent.clearEventsObject, chartData, dataProps);
    ...
    function extendMeta() {
        for (var metaObj in _activeEventDb.meta) {
            if (_activeSegmentationObj[metaObj] && _activeEventDb.meta[metaObj] && _activeSegmentationObj[metaObj].length !== _activeEventDb.meta[metaObj].length) {
                _activeSegmentationObj[metaObj] = userovoCommon.union(_activeSegmentationObj[metaObj], _activeEventDb.meta[metaObj]);
            }
        }
     ...
     */
    userovoEvent.getEventsForApps = function(appIds, callback) {
        /**
        ...
        function extractEvents(data, returnArray) {
            ...
            if (eventData && eventData.list) {
                for (var j = 0; j < eventData.list.length; j++) {
                    var eventNamePostfix = (appIds.length > 1) ? " (" + ((userovoGlobal.apps[eventData._id] && userovoGlobal.apps[eventData._id].name) || "Unknown") + ")" : "";
    ...
    userovoEvent.getLimitation = function() {
        return _activeEvents.limits;
    };
    userovoEvent.getActiveEventSegmentMeta = function() {
        return _activeEventDb.meta;
    };
}(window.userovoEvent = window.userovoEvent || {}, jQuery));
