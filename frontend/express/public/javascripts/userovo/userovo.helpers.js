/* global _, userovoGlobal, userovoCommon, _JSONEditor, app, TableTools, userovoDeviceDetails, moment, jQuery, $, store, Handlebars, userovoTaskManager, userovoVue*/
/*
 ...
 * @name UserovoHelpers
 * @global
 * @namespace UserovoHelpers
 */
(function(UserovoHelpers) {

    /**
    * This function checks if a Userovo plugin is enabled.
    ...
    */
    UserovoHelpers.isPluginEnabled = function(name) {
        if (userovoGlobal && userovoGlobal.pluginsFull && Array.isArray(userovoGlobal.pluginsFull)) {
            if (!Array.isArray(name)) {
                name = [name];
            }
            var isPluginsFull = false;
            for (var i = 0; i < name.length; i++) {
                if (userovoGlobal.pluginsFull.indexOf(name[i]) > -1) {
                    isPluginsFull = true;
                    break;
                }
            }
            if (isPluginsFull && userovoGlobal.plugins && Array.isArray(userovoGlobal.plugins)) {
                for (var j = 0; j < name.length; j++) {
                    if (userovoGlobal.plugins.indexOf(name[j]) > -1) {
                        return true;
                    }
                }
                return false;
    ...
    UserovoHelpers.logout = function(path) {
        if (path) {
    ...
    /**
    * Legacy method for displaying notifications. User {@link UserovoHelpers.notify} instead
    * @param {string} msg - msg to display
    * @returns {boolean} true - if message is not defined, else returns nothing
    */
    UserovoHelpers.parseAndShowMsg = function(msg) {
        if (!msg || !msg.length) {
            return true;
        }
        ...
        UserovoHelpers.notify({type: type, message: message});

        delete userovoGlobal.message;
    };
    /**
    ...
    * @example
    * UserovoHelpers.confirmWithCheckbox("Are you sure?", "red", true, "Chechbox label text", function (result) {
    *    if (!result) {
    ...
    * });
    */
    UserovoHelpers.confirmWithCheckbox = function(msg, type, hasCheckbox, checkboxTitle, callback, buttonText) {
    ...
    * @example
    * var drawer = UserovoHelpers.createDrawer({
    *           id: "my-id",
    ...
    *    
    */
    UserovoHelpers.createDrawer = function(options) {
    ...
    * });
    */
    UserovoHelpers.notify = function(msg) {
        ...
        if (msg.html) {
            ...
            payload.text = userovoCommon.encodeHtml(msg.message);
        }
        ...
        if (persistent) {
            userovoCommon.dispatchPersistentNotification(payload);
        }
        else {
            userovoCommon.dispatchNotificationToast(payload);
        }
    };
     ...
     */
    UserovoHelpers.removePersistentNotification = function(notificationId) {
        userovoCommon.removePersistentNotification(notificationId);
    };
     ...
     */
    UserovoHelpers.goTo = function(options) {
     ...
     */
    UserovoHelpers.getBacklink = function() {
     ...
     */
    UserovoHelpers.isActiveAppMobile = function() {
        return userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].type === 'mobile';
    },
    /**
    * Create new model
    */
    UserovoHelpers.model = function() {
        var self = this;
    ...
    */
    UserovoHelpers.newJSONEditor = function(json, type, callback) {
        var self = this;
    ...
    UserovoHelpers.blinkDots = function(times, speed, element) {
    ...
    UserovoHelpers.stopBlinking = function(element) {
        $(element).removeClass("blink");
    };
    UserovoHelpers.applyColors = function() {
        $('#custom-color-styles').remove();
        // overview bars
        var barStyles = '<style id="custom-color-styles">';
        barStyles += '.dashboard-summary .item.light .bar .bar-inner:nth-child(1) { background-color: ' + userovoCommon.GRAPH_COLORS[0] + '}';
        barStyles += '.dashboard-summary .item.light .bar .bar-inner:nth-child(2) { background-color: ' + userovoCommon.GRAPH_COLORS[1] + '}';
        barStyles += '.dashboard-summary .item.light .bar .bar-inner:nth-child(3) { background-color: ' + userovoCommon.GRAPH_COLORS[2] + '}';
        barStyles += '.dashboard-summary .item .bar .bar-inner-new .bar-inner-percent{color:' + userovoCommon.GRAPH_COLORS[0] + ';}';
        barStyles += '.dashboard-summary .item .bar .bar-inner-new:nth-child(2) .bar-inner-percent{color:' + userovoCommon.GRAPH_COLORS[1] + ';}';
        barStyles += '.dashboard-summary .item .bar .bar-inner-new:nth-child(3) .bar-inner-percent{color:' + userovoCommon.GRAPH_COLORS[2] + ';}';
        barStyles += '.dashboard-summary .item .bar .bar-inner-new::before{background-color:' + userovoCommon.GRAPH_COLORS[0] + ';}';
        barStyles += '.dashboard-summary .item .bar .bar-inner-new:nth-child(2)::before{background-color:' + userovoCommon.GRAPH_COLORS[1] + ';}';
        barStyles += '.dashboard-summary .item .bar .bar-inner-new:nth-child(3)::before{background-color:' + userovoCommon.GRAPH_COLORS[2] + ';}';
        barStyles += '.big-numbers-v2 .big-numbers.radio:nth-child(1) .color, .big-numbers-v2 .big-numbers.check:nth-child(1) .color {border: 1px solid ' + userovoCommon.GRAPH_COLORS[0] + ';}';
        barStyles += '.big-numbers-v2 .big-numbers.radio:nth-child(2) .color, .big-numbers-v2 .big-numbers.check:nth-child(2) .color, .big-numbers-v2 .big-numbers.check.event-sum .color {border: 1px solid ' + userovoCommon.GRAPH_COLORS[1] + ';}';
        barStyles += '.big-numbers-v2 .big-numbers.radio:nth-child(3) .color, .big-numbers-v2 .big-numbers.check:nth-child(3) .color, .big-numbers-v2 .big-numbers.check.event-dur .color {border: 1px solid ' + userovoCommon.GRAPH_COLORS[2] + ';}';
        barStyles += '.big-numbers-v2 .big-numbers.radio:nth-child(4) .color, .big-numbers-v2 .big-numbers.check:nth-child(4) .color {border: 1px solid ' + userovoCommon.GRAPH_COLORS[3] + ';}';
        barStyles += '.big-numbers-v2 .big-numbers.radio:nth-child(5) .color, .big-numbers-v2 .big-numbers.check:nth-child(5) .color {border: 1px solid ' + userovoCommon.GRAPH_COLORS[4] + ';}';
        barStyles += '.big-numbers-v2 .big-numbers.radio:nth-child(1).selected .color, .big-numbers-v2 .big-numbers.check:nth-child(1).selected .color {background-color:' + userovoCommon.GRAPH_COLORS[0] + '; box-shadow: inset 0 0 0 1px #FFF; border: 1px solid ' + userovoCommon.GRAPH_COLORS[0] + ';}';
        barStyles += '.big-numbers-v2 .big-numbers.radio:nth-child(2).selected .color, .big-numbers-v2 .big-numbers.check:nth-child(2).selected .color, .big-numbers-v2 .big-numbers.check.event-sum.selected .color {background-color:' + userovoCommon.GRAPH_COLORS[1] + '; box-shadow: inset 0 0 0 1px #FFF; border: 1px solid ' + userovoCommon.GRAPH_COLORS[1] + ';}';
        barStyles += '.big-numbers-v2 .big-numbers.radio:nth-child(3).selected .color, .big-numbers-v2 .big-numbers.check:nth-child(3).selected .color, .big-numbers-v2 .big-numbers.check.event-dur.selected .color {background-color:' + userovoCommon.GRAPH_COLORS[2] + '; box-shadow: inset 0 0 0 1px #FFF; border: 1px solid ' + userovoCommon.GRAPH_COLORS[2] + ';}';
        barStyles += '.big-numbers-v2 .big-numbers.radio:nth-child(4).selected .color, .big-numbers-v2 .big-numbers.check:nth-child(4).selected .color {background-color:' + userovoCommon.GRAPH_COLORS[3] + '; box-shadow: inset 0 0 0 1px #FFF; border: 1px solid ' + userovoCommon.GRAPH_COLORS[3] + ';}';
        barStyles += '.big-numbers-v2 .big-numbers.radio:nth-child(5).selected .color, .big-numbers-v2 .big-numbers.check:nth-child(5).selected .color {background-color:' + userovoCommon.GRAPH_COLORS[4] + '; box-shadow: inset 0 0 0 1px #FFF; border: 1px solid ' + userovoCommon.GRAPH_COLORS[4] + ';}';
        barStyles += '.big-numbers:nth-child(1) .color {background-color: ' + userovoCommon.GRAPH_COLORS[0] + ';box-shadow: inset 0 0 0 1px #FFF; border: 1px solid ' + userovoCommon.GRAPH_COLORS[0] + ';}';
        barStyles += '.big-numbers:nth-child(2) .color {background-color: ' + userovoCommon.GRAPH_COLORS[1] + ';box-shadow: inset 0 0 0 1px #FFF; border: 1px solid ' + userovoCommon.GRAPH_COLORS[1] + ';}';
        barStyles += '.big-numbers:nth-child(3) .color {background-color: ' + userovoCommon.GRAPH_COLORS[2] + ';box-shadow: inset 0 0 0 1px #FFF; border: 1px solid ' + userovoCommon.GRAPH_COLORS[2] + ';}';
        barStyles += '</style>';
    ...
    * @example
    * UserovoHelpers.popup("<h1>Hello</h1>", "red", true);
    */
    UserovoHelpers.popup = function(element, custClass, isHTML) {
    ...
    * @example
    * UserovoHelpers.openResource("http://resources.count.ly/docs");
    */
    UserovoHelpers.openResource = function(url) {
    ...
    * @example
    * UserovoHelpers.alert("Some error happened", "red");
    */
    UserovoHelpers.alert = function(msg, type, moreData) {
        if (userovoGlobal.ssr) {
            return;
        }
        ...
        if (window.userovoVue && window.userovoVue.vuex) {
            ...
            var confirmLabel = userovoVue.i18n('common.ok'),
            ...
            var payload = {
                intent: 'message',
                message: (moreData && moreData.title) ? userovoCommon.encodeSomeHtml(msg) : "",
                type: convertedType,
                confirmLabel: confirmLabel,
                title: (moreData && moreData.title) || userovoCommon.encodeSomeHtml(msg),
                image: moreData && moreData.image
            };

            var currentStore = window.userovoVue.vuex.getGlobalStore();
            if (currentStore) {
                currentStore.dispatch('userovoCommon/onAddDialog', payload);
            }
        }
    };
    ...
    * @example
    * UserovoHelpers.confirm("Are you sure?", "red", function (result) {
    *    if (!result) {
    *        //user did not confirm, just exit
    *        return true;
    *    }
    ...
    * });
    */
    UserovoHelpers.confirm = function(msg, type, callback, buttonText, moreData, testId = 'cly-confirm-test-id') {
        if (userovoGlobal.ssr) {
            return;
        }
        ...
        if (window.userovoVue && window.userovoVue.vuex) {
            ...
            var cancelLabel = userovoVue.i18n('common.cancel'),
                confirmLabel = userovoVue.i18n('common.continue'),
            ...
            var payload = {
                intent: 'confirm',
                message: userovoCommon.encodeSomeHtml(msg),
                type: convertedType,
                confirmLabel: confirmLabel,
    ...
    * @example
    * var dialog = UserovoHelpers.loading("we are doing something");
    * //later when done
    * UserovoHelpers.removeDialog(dialog);
    */
    UserovoHelpers.loading = function(msg) {
    ...
    * @param {string} moreData.title - alert title
    * @example
    * UserovoHelpers.showBlockerDialog("Some message");
    */
    UserovoHelpers.showBlockerDialog = function(msg, moreData) {
        if (userovoGlobal.ssr) {
            return;
        }
    ...
    * @example
    * UserovoHelpers.showQuickstartDialog();
    */
    UserovoHelpers.showQuickstartPopover = function(content) {
        if (userovoGlobal.ssr) {
            return;
        }
    ...
    * @example
    * UserovoHelpers.isJSON(variable);
    */
    UserovoHelpers.isJSON = function(val) {
        try {
            JSON.parse(val);
            return true;
        }
    ...
    * @returns {object} jQuery object reference to dialog
    * @example
    * var dialog = UserovoHelpers.export(300000);
    * //later when done
    * UserovoHelpers.removeDialog(dialog);
    */
    UserovoHelpers.export = function(count, data, asDialog, exportByAPI, instance) {
        //var hardLimit = userovoGlobal.config.export_limit;
        ...
        dialog.find(".export-data").click(function() {
            ...
            var url = userovoCommon.API_URL + (exportByAPI ? "/o/export/request" : "/o/export/db");
            if (data.url) {
                url = data.url;
            }
            ...
            if (exportByAPI && url === "/o/export/requestQuery") {
                ...
                $.ajax({
                    type: "POST",
                    url: userovoCommon.API_PARTS.data.r + "/export/requestQuery",
                    data: data,
                    success: function(result) {
                        ...
                        if (result && result.result && result.result.task_id) {
                            ...
                            userovoTaskManager.monitor(task_id);
                            UserovoHelpers.displayExportStatus(null, fileid, task_id);
                        }
                    ...
                    error: function(xhr, status, error) {
                        ...
                        UserovoHelpers.displayExportStatus(error, filename, null);
                    }
                });
            }
            else {
    ...
    UserovoHelpers.displayExportStatus = function(error, export_id, task_id) {
        if (error) {
            ...
            UserovoHelpers.notify({
                ...
                clearAll: true,
                onClick: function() {
                    var win = window.open(userovoCommon.API_PARTS.data.r + "/export/download/" + task_id + "?auth_token=" + userovoGlobal.auth_token + "&app_id=" + userovoCommon.ACTIVE_APP_ID, '_blank');
                    win.focus();
                }
            });
            self.refresh();
            ...
            UserovoHelpers.notify({type: "ok", title: jQuery.i18n.map["common.success"], message: jQuery.i18n.map["export.export-started"], sticky: false, clearAll: false});
            // self.refresh();
        }
        else {
            UserovoHelpers.alert(jQuery.i18n.map["export.export-failed"], "red");
        }
    };
    ...
    * UserovoHelpers.removeDialog(dialog);
    */
    UserovoHelpers.tableExport = function(dtable, data, asDialog, oSettings) {
        ...
        */
        function getFileName() {
            ...
            if ($(".widget #date-selector").length) {
                ...
                name += "_for_" + userovoCommon.getDateRange();
            }
            else {
                //include export date
        ...
        */
        ...
        dialog.find(".export-data").click(function() {
            ...
            var url = userovoCommon.API_URL + "/o/export/data";
            var form = $('<form method="POST" action="' + url + '">');
    ...
    * UserovoHelpers.revealDialog(dialog);
    */
    UserovoHelpers.revealDialog = function(dialog) {
        $("body").append(dialog);
        ...
        UserovoHelpers.makeSelectNative();
    };
    ...
    * UserovoHelpers.changeDialogHeight(dialog, true)
    */
    UserovoHelpers.changeDialogHeight = function(dialog, animate) {
        var dialogHeight = 0,
    ...
    var revealDialog = UserovoHelpers.revealDialog;

    //var changeDialogHeight = UserovoHelpers.changeDialogHeight; - not used anywhere anymore
    ...
    * UserovoHelpers.removeDialog(dialog);
    */
    UserovoHelpers.removeDialog = function(dialog) {
        dialog.remove();
        $("#overlay").fadeOut();
    };

    UserovoHelpers.setUpDateSelectors = function(self) {
        ...
        $("#date-selector").find(".date-selector").click(function() {
            ...
            if (userovoCommon.getPeriod() === selectedPeriod) {
                return true;
            }

            userovoCommon.setPeriod(selectedPeriod);
            ...
            $("#selected-date").text(userovoCommon.getDateRangeForCalendar());

        });

        $("#date-selector").find(".date-selector").each(function() {
            if (userovoCommon.getPeriod() === $(this).attr("id")) {
    ...
    /**
    * Initialize userovo dropdown select. In most cases it is done automatically, only in some cases, when content loaded via ajax request outside of view lifecycle, you may need to initialize it yourself for your content specifically
    * @param {object} element - jQuery object reference
    * @example
    * UserovoHelpers.initializeSelect($("#my-dynamic-div"));
    */
    UserovoHelpers.initializeSelect = function(element) {
    ...
    UserovoHelpers.makeSelectNative = function() {
    ...
    /**
    * Initialize userovo dropdown multi select. In most cases it is done automatically, only in some cases, when content loaded via ajax request outside of view lifecycle, you may need to initialize it yourself for your content specifically
    * @param {object} element - jQuery object reference
    * @example
    * UserovoHelpers.initializeMultiSelect($("#my-dynamic-div"));
    */
    UserovoHelpers.initializeMultiSelect = function(element) {
        element = element || $("body");
    ...
    * });
    */
    UserovoHelpers.initializeTableOptions = function(element) {
        element = element || $('body');
     ...
     * 
     *  Example:
     *   UserovoHelpers.addColumnSelector(dtable,{"disabled:"{"name1":true,"name2":true},maxCol:6},"myTableName");
     *  Safe way would be adding in "fnInitComplete" function:
     *
     *  "fnInitComplete": function(oSettings, json) {
     *      $.fn.dataTable.defaults.fnInitComplete(oSettings, json);
     *      UserovoHelpers.addColumnSelector(this, {"disabled":{"name1":true,"name2":true}, "maxCol":5 }, "viewsTable");
     * },
     */
    UserovoHelpers.addColumnSelector = function(dtable, config, tableName) {
        config = config || {};
        ...
        $($(dtable[0]).parent().find(".data-table-column-selector")).on("click", "td", function() {
            ...
            if (!(config.disabled && config.disabled[$(this).data("selectorname")] && config.disabled[$(this).data("selectorname")] === true)) {
                if (isChecked) {
                    ...
                    UserovoHelpers.changeDTableColVis(config, dtable, tableName, $(this).data("selectorname"), false);
                }
                else {
                    if (UserovoHelpers.changeDTableColVis(config, dtable, tableName, $(this).data("selectorname"), true)) {
                        $(checkbox).removeClass("fa-square-o");
                        $(checkbox).addClass("fa-check-square");
     ...
     */
    UserovoHelpers.changeDTableColVis = function(config, dtable, tableName, col, visible) {
        var settings = store.get(tableName + "VisibleDataTableColumns") || {};
    ...
    * @example
    * UserovoHelpers.refreshTable(self.dtable, data);
    */
    UserovoHelpers.refreshTable = function(dTable, newDataArr) {
    ...
    *      ]
    *  }));
    *  UserovoHelpers.expandRows(this.dtable, formatData);
    */
    UserovoHelpers.expandRows = function(dTable, getData, context) {
        dTable.aOpen = [];
    ...
    UserovoHelpers.expandRowIconColumn = function() {
        return {
            "mData":
            function() {
    ...
    /**
    * If you allow to open/expand rows, then when refreshing table they will close again. To avoid that you must call this function on each refresh after calling {@link UserovoHelpers.refreshTable}
    * @param {object} dTable - jQuery object datatable reference
    * @param {function} getData - callback function to be called when clicking ont he row. This function will receive original row data object you passed to data tables and should return HTML string to display in subcell
    * @param {object} context - this context if needed, which will be passed to getData function as second parameter
    * @example
    * UserovoHelpers.refreshTable(self.dtable, data);
    * UserovoHelpers.reopenRows(self.dtable, formatData);
    */
    UserovoHelpers.reopenRows = function(dTable, getData, context) {
        var nTr;
        if (dTable.aOpen) {
    ...
    * @param {object} dTable - jQuery object datatable reference
    * @example
    * UserovoHelpers.closeRows(self.dtable);
    */
    UserovoHelpers.closeRows = function(dTable) {
        if (dTable.aOpen) {
            $.each(dTable.aOpen, function(i, id) {
                var nTr = $("#" + id)[0];
                $(nTr).removeClass("selected");
    ...
    * UserovoHelpers.appIdsToNames(["586e3216326a8b0a07b8d87f", "586e339a326a8b0a07b8ecb9", "586e3343c32cb30a01558cc3"]);
    */
    UserovoHelpers.appIdsToNames = function(context) {
        var ret = "";
        ...
        for (var i = 0; i < context.length; i++) {
            ...
            else if (!userovoGlobal.apps[context[i]]) {
                ret += 'deleted app';
            }
            else {
                ret += userovoGlobal.apps[context[i]].name;
            }
    ...
    * @example
    * UserovoHelpers.loadJS("/myplugin/javascripts/custom.js");
    */
    UserovoHelpers.loadJS = function(js, callback) {
    ...
    * @param {callback=} callback - callback when file loaded
    * @example
    * UserovoHelpers.loadCSS("/myplugin/stylesheets/custom.css");
    */
    UserovoHelpers.loadCSS = function(css, callback) {
    ...
    UserovoHelpers.messageText = function(messagePerLocale) {
        if (!messagePerLocale) {
            return '';
        }
    ...
    */
    UserovoHelpers.clip = function(f, nothing) {
        return function(opt) {
    ...
    /**
    * Create Userovo metric model to fetch metric data from server and provide it to views
    * @param {object} userovoMetric - initial metric object if you want to pre provide some methods, etc
    ...
    *        return false;
    *   };
    *   UserovoHelpers.createMetricModel(window.userovoDensity, {name: "density", estOverrideMetric: "densities"}, jQuery, function(val, data, separate){
    *        if(separate){
    ...
    */
    UserovoHelpers.createMetricModel = function(userovoMetric, metric, $, fetchValue) {
        ...
        */
        ...
        userovoMetric.getCurrentLoadState = function() {
        ...
        *    return $.when(userovoMetric.initialize()).then(function () {});
        * }
        */
        userovoMetric.initialize = function(processed) {

            var periodToFetch = userovoCommon.getPeriodForAjax();

            var key = userovoCommon.ACTIVE_APP_ID + "-" + _name + "-" + periodToFetch;
            var key_refresh = userovoCommon.ACTIVE_APP_ID + "-" + _name + "-refresh";
            if (_promises[key]) {
            ...
            if (_initialized && _period === periodToFetch && _activeAppKey === userovoCommon.ACTIVE_APP_KEY) {
                return this.refresh();
            }
            _period = userovoCommon.getPeriodForAjax();
            ...
            if (!userovoCommon.DEBUG) {
                ...
                if (processed) {
                    ...
                    return $.ajax({
                        type: "GET",
                        url: userovoCommon.API_PARTS.data.r + "/analytics/metric",
                        data: {
                        ...
                        success: function(json) {
                            _Db = json;
                            if (userovoMetric.callback) {
                                userovoMetric.callback(false, json);
                            }
                        }
                    });
                }
                ...
                _Db = {"2012": {}};
                if (userovoMetric.callback) {
                    userovoMetric.callback(false, _Db);
                }
                return true;
            }
        };
        ...
        * @returns {jquery_promise} jquery promise to wait while data is loaded
        * @example
        *$.when(userovoMetric.refresh()).then(function () {
        *    //data loaded, do something
        *});
        */
        userovoMetric.refresh = function() {
            if (!userovoCommon.DEBUG) {
                ...
                if (_processed) {
                    if (userovoMetric.callback) {
                        userovoMetric.callback(true);
                    }
                }
                else {
                    var key = userovoCommon.ACTIVE_APP_ID + "-" + _name + "-refresh";
                    if (_promises[key]) {
                        return _promises[key]; //we are currently running request for that. So return that.
                    }
                    _promises[key] = $.ajax({
                        ...
                        success: function(json) {
                            userovoCommon.extendDbObj(_Db, json);
                            extendMeta();
                            if (userovoMetric.callback) {
                                userovoMetric.callback(true, json);
                            }
                            delete _promises[key];
                        },
                        error: function() {
                ...
                _Db = {"2012": {}};
                if (userovoMetric.callback) {
                    userovoMetric.callback(true, _Db);
                }
                return true;
            }
        };
        ...
        */
        userovoMetric.setDb = function(db) {
            _Db = db;
            setMeta();
        };
        ...
        */
        userovoMetric.extendDb = function(data) {
            userovoCommon.extendDbObj(_Db, data);
            extendMeta();
        };
        ...
        */
        userovoMetric.getMeta = function(metric1) {
        ...
        *}}
        */
        userovoMetric.getData = function(clean, join, metric1, estOverrideMetric) {
            var chartData = {};
            var i = 0;
            if (_processed) {
                chartData.chartData = [];
                ...
                for (i = 0; i < _Db.length; i++) {
                    if (fetchValue && !clean) {
                        data[i][metric1 || _name] = fetchValue(userovoCommon.decode(data[i]._id));
                    }
                    else {
                        data[i][metric1 || _name] = userovoCommon.decode(data[i]._id);
                    }
                    chartData.chartData[i] = data[i];
                }
            }
            else {
                chartData = userovoCommon.extractTwoLevelData(_Db, this.getMeta(metric1), this.clearObject, [
                    {
                        name: metric1 || _name,
                        func: function(rangeArr) {
                            rangeArr = userovoCommon.decode(rangeArr);
                            if (fetchValue && !clean) {
            ...
            chartData.chartData = userovoCommon.mergeMetricsByName(chartData.chartData, metric1 || _name);
        ...
        */
        userovoMetric.clearObject = function(obj) {
            if (obj) {
                if (!obj.t) {
                    obj.t = 0;
                }
        ...
        */
        userovoMetric.getBarsWPercentageOfTotal = function(segment, mtric, estOverrideMetric) {
            mtric = mtric || "t";
            if (_processed) {
                var rangeData = {};
                rangeData.chartData = [];
                ...
                for (var i = 0; i < _Db.length; i++) {
                    if (fetchValue) {
                        data[i].range = fetchValue(userovoCommon.decode(data[i]._id));
                    }
                    else {
                        data[i].range = userovoCommon.decode(data[i]._id);
                    }
                    rangeData.chartData[i] = data[i];
                }

                return userovoCommon.calculateBarDataWPercentageOfTotal(rangeData, mtric, this.fixBarSegmentData ? this.fixBarSegmentData.bind(null, segment) : undefined);
            }
            else {
                return userovoCommon.extractBarDataWPercentageOfTotal(_Db, this.getMeta(segment), this.clearObject, fetchValue, mtric, estOverrideMetric, this.fixBarSegmentData ? this.fixBarSegmentData.bind(null, segment) : undefined);
            }
        };
        ...
        * @returns {array} object to use when displaying bars as [{"name":"English","percent":44},{"name":"Italian","percent":29},{"name":"German","percent":27}]
        */
        userovoMetric.getBars = function(metric_pd) {
            if (_processed) {
                var rangeData = {};
                rangeData.chartData = [];
                var data = JSON.parse(JSON.stringify(_Db));
                ...
                return userovoCommon.extractBarData(_Db, this.getMeta(metric_pd), this.clearObject, fetchValue);
            }
        };
        ...
        * //call
        * //userovoMetric.getOSSegmentedData("wp")
        ...
        *]}
        */
        userovoMetric.getOSSegmentedData = function(os, clean, metric_pd, estOverrideMetric) {
            ...
            if (_processed) {
                ...
                for (i = 0; i < _Db.length; i++) {
                    if (fetchValue && !clean) {
                        data[i][metric_pd || _name] = fetchValue(userovoCommon.decode(data[i]._id));
                    }
                    else {
                        data[i][metric_pd || _name] = userovoCommon.decode(data[i]._id);
                    }
                    oSVersionData.chartData[i] = data[i];
                }
            }
            else {
                oSVersionData = userovoCommon.extractTwoLevelData(_Db, this.getMeta(metric_pd), this.clearObject, [
                    {
            ...
            oSVersionData = userovoDeviceDetails.eliminateOSVersion(oSVersionData, osSegmentation, metric_pd || _name, false);
            ...
            for (i = 0; i < platformVersionNames.length; i++) {
                chartData2[chartData2.length] = {
                    ...
                    label: platformVersionNames[i].replace(((userovoDeviceDetails.os_mapping[osSegmentation.toLowerCase()]) ? userovoDeviceDetails.os_mapping[osSegmentation.toLowerCase()].name : osSegmentation) + " ", "")
                };
            }
        ...
        * @example <caption>Example output</caption>
        * //call
        * //userovoMetric.getRangeData("f", "f-ranges", userovoSession.explainFrequencyRange);
        * //returns
        * {"chartData":[
        ...
        *  }
        **/
        userovoMetric.getRangeData = function(metric_pd, meta, explain, order) {

            var chartData = {chartData: {}, chartDP: {dp: [], ticks: []}};

            chartData.chartData = userovoCommon.extractRangeData(_Db, metric_pd, this.getMeta(meta), explain, order);
        ...
        */
        function extendMeta() {
            if (_Db.meta) {
                for (var i in _Db.meta) {
                    _metrics[i] = userovoCommon.union(_metrics[i], _Db.meta[i]);
                }
            }
        }
    ...
    /**
    * Initialize userovo text select. In most cases it is done automatically, only in some cases, when content loaded via ajax request outside of view lifecycle, you may need to initialize it yourself for your content specifically
    * @param {object} element - jQuery object reference
    * @example
    * UserovoHelpers.initializeTextSelect($("#my-dynamic-div"));
    */
    UserovoHelpers.initializeTextSelect = function(element) {
     ...
     */
    UserovoHelpers.shuffleString = function(text) {
        var j, x, i;
     ...
     */
    UserovoHelpers.getRandomValue = function(charSet, length = 1) {
    ...
    * @example
    * //outputs 4UBHvRBG1v
    * UserovoHelpers.generatePassword(10, true);
    */
    UserovoHelpers.generatePassword = function(length, no_special) {
        var text = [];
    ...
    * @example
    * //outputs true
    * UserovoHelpers.validateEmail("test@test.test");
    *
    * //outputs false
    * UserovoHelpers.validateEmail("test@test");
    */
    UserovoHelpers.validateEmail = function(email) {
    ...
    * @returns {boolean} true if valid and false if invalid
    */
    UserovoHelpers.validatePassword = function(password) {
        if (password.length < userovoGlobal.security.password_min) {
            return jQuery.i18n.prop("management-users.password.length", userovoGlobal.security.password_min);
        }
        if (userovoGlobal.security.password_char && !/[A-Z]/.test(password)) {
            return jQuery.i18n.map["management-users.password.has-char"];
        }
        if (userovoGlobal.security.password_number && !/\d/.test(password)) {
            return jQuery.i18n.map["management-users.password.has-number"];
        }
        if (userovoGlobal.security.password_symbol && !/[^A-Za-z\d]/.test(password)) {
    ...
    */
    UserovoHelpers.upload = function(el, url, data, callback) {
        $(el).simpleUpload(url, {
            data: data,
     ...
     */
    UserovoHelpers.initBreadcrumbs = function(breadcrumbs, el) {
    ...
    */
    UserovoHelpers.getPeriodUrlQueryParameter = function(period) {
    ...
    */
    UserovoHelpers.formatPercentage = function(value, decimalPlaces) {
     ...
     */
    UserovoHelpers.arrayDiff = function(a1, a2) {
        var a = [], diff = [];
     ...
     */
    UserovoHelpers.removeItemFromArray = function(item, array) {
     ...
     */
    UserovoHelpers.arrayUnique = function(array) {
        var a = array.concat();
     ...
     */
    UserovoHelpers.removeEmptyValues = function(array) {
     ...
     */
    UserovoHelpers.objectWithoutProperties = function(obj, excluded) {
    ...
    */
    UserovoHelpers.sha1 = function(str) {
...
}(window.UserovoHelpers = window.UserovoHelpers || {}));
...
$.extend(Template.prototype, {
    ...
    urlFor: function(name) {
        ...
        if (userovoGlobal.path && userovoGlobal.path.length && name.indexOf(userovoGlobal.path) !== 0) {
            name = userovoGlobal.path + name;
        }
        return name + "?" + userovoGlobal.userovoVersion;
    }
});
