/* global userovoCommon, userovoLocation, UserovoHelpers, userovoSession, google, _, userovoGlobal, Backbone, jQuery, $, T*/
(function() {
    ...
    T.get('/localization/countries/' + userovoCommon.BROWSER_LANG_SHORT + '/country.json', function(data) {
        _countryMap = data;
    });
    ...
    window.userovoLocation = window.userovoLocation || {};
    userovoLocation.getCountryName = function(cc) {
    ...
    userovoLocation.getCodesFromName = function(name) {
        var codes = [];
    ...
    UserovoHelpers.createMetricModel(window.userovoLocation, {name: "countries", estOverrideMetric: "countries"}, jQuery, userovoLocation.getCountryName);

    userovoLocation.getRegionName = function(rgn, country) {
        return _regionMap[rgn] || _regionMap[country + "-" + rgn] || rgn;
    };

    // Public Methods
    userovoLocation.initialize = function() {
        userovoLocation.setDb(userovoSession.getDb());
    };

    userovoLocation.refresh = function(newJSON) {
        if (newJSON) {
            userovoLocation.extendDb(newJSON);
        }
    };

    userovoLocation.drawGeoChart = function(options) {
        if (options) {
    ...
    userovoLocation.refreshGeoChart = function(metric) {
        if (google.visualization) {
            if (_chart === undefined) {
                draw(metric);
            }
    ...
    userovoLocation.orderByType = function(type, locationData) {
    ...
    userovoLocation.getLocationData = function(options) {
        ...
        var locationData = userovoCommon.extractTwoLevelData(userovoLocation.getDb(), userovoLocation.getMeta(), userovoLocation.clearObject, [
            {
                "name": "country",
                "func": function(rangeArr) {
                    return userovoLocation.getCountryName(rangeArr);
                }
            },
            {
                "name": "code",
        ...
        locationData.chartData = userovoCommon.mergeMetricsByName(locationData.chartData, "country");
        ...
        for (var i = 0; i < locationData.chartData.length; i++) {
            locationData.chartData[i].country_flag =
                "<div class='flag " + locationData.chartData[i].code + "' style='margin-top:2px; background-image:url(" + userovoGlobal.path + "/images/flags/" + locationData.chartData[i].code + ".png);'></div>" +
    ...
    userovoLocation.changeLanguage = function() {
        // Load local country names
        return T.get('/localization/countries/' + userovoCommon.BROWSER_LANG_SHORT + '/country.json', function(data) {
            _countryMap = data;
        });
    };
    ...
    */
    function draw(ob) {
        ...
        var tt = userovoCommon.extractTwoLevelData(userovoLocation.getDb(), userovoLocation.getMeta(), userovoLocation.clearObject, [
            {
                "name": "country",
                "func": function(rangeArr) {
                    return rangeArr;
        ...
        chartData.rows = _.map(tt.chartData, function(value) {
            ...
            return {
                c: [
                    ...
                    {v: value.country, f: userovoLocation.getCountryName(value.country)},
                    {v: value[ob.metric] === 0 ? null : value[ob.metric]}
                ]
            };
        }).filter(function(row) {
        ...
        if (userovoCommon.CITY_DATA !== false && Backbone.history.fragment === "/analytics/countries") {
            google.visualization.events.addListener(_chart, 'regionClick', function(eventData) {
                var activeAppCountry = userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].country;
