/* global Backbone, userovoAuth, Handlebars, userovoEvent, userovoCommon, userovoGlobal, userovoView, UserovoHelpers, userovoSession, moment, Drop, _, store, userovoLocation, jQuery, $, T, userovoVue*/
/**
 * View class to expand by plugins which need configuration under Management->Applications.
 * @name userovoManagementView
 * @global
 * @namespace userovoManagementView
 */
window.userovoManagementView = userovoView.extend({
    /**
     * Handy function which returns currently saved configuration of this plugin or empty object.
     * @memberof userovoManagementView
     * @return {Object} app object
     */
    config: function() {
        return userovoGlobal.apps[this.appId] &&
            userovoGlobal.apps[this.appId].plugins &&
            userovoGlobal.apps[this.appId].plugins[this.plugin] || {};
    },
    ...
    showError: function(error) {
        UserovoHelpers.alert(error, 'popStyleGreen', {title: jQuery.i18n.map['management-applications.plugins.smth'], image: 'empty-icon', button_title: jQuery.i18n.map['management-applications.plugins.ok']});
    },
    ...
    doOnChange: function(name, value) {
        ...
        if (name && userovoCommon.dot(this.templateData, name) !== value) {
            userovoCommon.dot(this.templateData, name, value);
        }
    ...
    save: function(ev) {
        ...
        this.prepare().then(function(data) {
            var dialog, timeout = setTimeout(function() {
                dialog = UserovoHelpers.loading(jQuery.i18n.map['management-applications.plugins.saving']);
            }, 300);

            $.ajax({
                type: "POST",
                url: userovoCommon.API_PARTS.apps.w + '/update/plugins',
                data: {
                    app_id: self.appId,
                ...
                success: function(result) {
                    ...
                    if (result.result === 'Nothing changed') {
                        UserovoHelpers.notify({type: 'warning', message: jQuery.i18n.map['management-applications.plugins.saved.nothing']});
                    }
                    else {
                        UserovoHelpers.notify({title: jQuery.i18n.map['management-applications.plugins.saved.title'], message: jQuery.i18n.map['management-applications.plugins.saved']});
                        if (!userovoGlobal.apps[result._id].plugins) {
                            userovoGlobal.apps[result._id].plugins = {};
                        }
                        self.savedTemplateData = JSON.stringify(self.templateData);
                        for (var k in result.plugins) {
                            userovoGlobal.apps[result._id].plugins[k] = result.plugins[k];
                        }
...
/**
* Drop class with embeded userovo theme, use as any Drop class/instance
* @name UserovoDrop
* @global
* @namespace UserovoDrop
*/
var UserovoDrop = Drop.createContext({
    classPrefix: 'userovo-drop',
});
...
function setMenuItems() {
    ...
    var type = userovoCommon.ACTIVE_APP_ID && userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID] && userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].type ? userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].type : "mobile";
...
var AppRouter = Backbone.Router.extend({
    ...
    */
    ...
    /**
    * Navigate to another view programmatically. If you need to change the view without user clicking anything, like redirect. You can do this using this method. This method is not define by userovo but is direct method of AppRouter object in Backbone js
    * @name app#navigate
    * @function
    * @instance
    ...
    */
    ...
    switchApp: function(app_id, callback) {
        userovoCommon.setActiveApp(app_id);
        $("#active-app-name").text(userovoGlobal.apps[app_id].name);
        $("#active-app-name").attr('title', userovoGlobal.apps[app_id].name);
        $("#active-app-icon").css("background-image", "url('" + userovoGlobal.path + "appimages/" + app_id + ".png')");
    ...
    **/
    addMenuCategory: function(category, node) {
        ...
        userovoVue.container.registerData("/sidebar/analytics/menuCategory", {
            name: category,
            priority: node.priority,
            title: node.text || userovoVue.i18n("sidebar.category." + category),
            node: node
            /*
    ...
    **/
    addMenuForType: function(app_type, category, node) {
        ...
        //New sidebar container hook
        userovoVue.container.registerData("/sidebar/analytics/menu", {
            app_type: app_type,
            category: category,
            name: node.code,
            priority: node.priority,
    ...
    **/
    addSubMenuForType: function(app_type, parent_code, node) {
        ...
        //New sidebar container hook
        userovoVue.container.registerData("/sidebar/analytics/submenu", {
            app_type: app_type,
    ...
    **/
    addMenu: function(category, node) {
        ...
        if (node && (node.pluginName || node.permission) && !UserovoHelpers.isPluginEnabled(node.pluginName || node.permission)) {
            return;
        }
        else {
    ...
    **/
    ...
    main: function(/*forced*/) {
        ...
        if (Backbone.history.fragment.indexOf("/app/") === 0) {
            ...
            if (app_id && app_id.length) {
                ...
                if (app_id !== userovoCommon.ACTIVE_APP_ID && userovoGlobal.apps[app_id]) {
        ...
        else if (Backbone.history.fragment.indexOf("/0/") === 0 && userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID]) {
            this.navigate("#/" + userovoCommon.ACTIVE_APP_ID + Backbone.history.fragment.replace("/0", ""), true);
            return;
        }
        else if (Backbone.history.fragment !== "/" && userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID]) {
            var type = userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].type || "mobile";
            var urls = userovoVue.container.getAllRoutes();
        ...
        if (redirect) {
            ...
            else if (userovoCommon.APP_NAMESPACE !== false) {
                this.navigate("#/" + userovoCommon.ACTIVE_APP_ID + Backbone.history.fragment, true);
            }
            else {
                this.dashboard();
            }
            ...
            if (userovoCommon.APP_NAMESPACE !== false) {
                this.navigate("#/" + userovoCommon.ACTIVE_APP_ID + Backbone.history.fragment, true);
            }
            else {
    ...
    dashboard: function() {
        if (userovoGlobal.member.restrict && userovoGlobal.member.restrict.indexOf("#/") !== -1) {
            return;
        }
        if (_.isEmpty(userovoGlobal.apps)) {
            this.renderWhenReady(this.manageAppsView);
        }
        else if (typeof this.appTypes[userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].type] !== "undefined") {
            if (this.appTypes[userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].type] !== null) {
                this.renderWhenReady(this.appTypes[userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].type]);
            }
            else {
    ...
    performRefresh: function(self) {
        ...
        if (userovoCommon.periodObj.periodContainsToday && self.activeView.isLoaded && !userovoCommon.DISABLE_AUTO_REFRESH) {
    ...
    renderWhenReady: function(viewName) { //all view renders end up here
        ...
        if (typeof userovoGlobal.member.password_changed === "undefined") {
            userovoGlobal.member.password_changed = Math.round(new Date().getTime() / 1000);
        }
        this.routesHit++;
        ...
        else if ((userovoGlobal.security.password_expiration > 0) &&
                (userovoGlobal.member.password_changed + userovoGlobal.security.password_expiration * 24 * 60 * 60 < new Date().getTime() / 1000) &&
                (!userovoGlobal.ssr)) {
        ...
        if (userovoGlobal && userovoGlobal.message) {
            UserovoHelpers.parseAndShowMsg(userovoGlobal.message);
        }

        /**
         * Identify the actuve sidebar menu item.
         *
         * userovoVue.sideBarComponent is null on the initial load.
         * But don't worry, we identify selected menu items when its mounted aswell.
         */
        if (userovoVue.sideBarComponent) {
            userovoVue.sideBarComponent.$children[0].identifySelected();
        }
    },
    sidebar: {
        init: function() {
    ...
    initialize: function() { //initialize the dashboard, register helpers etc.
        ...
        // if (userovoAuth.validateUpdate('events') || userovoAuth.validateDelete('events')) {
        ...
        if (userovoAuth.validateCreate('core')) {
        ...
        if (userovoAuth.validateAnyAppAdmin()) {
        ...
        */
        ...
        /**
        * Shorten number, Handlebar binding to {@link userovoCommon.getShortNumber}
        * @name getShortNumber
        * @memberof Handlebars
        * @example
        ...
        */
        Handlebars.registerHelper('getShortNumber', function(context) {
            return userovoCommon.getShortNumber(context);
        });
        /**
        ...
        */
        ...
        /**
        * Convert array of app ids to comma separate string of app names. Handlebar binding to {@link UserovoHelpers.appIdsToNames}
        * @name appIdsToNames
        * @memberof Handlebars
        * @example
        ...
        */
        Handlebars.registerHelper('appIdsToNames', function(context) {
            return UserovoHelpers.appIdsToNames(context);
        });
        /**
		...
		*/
        Handlebars.registerHelper('round', function(number, limit) {
            return userovoCommon.round(number, limit);
        });
        ...
        */
        ...
        /**
        * Format timestamp to twitter like time ago format, Handlebar binding to {@link userovoCommon.formatTimeAgo}
        * @name formatTimeAgo
        * @memberof Handlebars
        * @example
        ...
        */
        Handlebars.registerHelper('formatTimeAgo', function(context) {
            return userovoCommon.formatTimeAgo(parseInt(context) / 1000);
        });
        /**
        ...
        */
        ...
        $("body").addClass("lang-" + userovoCommon.BROWSER_LANG_SHORT);
        jQuery.i18n.properties({
            name: window.production ? 'localization/min/locale' : ["localization/dashboard/dashboard", "localization/help/help", "localization/mail/mail"].concat(userovoGlobal.plugins.map(function(plugin) {
                return plugin + "/localization/" + plugin;
            })),
            cache: true,
            language: userovoCommon.BROWSER_LANG_SHORT,
            userovoVersion: userovoGlobal.userovoVersion + "&" + userovoGlobal.pluginsSHA,
            path: userovoGlobal.cdn,
            mode: 'map',
            callback: function() {
                for (var key in jQuery.i18n.map) {
                    if (userovoGlobal.company) {
                        jQuery.i18n.map[key] = jQuery.i18n.map[key].replace(new RegExp("Userovo", 'ig'), userovoGlobal.company);
                    }
                    jQuery.i18n.map[key] = userovoCommon.encodeSomeHtml(jQuery.i18n.map[key]);
                }
        ...
        $(document).ready(function() {
            ...
            UserovoHelpers.initializeSelect();
            UserovoHelpers.initializeTextSelect();
            UserovoHelpers.initializeMultiSelect();

            if (userovoGlobal.licenseNotification && userovoGlobal.licenseNotification.length && !_.isEmpty(userovoGlobal.apps)) {
                for (var idx = 0; idx < userovoGlobal.licenseNotification.length; idx++) {
                    userovoGlobal.licenseNotification[idx].id = userovoCommon.generateId();
                    UserovoHelpers.notify(userovoGlobal.licenseNotification[idx]);
                }
            }
            ...
            $.ajaxPrefilter(function(options) {
                ...
                if (last5char === ".html") {
                    var version = userovoGlobal.userovoVersion || "";
                    options.url = options.url + "?v=" + version;
                }
            });
            var validateSession = function() {
                $.ajax({
                    url: userovoGlobal.path + "/session",
                    data: {check_session: true},
                    success: function(result) {
                        if (result === "logout") {
                            UserovoHelpers.logout("/logout");

                        }
                        if (result === "login") {
                            UserovoHelpers.logout();
                        }
                        setTimeout(function() {
                            validateSession();
            ...
            setTimeout(function() {
                validateSession();
            }, userovoCommon.DASHBOARD_VALIDATE_SESSION || 30000);//validates session each 30 seconds
            if (parseInt(userovoGlobal.config.session_timeout)) {
                    ...
                    $.ajax({
                        ...
                        success: function(result) {
                            ...
                            if (result === "login") {
                                ...
                                shouldRecordAction = false;
                                var myTimeoutValue = parseInt(userovoGlobal.config.session_timeout) * 1000 * 60;
                ...
                var resetSessionTimeouts = function(timeout) {
                    ...
                    if (minute > 0) {
                        minTimeout = setTimeout(function() {
                            UserovoHelpers.notify({ title: jQuery.i18n.map["common.session-expiration"], message: jQuery.i18n.map["common.expire-minute"], info: jQuery.i18n.map["common.click-to-login"] });
                        }, minute);
                    }
                    ...
                    if (tenSeconds > 0) {
                        tenSecondTimeout = setTimeout(function() {
                            UserovoHelpers.notify({ title: jQuery.i18n.map["common.session-expiration"], message: jQuery.i18n.map["common.expire-seconds"], info: jQuery.i18n.map["common.click-to-login"] });
                        }, tenSeconds);
                    }
                    if (logoutTimeout) {
            ...
            var periodObj = userovoCommon.getPeriod();
            if (Object.prototype.toString.call(periodObj) === '[object Array]' && periodObj.length === 2) {
                self.dateFromSelected = parseInt(periodObj[0], 10) + userovoCommon.getOffsetCorrectionForTimestamp(parseInt(periodObj[0], 10));
                self.dateToSelected = parseInt(periodObj[1], 10) + userovoCommon.getOffsetCorrectionForTimestamp(parseInt(periodObj[1], 10));
            }
            ...
            $(".reveal-language-menu").text(userovoCommon.BROWSER_LANG_SHORT.toUpperCase());

            $("#sidebar-events").click(function(e) {
                $.when(userovoEvent.refreshEvents()).then(function() {
                    if (userovoEvent.getEvents().length === 0) {
                        UserovoHelpers.alert(jQuery.i18n.map["events.no-event"], "black");
            ...
            $("#language-menu .item").click(function() {
                ...
                $("#date-to").datepicker("option", $.datepicker.regional[userovoCommon.BROWSER_LANG]);
                $("#date-from").datepicker("option", $.datepicker.regional[userovoCommon.BROWSER_LANG]);
                ...
                jQuery.i18n.properties({
                    ...
                    callback: function() {
                        ...
                        self.origLang = JSON.stringify(jQuery.i18n.map);
                        $.when(userovoLocation.changeLanguage()).then(function() {
            ...
            $(document).on('click', "#save-account-details:not(.disabled)", function() {
                ...
                $.ajax({
                    type: "POST",
                    url: userovoGlobal.path + "/user/settings",
                    data: {
                        "username": username,
                        "old_pwd": old_pwd,
                        "new_pwd": new_pwd,
                    ...
                    success: function(result) {
                        ...
                        if (result === "username-exists") {
                            ...
                            userovoGlobal.member.username = username;
                            userovoGlobal.member.api_key = api_key;
                        }
            ...
            var help = _.once(function() {
                UserovoHelpers.alert(jQuery.i18n.map["help.help-mode-welcome"], "popStyleGreen popStyleGreenWide", {button_title: jQuery.i18n.map["common.okay"] + "!", title: jQuery.i18n.map["help.help-mode-welcome-title"], image: "welcome-to-help-mode"});
            });
            ...
            $("#help-menu").click(function(e) {
                ...
                if ($("#help-toggle").hasClass("active")) {
                    ...
                    $.idleTimer(userovoCommon.DASHBOARD_IDLE_MS);
                }
            });
            ...
            $("#user-logout").click(function(e) {
                e.preventDefault();
                store.remove('userovo_active_app');
            ...
            $("#content").on("click", "#graph-note", function() {
                ...
                $(".note-date:visible").datepicker("option", $.datepicker.regional[userovoCommon.BROWSER_LANG]);
                ...
                if (userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID] && userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].notes) {
                    var noteDateIds = _.sortBy(_.keys(userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].notes), function(el) {
                        return -parseInt(el);
                    });

                    for (var i = 0; i < noteDateIds.length; i++) {
                        var currNotes = userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].notes[noteDateIds[i]];
                ...
                $(".note-popup:visible .note .delete-note").on("click", function() {
                    ...
                    $.ajax({
                        type: "POST",
                        url: userovoGlobal.path + '/graphnotes/delete',
                        data: {
                ...
                $(".note-popup:visible .create-note").on("click", function() {
                    ...
                    $.ajax({
                        type: "POST",
                        url: userovoGlobal.path + '/graphnotes/create',
                        data: {
                            "app_id": userovoCommon.ACTIVE_APP_ID,
                            "date_id": dateId,
                            "note": note,
                ...
                */
                function updateGlobalNotes(noteObj, operation) {
                    var globalNotes = userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].notes;

                    if (operation === "create") {
                        if (globalNotes) {
                            if (globalNotes[noteObj.date_id]) {
                                userovoCommon.arrayAddUniq(globalNotes[noteObj.date_id], noteObj.note);
                            }
                            else {
                            ...
                            userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].notes = tmpNote;
                        }
                    }
            ...
            $appNavigation.on("click", function() {
                var appList = $(this).find(".list"),
                    apps = _.sortBy(userovoGlobal.apps, function(app) {
                ...
                for (var i = 0; i < apps.length; i++) {
                    ...
                    appIcon.css("background-image", "url(" + userovoGlobal.cdn + "appimages/" + currApp._id + ".png");
        ...
        if (!_.isEmpty(userovoGlobal.apps)) {
            if (!userovoCommon.ACTIVE_APP_ID) {
                var activeApp = (userovoGlobal.member && userovoGlobal.member.active_app_id && userovoGlobal.apps[userovoGlobal.member.active_app_id])
                    ? userovoGlobal.apps[userovoGlobal.member.active_app_id]
                    : userovoGlobal.defaultApp;

                userovoCommon.setActiveApp(activeApp._id);
                self.activeAppName = activeApp.name;
                $('#active-app-name').html(activeApp.name);
                $('#active-app-name').attr('title', activeApp.name);
                $("#active-app-icon").css("background-image", "url('" + userovoGlobal.cdn + "appimages/" + userovoCommon.ACTIVE_APP_ID + ".png')");
            }
            else {
                $("#active-app-icon").css("background-image", "url('" + userovoGlobal.cdn + "appimages/" + userovoCommon.ACTIVE_APP_ID + ".png')");
                $("#active-app-name").text(userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].name);
                $('#active-app-name').attr('title', userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].name);
                self.activeAppName = userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].name;
            }
        }
        else {
        ...
        */
        function getCustomDateInt(s) {
            ...
            s = moment(s, userovoCommon.getDateFormat(userovoCommon.periodObj.dateString)).format(userovoCommon.periodObj.dateString);
            var dateParts = "";
            if (s.indexOf(":") !== -1) {
                if (s.indexOf(",") !== -1) {
                    s = s.replace(/,|:/g, "");
                    ...
                    return parseInt((userovoCommon.getMonths().indexOf(dateParts[1]) + 1) * 1000000) +
                ...
                return userovoCommon.getMonths().indexOf(s) + 1;
            }
                ...
                if (dateParts.length === 3) {
                    return (parseInt(dateParts[2]) * 10000) + parseInt((userovoCommon.getMonths().indexOf(dateParts[1]) + 1) * 100) + parseInt(dateParts[0]);
                }
                else {
                    if (dateParts[0].length === 3) {
                        return parseInt((userovoCommon.getMonths().indexOf(dateParts[0]) + 1) * 100) + parseInt(dateParts[1] * 10000);
                    }
                    else {
                        return parseInt((userovoCommon.getMonths().indexOf(dateParts[1]) + 1) * 100) + parseInt(dateParts[0]);
                    }
                }
            }
        }
        ...
        */
        ...
        jQuery.fn.dataTableExt.oSort['loyalty-asc'] = function(x, y) {
            x = userovoSession.getLoyaltyIndex(x);
            y = userovoSession.getLoyaltyIndex(y);
        ...
        jQuery.fn.dataTableExt.oSort['frequency-asc'] = function(x, y) {
            x = userovoSession.getFrequencyIndex(x);
            y = userovoSession.getFrequencyIndex(y);
        ...
        jQuery.fn.dataTableExt.oSort['session-duration-asc'] = function(x, y) {
            x = userovoSession.getDurationIndex(x);
            y = userovoSession.getDurationIndex(y);
        ...
        jQuery.fn.dataTableExt.oSort['app_versions-asc'] = function(x, y) {
            return userovoCommon.compareVersions(x, y);
        };
        ...
        */
        function saveCurrentPage(dtable, settings) {
            ...
            userovoCommon.dtSettings = userovoCommon.dtSettings || [];

            var previosTableStatus = userovoCommon.dtSettings.filter(function(item) {
        ...
        */
        function setCurrentPage(dtable, settings) {
            var tablePersistSettings = userovoCommon.dtSettings.filter(function(item) {
                return (item.viewId === app.activeView.cid && item.selector === settings.sTableId);
            })[0];
        ...
        */
        function getPageSize(settings) {
            var pageSizeSettings = userovoCommon.getPersistentSettings().pageSizeSettings;
            if (!pageSizeSettings) {
                pageSizeSettings = [];
            }
            ...
            "fnInitComplete": function(oSettings) {
                ...
                tableWrapper.find('#dataTables_length_input').bind('change.DT', function(/*e, _oSettings*/) {
                    ...
                    if ($(this).val() && $(this).val().length > 0) {
                        ...
                        userovoCommon.setPersistentSettings({ pageSizeSettings: pageSizeSettings });
                    }
                });
                var exportDrop;
                if (oSettings.oFeatures.bServerSide && !oSettings.oFeatures.localExport) {
                    ...
                    if (exportAPIData || exportQueryData) {
                        ...
                        exportDrop.on("open", function() {
                            if (exportAPIData) {
                                $(".server-export .userovo-drop-content").empty().append(UserovoHelpers.export(oSettings._iRecordsDisplay, app[exportView].getExportAPI(oSettings.sTableId), null, true, oSettings.oInstance).removeClass("dialog"));
                            }
                            else if (exportQueryData) {
                                $(".server-export .userovo-drop-content").empty().append(UserovoHelpers.export(oSettings._iRecordsDisplay, app[exportView].getExportQuery(oSettings.sTableId), null, null, oSettings.oInstance).removeClass("dialog"));
                            }
                            exportDrop.position();
                        });
                    }
                        ...
                        if (item) {
                            ...
                            exportDrop.on("open", function() {
                                $(".server-export .userovo-drop-content").empty().append(UserovoHelpers.tableExport(dtable, { api_key: userovoGlobal.member.api_key }, null, oSettings).removeClass("dialog"));
                                exportDrop.position();
                            });
                        }
                    }
        ...
        $(document).ready(function() {
            setTimeout(function() {
                self.onAppSwitch(userovoCommon.ACTIVE_APP_ID, true, true);
            }, 1);
        });
    },
    /**
    ...
    * @param {userovoView} view - instance of the userovoView to show as main dashboard for provided app type
    * @memberof app
    * @instance
    * @example
    ...
    */
    ...
    /**
     * Add a userovoManagementView-extending view which will be displayed in accordion tabs on Management->Applications screen
     * @memberof app
    ...
    addAppManagementView: function(plugin, title, View) {
        if (UserovoHelpers.isPluginEnabled(plugin)) {
    ...
    *     toDisplay: function(appId, elem){$(elem).text(process(userovoGlobal['apps'][appId]["my_setting"]));},
    *     toInput: function(appId, elem){$(elem).val(process(userovoGlobal['apps'][appId]["my_setting"]));},
    ...
    */
    addPageScript: function(view, callback, name) {
        if (!name || UserovoHelpers.isPluginEnabled(name)) {
    ...
    */
    ...
    onAppSwitch: function(appId, refresh, firstLoad) {
        if (appId !== 0) {
            ...
            var type = userovoGlobal.apps[appId].type;
            ...
            for (var i = 0; i < this.appSwitchCallbacks.length; i++) {
                if (UserovoHelpers.isPluginEnabled(this.appSwitchCallbacks[i].name)) {
    ...
    onAppManagementSwitch: function(appId, type) {
        for (var i = 0; i < this.appManagementSwitchCallbacks.length; i++) {
            this.appManagementSwitchCallbacks[i](appId, type || userovoGlobal.apps[appId].type);
        }
    ...
    pageScript: function() { //scripts to be executed on each view change
        ...
        $(document).ready(function() {
            ...
            var selectedDateID = userovoCommon.getPeriod();
            ...
            UserovoHelpers.setUpDateSelectors(self.activeView);
            ...
            $("#date-submit").click(function() {
                ...
                userovoCommon.setPeriod([
                    self.dateFromSelected - userovoCommon.getOffsetCorrectionForTimestamp(self.dateFromSelected),
                    self.dateToSelected - userovoCommon.getOffsetCorrectionForTimestamp(self.dateToSelected) + 24 * 60 * 60 * 1000 - 1
                ]);
            ...
            $(".resource-link").on('click', function() {
                if ($(this).data("link")) {
                    UserovoHelpers.openResource($(this).data("link"));
                }
            });
...
for (var i in userovoGlobal.apps) {
    Backbone.history.appIds.push(i);
}
Backbone.history.getFragment = function() {
    var fragment = Backbone.history._getFragment();
    if (fragment.indexOf("/" + userovoCommon.ACTIVE_APP_ID) === 0) {
        fragment = fragment.replace("/" + userovoCommon.ACTIVE_APP_ID, "");
    }
    return fragment;
};
Backbone.history.checkUrl = function() {
    store.set("userovo_fragment_name", Backbone.history._getFragment());
    var app_id = Backbone.history._getFragment().split("/")[1] || "";
    if (userovoCommon.APP_NAMESPACE !== false && userovoCommon.ACTIVE_APP_ID !== 0 && userovoCommon.ACTIVE_APP_ID !== app_id && Backbone.history.appIds.indexOf(app_id) === -1) {
        Backbone.history.noHistory("#/" + userovoCommon.ACTIVE_APP_ID + Backbone.history._getFragment());
        app_id = userovoCommon.ACTIVE_APP_ID;
    }

    if (userovoCommon.ACTIVE_APP_ID !== 0 && userovoCommon.ACTIVE_APP_ID !== app_id && Backbone.history.appIds.indexOf(app_id) !== -1) {
        app.switchApp(app_id, function() {
            if (Backbone.history.checkOthers()) {
...
var checkGlobalAdminOnlyPermission = function() {
    ...
    if (!userovoGlobal.member.global_admin && !userovoGlobal.config.autonomous) {
        var existed = false;
        var checkList = userCheckList;
        if (userovoAuth.getAdminApps(userovoGlobal.member) && userovoAuth.getAdminApps(userovoGlobal.member).length) {
...
//initial hash check
(function() {
    if (!Backbone.history.getFragment() && store.get("userovo_fragment_name")) {
        Backbone.history.noHistory("#" + store.get("userovo_fragment_name"));
    }
    else {
        var app_id = Backbone.history._getFragment().split("/")[1] || "";
        if (userovoCommon.ACTIVE_APP_ID === app_id || Backbone.history.appIds.indexOf(app_id) !== -1) {
            //we have app id
            if (app_id !== userovoCommon.ACTIVE_APP_ID) {
...
app.noHistory = function(hash) {
    if (userovoCommon.APP_NAMESPACE !== false) {
        hash = "#/" + userovoCommon.ACTIVE_APP_ID + hash.substr(1);
    }
    if (history && history.replaceState) {
