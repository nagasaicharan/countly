/* global userovoCommon, userovoGlobal, _, jQuery */
(function(userovoTotalUsers, $) {
    ...
    //Public Methods
    userovoTotalUsers.initialize = function(forMetric) {
        _period = userovoCommon.getPeriodForAjax();
        _activeAppId = userovoCommon.ACTIVE_APP_ID;

        if (!userovoTotalUsers.isUsable()) {
            return true;
        }

        if (isInitialized(forMetric)) {
            return userovoTotalUsers.refresh(forMetric);
        }
    ...
    userovoTotalUsers.refresh = function(forMetric) {
        return $.ajax({
            type: "GET",
            url: userovoCommon.API_PARTS.data.r,
            data: {
    ...
    userovoTotalUsers.get = function(forMetric, prev) {
     ...
     */
    userovoTotalUsers.isUsable = function() {
        if (userovoGlobal.plugins.indexOf("drill") !== -1) {
            return true;
        }
        return userovoCommon.periodObj.periodContainsToday;
    };
...
}(window.userovoTotalUsers = window.userovoTotalUsers || {}, jQuery));