/* global userovoCommon, userovoEvent, $, Backbone, app, UserovoHelpers, _, Handlebars */

var initializeOnce = _.once(function() {
    return $.when(userovoEvent.initialize()).then(function() { });
});

/**
* Default Backbone View template from which all userovo views should inherit.
* A userovo view is defined as a page corresponding to a url fragment such
...
* @name userovoView
* @global
* @namespace userovoView
* @example <caption>Extending default view and overwriting its methods</caption>
*  window.DashboardView = userovoView.extend({
*       renderCommon:function () {
*           if(userovoGlobal["apps"][userovoCommon.ACTIVE_APP_ID]){
*               var type = userovoGlobal["apps"][userovoCommon.ACTIVE_APP_ID].type;
...
*   });
*/
window.userovoView = Backbone.View.extend({
    /**
    * Checking state of view, if it is loaded
    * @type {boolean}
    * @instance
    * @memberof userovoView
    */
    isLoaded: false,
    /**
    * Handlebar template
    ...
    */
    appChanged: function(callback) { //called when user changes selected app from the sidebar
        userovoEvent.reset();
        $.when(userovoEvent.initialize()).always(function() {
            if (callback) {
                callback();
            }
        });
    ...
    *       self.template = src;
    *   }), userovoDeviceDetails.initialize(), userovoTotalUsers.initialize("densities"), userovoDensity.initialize()).then(function () {});
    *}
    */
    beforeRender: function() {
        return true;
    ...
    */
    ...
    /**
    * Main render method, better not to over write it, but use {@link userovoView.renderCommon} instead
    * @returns {object} this
    * @memberof userovoView
    * @instance
    */
    render: function() { //backbone.js view render function
        ...
        if (userovoGlobal.member.member_image) {
            $('.member_image').html("");
            $('.member_image').css({'background-image': 'url(' + userovoGlobal.member.member_image + '?now=' + Date.now() + ')', 'background-size': '100%'});
        }
        else {
            var defaultAvatarSelector = userovoGlobal.member.created_at % 16 * 30;
            var name = userovoGlobal.member.full_name.split(" ");
    ...
    */
    ...
    /**
    * Called when view is refreshed, you can reload data here or call {@link userovoView.renderCommon} with parameter true for code reusability
    * @returns {boolean} true
    * @memberof userovoView
    * @instance
    * @example
