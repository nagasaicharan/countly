/* global jQuery, Vue, _, CV, userovoCommon, userovoGlobal, UserovoHelpers, userovoTaskManager, _merge, Sortable */
...
(function(userovoVue, $) {
            ...
            paginationInfo() {
                ...
                if (filteredTotal > 0) {
                    info = this.i18n('common.showing')
                        .replace('_START_', userovoCommon.formatNumber(startEntries))
                        .replace('_END_', userovoCommon.formatNumber(endEntries))
                        .replace('_TOTAL_', userovoCommon.formatNumber(filteredTotal));
                }
            ...
            getControlParams() {
                ...
                try {
                    if (
                        ...
                        userovoGlobal.member.columnOrder[this.persistKey].tableSortMap
                    ) {
                        defaultState.selectedDynamicCols = userovoGlobal.member.columnOrder[this.persistKey].tableSortMap;
                    }
            ...
            setControlParams() {
                if (this.persistKey) {
                    ...
                    $.ajax({
                        ...
                        success: () => {
                            // NOTE: since userovoGlobal.member does not updates automatically till refresh
                            if (!userovoGlobal.member.columnOrder) {
                                userovoGlobal.member.columnOrder = {};
                            }

                            if (!userovoGlobal.member.columnOrder[this.persistKey]) {
                                userovoGlobal.member.columnOrder[this.persistKey] = {};
                            }

                            userovoGlobal.member.columnOrder[this.persistKey].tableSortMap = this.controlParams.selectedDynamicCols;
                        },
                        type: 'POST',
    ...
    var ExportHandlerMixin = {
        ...
        methods: {
            ...
            getDefaultFileName: function() {
                var siteName = userovoGlobal.userovoTitle;
                var sectionName = "";
                var selectedMenuItem = this.$store.getters["userovoSidebar/getSelectedMenuItem"];
                if (selectedMenuItem && selectedMenuItem.item && selectedMenuItem.item.title) {
                    sectionName = this.i18n(selectedMenuItem.item.title);
                }
                var appName = userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].name;
                var date = userovoCommon.getDateRangeForCalendar();
                ...
                if (rows && rows.length && this.$refs.elTable && this.$refs.elTable.columns && this.$refs.elTable.columns.length) {
                    ...
                    for (var r = 0; r < rows.length; r++) {
                        ...
                        for (var c = 0; c < columns.length; c++) {
                            ...
                            item[columns[c].label.toUpperCase()] = userovoCommon.unescapeHtml(property);
                        }
                        table.push(item);
                    }
                    return table;
            ...
            initiateExport: function(params) {
                ...
                if (this.exportApi) {
                    ...
                    url = userovoCommon.API_URL + (formData.url || "/o/export/request");
                    ...
                    url = userovoCommon.API_URL + (formData.url || "/o/export/db");
                }
                else if (this.dataSource) { // default export logic for server tables
                    url = userovoCommon.API_URL + "/o/export/request";
                    ...
                    var path = lastRequest.url + "?" + (Object.keys(lastRequest.data).reduce(function(acc, key) {
                        ...
                        return acc;
                    }, ["api_key=" + userovoGlobal.member.api_key]).join("&"));
                    ...
                    url = userovoCommon.API_URL + "/o/export/data";
                    formData = {
                        type: params.type,
    ...
    Vue.component("cly-datatable-n", userovoVue.components.create({
        mixins: [
        ...
        template: CV.T('/javascripts/userovo/vue/templates/datatable.html'),
        mounted: function() {
            var self = this;
            if (this.sortable) {
    ...
    Vue.component("cly-datatable-undo-row", userovoBaseComponent.extend({
        props: {
            delayedAction: {
                type: Object
            },
