/* global jQuery, Vue, ELEMENT, CV, UserovoHelpers */
...
(function(userovoVue) {
    ...
    Vue.component("cly-input-dropdown-trigger", userovoBaseComponent.extend({
        props: {
    ...
    var triggerProxy = userovoBaseComponent.extend({
        template: '<div style="width:100%"><slot v-slot></slot></div>'
    });

    Vue.component("cly-dropdown", userovoBaseComponent.extend({
        components: {
    ...
    Vue.component("cly-fields-select", userovoBaseComponent.extend({
        mixins: [_mixins.i18n],
    ...
    Vue.component("cly-multi-select", userovoBaseComponent.extend({
        mixins: [_mixins.i18n],
    ...
    Vue.component("cly-more-options", userovoBaseComponent.extend({
            ...
            handleMenuItemClick: function(command, instance) {
                if (!this.disabled) {
                    if (command && command.url) {
                        UserovoHelpers.goTo({url: command.url, download: !!command.download, isExternalLink: !!command.isExternalLink});
                    }
                    else {
