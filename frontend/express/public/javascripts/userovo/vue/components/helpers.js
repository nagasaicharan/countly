/* global Vue, CV, $, app, userovoEvent, userovoGlobal, userovoAuth, VueJsonPretty, ElementTiptapPlugin, userovoCommon UserovoHelpers*/
...
(function(userovoVue) {
    ...
    Vue.component("cly-back-link", userovoBaseComponent.extend(
        // @vue/component
        {
            mixins: [
                _mixins.i18n
    ...
    Vue.component("cly-in-page-notification", userovoBaseComponent.extend(
        {
            props: {
    ...
    Vue.component("cly-empty-home", userovoBaseComponent.extend({
    ...
    Vue.component('cly-status-tag', userovoBaseComponent.extend({
        props: {
            color: {
                default: 'green',
                type: String
    ...
    Vue.component("cly-diff-helper", userovoBaseComponent.extend({
        mixins: [
            _mixins.i18n
        ],
        props: {
    ...
    Vue.component("cly-metric-cards", userovoBaseComponent.extend({
    ...
    Vue.component("cly-metric-card", userovoBaseComponent.extend({
        mixins: [
    ...
    Vue.component("cly-metric-breakdown", userovoVue.components.create({
        template: userovoVue.T('/javascripts/userovo/vue/templates/breakdown.html'),
        mixins: [
            _mixins.i18n
        ],
        props: {
    ...
    Vue.component("cly-tooltip-icon", userovoBaseComponent.extend({
        props: {
            icon: {
                type: String,
        ...
        computed: {
            tooltipConf: function() {
                return {
                    content: userovoCommon.unescapeHtml(this.tooltip),
    ...
    Vue.component("cly-remover", userovoBaseComponent.extend({
        props: {
            disabled: {
                type: Boolean,
                default: false
    ...
    Vue.component("cly-popover", userovoBaseComponent.extend({
        props: {
            size: {
                type: String,
                default: 'medium',
    ...
    Vue.component("cly-value", userovoBaseComponent.extend({
        template: '<span></span>',
        props: ['value']
    }));
    ...
    Vue.component("cly-app-select", {
            ...
            apps: function() {
                ...
                if (this.auth && this.auth.feature && this.auth.permission) {
                    ...
                    formattedApps = Object.keys(apps).reduce(function(acc, key) {
                        ...
                        if (userovoAuth.validate(expectedPermission, targetFeature, null, currentApp._id)) {
                            acc.push({
                    ...
                    formattedApps = Object.keys(apps).map(function(key) {
                        return {
                            label: userovoCommon.unescapeHtml(apps[key].name),
                            value: apps[key]._id
                        };
                    });
                }
        ...
        data: function() {
            return {
                dropdownApps: userovoGlobal.apps || {}
            };
        }
    });

    Vue.component("cly-event-select", userovoBaseComponent.extend({
        mixins: [userovoVue.mixins.i18n],
        ...
        methods: {
            prepareAvailableEvents: function() {
                ...
                if (userovoGlobal.plugins.indexOf('surveys') !== -1) {
                ...
                if (userovoGlobal.plugins.indexOf('compliance-hub') !== -1) {
                    preparedEventList.push({
                ...
                if (userovoGlobal.plugins.indexOf('crashes') !== -1) {
                    preparedEventList.push({
                ...
                if (userovoGlobal.plugins.indexOf('push') !== -1) {
                ...
                if (userovoGlobal.plugins.indexOf('journey_engine') !== -1) {
                    preparedEventList.push({
                ...
                return new Promise(function(resolve) {
                    if (this.selectedApp) {
                        self.isLoading = true;
                        userovoEvent.getEventsForApps([this.selectedApp], function(eData) {
                            preparedEventList[1].options = eData.map(function(e) {
                                return {label: userovoCommon.unescapeHtml(e.name), value: e.value};
                            });
                        });
                        ...
                        $.when(userovoEvent.refreshEvents()).then(function() {
                            const events = userovoEvent.getEvents();
                            preparedEventList[1].options = events.map(function(event) {
                                return {label: userovoCommon.unescapeHtml(event.name), value: event.key};
                            });
                            preparedEventList = preparedEventList.filter(function(evt) {
                                return !(self.blacklistedEvents.includes(evt.name));
                            });
    ...
    Vue.component("cly-paginate", userovoBaseComponent.extend({
        template: '<div>\
    ...
    Vue.component("cly-color-tag", userovoBaseComponent.extend({
        data: function() {
            return {
    ...
    Vue.component("cly-json-editor", userovoBaseComponent.extend({
        mixins: [
            _mixins.i18n
        ],
        data: function() {
    ...
    Vue.component("cly-notification", userovoBaseComponent.extend({
        template: `
            <div
                v-if="isModalVisible"
    ...
    Vue.component("cly-empty-view", userovoBaseComponent.extend({
        template: '<div :class="classes">\
                        ...
                        <div class="bu-mt-2 bu-is-flex bu-is-flex-direction-column">\
                            ...
                            <slot name="action" v-if="hasCreateRight && hasAction">\
								<div v-if="visual==\'framed\'" style=\'width: 200px\' class="bu-ml-5 bu-pb-4"><el-button   :data-test-id="testId + \'-empty-view-action-button\'" @click="actionFunc"><i class=\'cly-userovo-icon-outline cly-userovo-icon-outline-plus-circle-16px bu-pr-4 bu-mr-1\'></i> {{actionTitle}}\</el-button></div>\
        ...
        props: {
            title: { default: userovoVue.i18n('common.emtpy-view-title'), type: String },
            subTitle: { default: userovoVue.i18n('common.emtpy-view-subtitle'), type: String },
    ...
    Vue.component("cly-breadcrumbs", userovoBaseComponent.extend({
        mixins: [
            _mixins.i18n
        ],
        props: {
    ...
    Vue.component("cly-list-drawer", userovoBaseComponent.extend({
        props: {
            list: {
                type: Array,
                required: true,
    ...
    Vue.component("cly-auto-refresh-toggle", userovoBaseComponent.extend({
        ...
        watch: {
            autoRefresh: function(newValue) {
                localStorage.setItem("auto_refresh_" + this.feature + "_" + userovoCommon.ACTIVE_APP_ID, newValue);
            },
        },
        mounted: function() {
            var autoRefreshState = localStorage.getItem("auto_refresh_" + this.feature + "_" + userovoCommon.ACTIVE_APP_ID);
            if (autoRefreshState) {
                this.autoRefresh = autoRefreshState === "true";
            }
            else {
                localStorage.setItem("auto_refresh_" + this.feature + "_" + userovoCommon.ACTIVE_APP_ID, this.defaultValue);
