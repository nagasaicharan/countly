/* global Vue, CV, userovoGlobal, $, _, userovoCommon */
...
(function(userovoVue) {
    ...
    Vue.component("cly-colorpicker", userovoVue.components.create({
        template: CV.T('/javascripts/userovo/vue/templates/UI/color-picker.html'),
    ...
    var AbstractListBox = userovoVue.views.BaseView.extend({
        props: {
            options: {type: Array},
                                    ...
                                    <div class="bu-level">\
                                        <div class="bu-level-left">\
                                            ...
                                            <slot name="option-label" v-bind="option">\
                                              <div :data-test-id="testId + \'-item-\' + (option.label ? option.label.replace(/[\\s_]+/g, \'-\').toLowerCase() : \' \')" class="cly-vue-listbox__item-label" v-tooltip="decodeHtml(option.label)">{{decodeHtml(option.label)}}</div>\
                                            </slot>\
                                        </div>\
    ...
    Vue.component("cly-checklistbox", AbstractListBox.extend({
        ...
        data: function() {
            ...
            if (this.persistColumnOrderKey && userovoGlobal.member.columnOrder && userovoGlobal.member.columnOrder[this.persistColumnOrderKey] && userovoGlobal.member.columnOrder[this.persistColumnOrderKey].reorderSortMap) {
                savedSortMap = userovoGlobal.member.columnOrder[this.persistColumnOrderKey].reorderSortMap;
        ...
        methods: {
            ...
            saveColumnOrder() {
                ...
                $.ajax({
                    type: "POST",
                    url: userovoGlobal.path + "/user/settings/column-order",
                    data: {
                    ...
                    success: function() {
                        //since userovoGlobal.member does not updates automatically till refresh
                        if (!userovoGlobal.member.columnOrder) {
                            userovoGlobal.member.columnOrder = {};
                        }
                        if (!userovoGlobal.member.columnOrder[self.persistColumnOrderKey]) {
                            userovoGlobal.member.columnOrder[self.persistColumnOrderKey] = {};
                        }
                        userovoGlobal.member.columnOrder[self.persistColumnOrderKey].reorderSortMap = sortMap;
                    }
                });
            }
        },
                        ...
                        <div :style="wrapperStyle" class="cly-vue-listbox__items-wrapper">\
                            <el-checkbox-group\
                                ...
                                <div\
                                    ...
                                    <el-checkbox :test-id="testId + \'-\' + (option.label ? option.label.replaceAll(\' \', \'-\').replaceAll(\'_\', \'-\').toLowerCase() : \'el-checkbox\')" :label="option.value" v-tooltip="option.label" :key="option.value" :disabled="(disableNonSelected && !innerValue.includes(option.value)) || option.disabled">{{option.label}}</el-checkbox>\
                                </div>\
                                </draggable>\
                            </el-checkbox-group>\
                        </div>\
    ...
    Vue.component("cly-select-x", userovoVue.components.create({
        mixins: [TabbedOptionsMixin, SearchableOptionsMixin, _mixins.i18n],
        template: CV.T('/javascripts/userovo/vue/templates/selectx.html'),
        props: {
    ...
    Vue.component("cly-check", userovoVue.components.BaseComponent.extend(
        // @vue/component
        {
            props: {
    ...
    Vue.component('cly-radio-block', userovoVue.components.BaseComponent.extend({
        props: {
    ...
    Vue.component('cly-dynamic-textarea', userovoVue.components.BaseComponent.extend({
    ...
    Vue.component('cly-select-email', userovoVue.components.BaseComponent.extend({
        mixins: [
            _mixins.i18n
        ],
    ...
    Vue.component("cly-sortable-items", userovoVue.components.BaseComponent.extend({
        props: {
            value: {
                type: Array
            },
