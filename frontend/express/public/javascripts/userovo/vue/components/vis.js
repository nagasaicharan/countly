/* global Vue, userovoCommon, userovoLocation, _mergeWith, CommonConstructor, userovoGlobal, Vue2Leaflet, CV, moment, L, userovoGraphNotesCommon, userovoAuth */
// _mergeWith is Lodash mergeWith - /frontend/express/public/javascripts/utils/lodash.mergeWith.js

(function(userovoVue) {

    var userovoBaseComponent = userovoVue.components.BaseComponent,
        _mixins = userovoVue.mixins;
    ...
    userovoVue.mixins.zoom = ExternalZoomMixin;
            ...
            handleXAxisOverflow: function(options, strategy, size) {
                ...
                if (xAxis.data.length) {
                    xAxis.data = xAxis.data.map(function(item) {
                        return userovoCommon.unescapeHtml(item);
                    });
                }
                ...
                if (longestLabelTextW > labelW) {
                    ...
                    returnObj.xAxis.axisLabel.formatter = function(value) {
                        value = userovoCommon.encodeHtml(value);
                        var ellipsis = "...";
    ...
    var GraphNotesMixin = {
        ...
        computed: {
            areNotesHidden: function() {
                return this.$store.getters['userovoCommon/getAreNotesHidden'];
            }
        },
        methods: {
            ...
            graphNotesTimeConverter: function(ts) {
                ...
                if (this.category === "drill" || this.category === "formulas") {
                    if (this.notationSelectedBucket === "hourly") {
                        return userovoCommon.formatDate(moment(graphNoteDate), "D MMM YYYY hh:00") || 0;
                    }
                    else if (this.notationSelectedBucket === "daily") {
                        return userovoCommon.formatDate(moment(graphNoteDate), "D MMM YYYY") || 0;
                    }
                        ...
                        return userovoCommon.formatDate(moment(graphNoteDate), "MMM YYYY");
                    }
                }
                    ...
                    if (this.notationSelectedBucket === "weekly") {
                        ...
                        return userovoCommon.formatDate(moment(graphNoteDate), "YYYY MMM");
                    }
                }
                else {
                    var xAxisLabel = null;
                    ...
                    return userovoCommon.formatDate(moment(ts), formatType) || 0;
                }
            },
            ...
            graphNotesFilterChecks: function() {
                var returnedObj = {};
                var filter = {};
                var appIds = [userovoCommon.ACTIVE_APP_ID];
                if (this.$parent && this.$parent.data) {
                    ...
                    if (this.$parent.data.custom_period && this.$parent.data.custom_period.length) {
                        if (typeof this.$parent.data.custom_period === "string") {
                            var convertedTimeObj = userovoCommon.getPeriodObj(this.$parent.data.custom_period);
            ...
            getGraphNotes: function() {
                if (!this.hideNotation && !this.areNotesHidden) {
                    ...
                    userovoCommon.getGraphNotes(filter.appIds, filter.customPeriod /*{category: categories.length ? categories : [this.category]}*/).then(function(data) {
                        ...
                        if (self.notes && self.notes.length) {
                            ...
                            if ((Array.isArray(userovoCommon.periodObj._period) && userovoCommon.periodObj.currentPeriodArr.length > 30)) {
                                mergeByDate = true;
                            }
                            ...
                            self.mergedNotes.forEach(function(note, index) {
                                ...
                                self.seriesOptions.markPoint.data[index].itemStyle = {
                                    color: note.times > 1 ? userovoGraphNotesCommon.COLOR_TAGS[0].label : userovoGraphNotesCommon.COLOR_TAGS.find(x=>x.value === note.color).label
                                };
            ...
            onClick: function() {
                ...
                userovoCommon.DISABLE_AUTO_REFRESH = true;
            }
        },
        watch: {
    ...
    */
    ...
    var BaseChart = _mixins.BaseContent.extend({
        ...
        props: {
            ...
            valFormatter: {
                type: Function,
                default: userovoCommon.getShortNumber,
                required: false
            },
            skin: {
                type: String,
        ...
        data: function() {
            ...
            return {
                baseOptions: {
                            ...
                            if (params.seriesType === 'pie') {
                                ...
                                for (var i = 0; i < params.length; i++) {
                                    if (params[i].seriesName.toLowerCase() === 'duration' || params[i].seriesName.toLowerCase() === 'avg. duration') {
                                        formatter = userovoCommon.formatSecond;
                                    }
                                    else {
                        ...
                        axisLabel: {
                            ...
                            formatter: (value) => {
                                if (typeof value === "number") {
                                    return userovoCommon.getShortNumber(this.sanitizeHtml(value));
                                }
                                return value;
                            }
                        },
                    ...
                    color: userovoCommon.GRAPH_COLORS,
                    textStyle: {
        ...
        methods: {
            ...
            sanitizeHtml: function(value) {
                if (value) {
                    value = userovoCommon.encodeHtml(value);
                    return userovoCommon.unescapeHtml(value);
                }
                return value;
            }
        }
    ...
    */
    ...
    /*
    var ZoomDropdown = userovoBaseComponent.extend({
        data: function() {
            return {
                zoomNumbers: [
                    {
    ...
    */
    ...
    var ZoomInteractive = userovoBaseComponent.extend({
        props: {
            echartRef: {
                type: Object
            },
    ...
    var MagicSwitch = userovoBaseComponent.extend({
        props: {
            chartType: {
                type: String,
                default: 'line'
    ...
    var AnnotationHandleCommand = {
        ...
        methods: {
            ...
            notesVisibility: function() {
                this.$store.dispatch('userovoCommon/setAreNotesHidden', !this.areNotesHidden);
            },
        }
    };

    userovoVue.mixins.graphNotesCommand = AnnotationHandleCommand;

    var AnnotationManagement = userovoBaseComponent.extend({
        props: {
            category: {
                type: String,
                default: '',
        ...
        mixins: [userovoVue.mixins.hasDrawers("annotation"), userovoVue.mixins.i18n, userovoVue.mixins.graphNotesCommand],
        data: function() {
            return {
                selectedItem: '',
                drawerSettings: {
        ...
        computed: {
            hasCreateRight: function() {
                return userovoAuth.validateCreate("core");
            },
            hasUpdateRight: function() {
                return userovoAuth.validateUpdate("core");
            }
        },
        methods: {
            refresh: function() {
            ...
            getIconUrl: function(icon) {
                return `${userovoGlobal.path}/images/annotation/${icon}.svg`;
            }
        },
        components: {
    ...
    var ChartHeader = userovoBaseComponent.extend({
    ...
    var SecondaryLegend = userovoBaseComponent.extend({
        mixins: [
    ...
    var PrimaryLegend = userovoBaseComponent.extend({
        mixins: [
            userovoVue.mixins.commonFormatters,
        ],
        props: {
    ...
    */
    var CustomLegend = userovoBaseComponent.extend({
        mixins: [EchartRefMixin],
        props: {
            options: {
                type: Object,
            ...
            chartOptions: function() {
                ...
                if (!opt.xAxis.data) {
                    ...
                    */
                    ...
                    var period = (this.period && this.period.length) ? this.period : userovoCommon.getPeriod();
                    ...
                    else if (userovoCommon.periodObj.numberOfDays === 1 && this.noHourly) {
                        tickObj = {
                            ticks: [[0, userovoCommon.formatDate(moment((userovoCommon.periodObj.activePeriod).replace(/\./g, "/"), "YYYY/MM/DD"), "D MMM")]],
                            tickTexts: [userovoCommon.formatDate(moment((userovoCommon.periodObj.activePeriod).replace(/\./g, "/"), "YYYY/MM/DD"), "D MMM")]
                        };
                    }
                    else {
    ...
    Vue.component("cly-chart-bar", BaseBarChart.extend({
        ...
        template: '<div class="cly-vue-chart" :class="chartClasses" :style="chartStyles" :data-test-id="testId + \'-chart\'">\
                        <div class="cly-vue-chart__echart bu-is-flex bu-is-flex-direction-column bu-is-flex-grow-1 bu-is-flex-shrink-1" style="min-height: 0">\
                            <chart-header :chart-type="\'bar\'" ref="header" v-if="!isChartEmpty" :test-id="testId" @series-toggle="onSeriesChange" :show-zoom="showZoom" :show-toggle="showToggle" :show-download="showDownload" @graph-notes-refresh="refresh" @notes-visibility="notesVisibility">\
                                <template v-for="item in forwardedSlots" v-slot:[item]="slotScope">\
                                    <slot :name="item" v-bind="slotScope"></slot>\
                                </template>\
                            </chart-header>\
    ...
    Vue.component("cly-map-picker", userovoVue.components.create({
        components: {
        ...
        template: CV.T('/javascripts/userovo/vue/templates/mappicker.html')
    }));

    Vue.component("cly-worldmap", userovoVue.components.create({
        components: {
            'l-map': Vue2Leaflet.LMap,
        ...
        mixins: [userovoVue.mixins.commonFormatters, userovoVue.mixins.i18n],
        props: {
            navigationLoading: {
                type: Boolean,
                default: false,
        ...
        computed: {
            ...
            locations: function() {
                ...
                case "main":
                    ...
                    arr = countryCodes.map(function(code) {
                        return {
                            label: userovoLocation.getCountryName(code),
                            value: code,
                            icon: userovoGlobal.cdn + "images/flags/" + code.toLowerCase() + ".svg",
                ...
                case "regions":
                    ...
                    arr = regionCodes.map(function(code) {
                        return {
                            label: userovoLocation.getRegionName(code, self.country),
                            value: code,
            ...
            countryName: function() {
                return userovoLocation.getCountryName(this.country);
            },
        ...
        methods: {
            ...
            getMarkerTooltipTitle: function(code) {
                switch (this.currentViewType) {
                case "main":
                    return userovoLocation.getCountryName(code);
                case "regions":
                    return userovoLocation.getRegionName(code, self.country);
                case "cities":
                    return code;
                }
            },
            ...
            countryIcon: function(code) {
                return userovoGlobal.cdn + "images/flags/" + code.toLowerCase() + ".svg";
            }
        },
        template: CV.T('/javascripts/userovo/vue/templates/worldmap.html')
    }));
