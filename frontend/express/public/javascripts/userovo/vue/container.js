/* global userovoAuth, UserovoHelpers*/
...
(function(userovoVue) {
    ...
    Container.prototype.registerData = function(id, value, type) {
        if (value && (value.pluginName || value.permission) && !UserovoHelpers.isPluginEnabled(value.pluginName || value.permission)) {
            return;
        }
        else {
    ...
    Container.prototype.registerTab = function(id, tab) {
        if (tab) {
            if ((tab.pluginName || tab.permission) && !UserovoHelpers.isPluginEnabled(tab.pluginName || tab.permission)) {
                return;
            }
            else {
                if (!Object.prototype.hasOwnProperty.call(this.dict, id)) {
    ...
    Container.prototype.registerMixin = function(id, mixin) {
        if (mixin && (!mixin.pluginName || UserovoHelpers.isPluginEnabled(mixin.pluginName))) {
            if (!Object.prototype.hasOwnProperty.call(this.dict, id)) {
                this.dict[id] = {};
            }
                    ...
                    if (Array.isArray(dataOb)) {
                        acc[val] = dataOb.filter(function(data) {
                            if (data && data.permission) {
                                return userovoAuth.validateRead(data.permission);
                            }
                            return true;
                        });
                    }
                        ...
                        for (var key in dataOb) {
                            if (dataOb[key] && dataOb[key].permission) {
                                if (userovoAuth.validateRead(dataOb[key].permission)) {
                                    acc[val] = dataOb;
                                }
                            }
                            else {
            ...
            data: function() {
                var ob = Object.keys(mapping).reduce(function(acc, val) {
                    acc[val] = (self.dict[mapping[val]] ? self.dict[mapping[val]].tabs : []).filter(function(tab) {
                        var permissionOk;
                        if (tab.permission) {
                            permissionOk = userovoAuth.validateRead(tab.permission);
                        }
                        else {
                            permissionOk = userovoAuth.validateGlobalAdmin();
                        }
                        if (!permissionOk) {
                            return false;
                        }
    ...
    userovoVue.container = new Container();
