/* global userovoCommon, jQuery, Vue, Vuex, T, userovoView, VueCompositionAPI, app, userovoGlobal, store, userovoAuth, UserovoHelpers */
...
(function(userovoVue, $) {
    ...
    var _i18n = function() {
        var appType = (userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID] && userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].type) || "mobile";
        arguments = arguments || [];
    ...
    */
        ...
        return {
            ...
            computed: {
                canUserCreate: function() {
                    return checkAuthArray(userovoAuth.validateCreate);
                },
                canUserRead: function() {
                    return checkAuthArray(userovoAuth.validateRead);
                },
                canUserUpdate: function() {
                    return checkAuthArray(userovoAuth.validateUpdate);
                },
                canUserDelete: function() {
                    return checkAuthArray(userovoAuth.validateDelete);
                },
                isUserGlobalAdmin: function() {
                    return userovoAuth.validateGlobalAdmin();
                }
            }
        };
    };
    ...
    var commonFormattersMixin = {
        methods: {
            parseTimeAgo: userovoCommon.formatTimeAgoText,
            formatTimeAgo: userovoCommon.formatTimeAgo,
            formatNumber: userovoCommon.formatNumber,
            formatNumberSafe: userovoCommon.formatNumberSafe,
            getShortNumber: userovoCommon.getShortNumber,
            unescapeHtml: userovoCommon.unescapeHtml
        }
    };
    ...
    var DashboardsAppsMixin = {
        computed: {
            __allApps: function() {
                return this.$store.getters["userovoDashboards/allApps"];
            }
        },
        methods: {
        ...
        methods: {
            ...
            calculateStackedBarOptionsFromWidget: function(widgetData, map) {
                ...
                if (widgetData.bar_color && widgetData.bar_color > 0) {
                    return {xAxis: {data: labels}, series: [{"name": metricName, color: userovoCommon.GRAPH_COLORS[this.data.bar_color - 1], "data": series, stack: "A"}]};
                }
                else {
                    ...
                    series: [
                        {
                            ...
                            data: dd.graph,
                            label: {
                                formatter: "{a|" + namingMap[metric] + "}\n" + (userovoCommon.getShortNumber(total) || 0),
                                fontWeight: 500,
                                fontSize: 16,
                                fontFamily: "Inter",
                                lineHeight: 24,
        ...
        modules: {
            userovoCommon: {
                ...
                state: {
                    ...
                    period: userovoCommon.getPeriod(),
                    periodLabel: userovoCommon.getDateRangeForCalendar(),
                    activeApp: null,
                    allApps: userovoGlobal.apps,
                    notificationToasts: [],
                ...
                mutations: {
                    ...
                    addNotificationToast: function(state, payload) {
                        payload.id = userovoCommon.generateId();
    ...
    $(document).ready(function() {
        app.addAppSwitchCallback(function(appId) {
            _globalVuexStore.dispatch("userovoCommon/updateActiveApp", appId);
        });
    });
    ...
    var NotificationToastsView = {
        ...
        computed: {
            notificationToasts: function() {
                return this.$store.state.userovoCommon.notificationToasts;
            }
        },
        methods: {
            onClose: function(id) {
                this.$store.dispatch('userovoCommon/onRemoveNotificationToast', id);
            }
        }
    };
    ...
    var DialogsView = {
        ...
        computed: {
            messageDialogs: function() {
                return this.$store.getters['userovoCommon/messageDialogs'];
            },
            confirmDialogs: function() {
                return this.$store.getters['userovoCommon/confirmDialogs'];
            },
            blockerDialogs: function() {
                return this.$store.getters['userovoCommon/blockerDialogs'];
            },
        },
        methods: {
            onCloseDialog: function(dialog, status) {
                ...
                this.$store.dispatch('userovoCommon/onRemoveDialog', dialog.id);
            }
        }
    };
    ...
    var QuickstartPopoverView = {
        ...
        computed: {
            quickstartContent: function() {
                return this.$store.getters['userovoCommon/quickstartContent'];
            },
        },
        methods: {
            handleCloseClick: function(dialogId) {
                this.$store.dispatch('userovoCommon/onRemoveDialog', dialogId);
            },
        },
    };

    var userovoVueWrapperView = userovoView.extend({
    ...
    var userovoBaseComponent = Vue.extend({
        mixins: [
    ...
    var userovoBaseView = userovoBaseComponent.extend(
        // @vue/component
        {
            mixins: [
                _mixins.autoRefresh,
    ...
    var BaseContentMixin = userovoBaseComponent.extend(
        // @vue/component
        {
            inheritAttrs: false,
            mixins: [
    ...
    var _components = {
        BaseComponent: userovoBaseComponent,
        create: asyncCreate(userovoBaseComponent)
    };

    var _views = {
        BackboneWrapper: userovoVueWrapperView,
        BaseView: userovoBaseView,
        create: asyncCreate(userovoBaseView)
    };
    ...
    for (var key in rootElements) {
        userovoVue[key] = rootElements[key];
    }

    window.CV = userovoVue;
