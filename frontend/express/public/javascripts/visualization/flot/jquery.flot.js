...
(function($) {
	...
	Canvas.prototype.addText = function(layer, x, y, text, font, angle, width, halign, valign) {
		...
		if (valign == "middle") {
			...
			y -= info.height;
		}  else if (valign == "top") { // onur, add top option
            y -= info.height + 2;
        }
    ...
    function Plot(placeholder, data_, options_, plugins) {
        ...
        function adjustLayoutForThingsStickingOut() {
	        ...
	        // onur, left and right are fixed values now
            plotOffset.left = 10; // Math.ceil(Math.max(margins.left, plotOffset.left));
            plotOffset.right = 10; // Math.ceil(Math.max(margins.right, plotOffset.right));
