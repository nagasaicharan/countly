...
(function(userovoAlerts, $) {
    const eventMaps = {};
    var FEATURE_NAME = "alerts";
    userovoAlerts.RatingOptions = [
    ...
    function extractEvents(data, returnArray) {
        ...
        if (eventData && eventData.list) {
            for (var i = 0; i < eventData.list.length; i++) {
                // var eventNamePostfix = " (" + userovoGlobal.apps[eventData._id].name + ")";
                returnArray.push({
    ...
    userovoAlerts.getCohortsForApp = getCohortsForApp;
    ...
    function getSurveysForApp(appId, callback) {
        $.ajax({
            type: "GET",
            url: userovoCommon.API_PARTS.data.r + "/surveys/survey/widgets",
            data: {
                app_id: appId,
            },
            dataType: "json",
    ...
    userovoAlerts.getSurveysForApp = getSurveysForApp;
    ...
    function getNPSForApp(appId, callback) {
        $.ajax({
            type: "GET",
            url: userovoCommon.API_PARTS.data.r + "/surveys/nps/widgets",
            data: {
                app_id: appId,
            },
            dataType: "json",
    ...
    userovoAlerts.getNPSForApp = getNPSForApp;
    ...
    userovoAlerts.getCrashesForFilter = getCrashesForFilter;
    ...
    function getRatingForApp(appId, callback) {
        $.ajax({
            type: "GET",
            url: userovoCommon.API_PARTS.data.r + "/feedback/widgets",
            data: {
                app_id: appId,
            },
            dataType: "json",
    ...
    userovoAlerts.getRatingForApp = getRatingForApp;
    ...
    userovoAlerts.getEventsForApp = function(appId, callback) {
        if (!appId) {
            callback([]);
            return;
        }
    ...
    userovoAlerts.getViewForApp = function(appId, callback) {
        if (!appId) {
            return callback([]);
        }
        $.ajax({
    ...
    userovoAlerts.getVuexModule = function() {
        ...
        var actions = {
            ...
            refresh: function(context) {
                context.dispatch("userovoAlerts/table/fetchAll", null, {
                    root: true,
                });
            },
            saveAlert: function(context, alertConfig) {
                ...
                return CV.$.ajax({
                    type: "GET",
                    url: userovoCommon.API_PARTS.data.w + "/alert/save",
                    data: {
                        alert_config: JSON.stringify(alertConfig),
                        app_id:
                            alertConfig.selectedApps[0] === "all"
                                ? userovoCommon.ACTIVE_APP_ID
            ...
            deleteAlert: function(context, options) {
                return CV.$.ajax({
                    type: "GET",
                    url: userovoCommon.API_PARTS.data.w + "/alert/delete",
                    data: {
            ...
            deleteOnlineUsersAlert: function(context, options) {
                return CV.$.ajax({
                    type: "GET",
                    url:
                        userovoCommon.API_PARTS.data.w +
                    ...
                    error: function(e) {
                        UserovoHelpers.notify({ message: e });
                    },
                });
            },
                ...
                return CV.$.ajax({
                    ...
                    success: function(data) {
                        if (typeof data === "object" && data.result) {
                            UserovoHelpers.alert(data.result, "red");
                            return;
                        }
                        UserovoHelpers.notify({
                            message:
        ...
        var tableResource = userovoVue.vuex.Module("table", {
            ...
            actions: {
                updateStatus: function(context, status) {
                    return CV.$.ajax({
                        type: "post",
                        url: userovoCommon.API_PARTS.data.w + "/alert/status",
                        data: {
                            app_id: userovoCommon.ACTIVE_APP_ID,
                        ...
                        dataType: "json",
                        success: function() {
                            // UserovoHelpers.notify({message: jQuery.i18n.map['alerts.update-status-success']});
                        },
                    });
                },
                    ...
                    return CV.$.ajax({
                        ...
                        for (var i = 0; i < alertsList.length; i++) {
                            ...
                            if (alertsList[i].selectedApps) {
                                appNameList = _.map(
                                    ...
                                    function(appID) {
                                        ...
                                        return (
                                            userovoGlobal.apps[appID] &&
                                            userovoGlobal.apps[appID].name
                                        );
                                    }
                                );
                            }
                            ...
                            rowData0 = Object.assign(rowData0, {
                                ...
                                _canUpdate: userovoAuth.validateUpdate(
                                    FEATURE_NAME,
                                    userovoGlobal.member,
                                    alertsList[i].selectedApps[0]
                                ),
                                _canDelete: userovoAuth.validateDelete(
                                    FEATURE_NAME,
                                    userovoGlobal.member,
                                    alertsList[i].selectedApps[0]
                                ),
                        ...
                        if (
                            userovoGlobal.plugins.indexOf("concurrent_users") <
                                0 ||
                            !userovoAuth.validateRead("concurrent_users")
                        ) {
                        ...
                        CV.$.ajax({
                            ...
                            for (var j = 0; j < list.length; j++) {
                                ...
                                rowData = Object.assign(rowData, {
                                    ...
                                    appNameList:
                                        userovoGlobal.apps[list[j].app].name,
        ...
        return userovoVue.vuex.Module("userovoAlerts", {
    ...
    userovoAlerts.defaultDrawerConfigValue = function() {
        return {
            _id: null,
            alertName: null,
            alertDataType: null,
...
})((window.userovoAlerts = window.userovoAlerts || {}), jQuery);
