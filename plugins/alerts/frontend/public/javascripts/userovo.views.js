...
(function() {
    ...
    var AlertDrawer = userovoVue.views.BaseView.extend({
        ...
        computed: {
            ...
            filteredEmailOptions: function() {
                if (!userovoGlobal.plugins.includes("groups")) {
            ...
            alertIntervalValidationRules: function() {
                ...
                if (this.$refs.drawerData.editedObject.alertDataType === 'onlineUsers') {
                    var concurrentUserConfig = userovoPlugins.getConfigsData().concurrent_users || {};
        ...
        methods: {
            ...
            getMetrics: function() {
                ...
                if (formData.alertDataType === "views") {
                    userovoAlerts.getViewForApp(
                        formData.selectedApps,
                        (viewList) => {
                            this.alertDataSubType2Options = viewList.map(
                                (v) => {
                                    return { value: v.value, label: userovoCommon.unescapeHtml(v.name) };
                                }
                            );
                        }
                    );
                ...
                if (formData.alertDataType === "events") {
                    userovoAlerts.getEventsForApp(
                        formData.selectedApps,
                        ({ events, segments }) => {
                            this.alertDataSubType2Options = events.map((e) => {
                                return { value: e.value, label: userovoCommon.unescapeHtml(e.name) };
                            });
                ...
                if (formData.alertDataType === "cohorts") {
                    userovoAlerts.getCohortsForApp(
                        formData.selectedApps,
                        (data) => {
                            ...
                            this.alertDataSubType2Options = filtered.map((c) => {
                                return { value: c._id, label: userovoCommon.unescapeHtml(c.name) };
                            });
                        }
                    );
                }
                ...
                if (formData.alertDataType === "survey") {
                    userovoAlerts.getSurveysForApp(
                        formData.selectedApps,
                        (data) => {
                            this.alertDataSubType2Options = data.filter(s => s.status).map((s) => {
                                return { value: s._id, label: userovoCommon.unescapeHtml(s.name) };
                            });
                        }
                    );
                }
                if (formData.alertDataType === "nps") {
                    userovoAlerts.getNPSForApp(
                        formData.selectedApps,
                        (data) => {
                            this.alertDataSubType2Options = data.filter(n => n.status).map((n) => {
                                return { value: n._id, label: userovoCommon.unescapeHtml(n.name) };
                            });
                        }
                    );
                }
                if (formData.alertDataType === "rating") {
                    userovoAlerts.getRatingForApp(
                        formData.selectedApps,
                        (data) => {
                            this.alertDataSubType2Options = data.filter(r => r.is_active === 'true').map((r) => {
                                return {
                                    value: r._id,
                                    label: userovoCommon.unescapeHtml(r.popup_header_text),
                                };
                            });
                        }
                    );
            ...
            dataTypeSelected: function(val) {
                ...
                if (
                    val === "dataPoint" &&
                    userovoGlobal.member.global_admin === true
                ) {
                    this.allowAll = true;
                }
                if (val === "onlineUsers") {
            ...
            setFilterValueOptions: function() {
                ...
                if (formData.alertDataType === "crashes") {
                    ...
                    userovoAlerts.getCrashesForFilter(
                        formData.selectedApps,
                        (data) => {
            ...
            onSubmit: function(settings) {
                ...
                if (settings._id) {
                    var rows = this.$store.getters["userovoAlerts/table/all"];
                    for (var i = 0; i < rows.length; i++) {
                        if (
                            ...
                            if (rows[i].alertDataType !== "onlineUsers") {
                                this.$store.dispatch(
                                    "userovoAlerts/deleteAlert",
                                    rows[i]._id
                                );
                            }
                            else {
                                this.$store.dispatch(
                                    "userovoAlerts/deleteOnlineUsersAlert",
                                    rows[i]
                                );
                            }
                            settings._id = null;
                ...
                if (settings.alertDataType === "onlineUsers") {
                    var config = {
                        ...
                        app_name:
                            userovoGlobal.apps[settings.selectedApps[0]].name,
                    ...
                    this.$store.dispatch(
                        "userovoAlerts/saveOnlineUsersAlert",
                        config
                    );
                    this.resetAlertConditionShow();
                    return;
                }
                this.$store.dispatch("userovoAlerts/saveAlert", settings);
                this.resetAlertConditionShow();
            },
            ...
            onCopy: function(newState) {
                ...
                newState.alertName = userovoCommon.unescapeHtml(newState.alertName);
                newState.alertValues = newState.alertValues.map((email) => userovoCommon.unescapeHtml(email));
                this.getMetrics();
                this.setFilterKeyOptions();
                this.setFilterValueOptions();
                if (newState._id !== null) {
                    ...
                    this.alertDataFilterKey = userovoCommon.unescapeHtml(newState.filterKey);
                    this.alertDataFilterValue = userovoCommon.unescapeHtml(newState.filterValue);
    ...
    var TableView = userovoVue.views.BaseView.extend({
        ...
        mixins: [userovoVue.mixins.auth(ALERTS_FEATURE_NAME)],
        computed: {
            tableRows: function() {
                var rows = this.$store.getters["userovoAlerts/table/all"];
            ...
            initialized: function() {
                var result =
                    this.$store.getters["userovoAlerts/table/getInitialized"];
                return result;
            },
        ...
        data: function() {
            ...
            for (var id in userovoGlobal.apps) {
                appsSelectorOption.push({
                    label: userovoGlobal.apps[id].name,
                    value: id,
                });
            }
        ...
        methods: {
            ...
            updateStatus: function(scope) {
                ...
                this.$store
                    .dispatch("userovoAlerts/table/updateStatus", alertStatus)
                    .then(function() {
                        return self.$store
                            .dispatch(
                                "userovoAlerts/table/updateOnlineusersAlertStatus",
                                self.onlineUsersAlertStatus
                            )
                            .then(function() {
                                return self.$store.dispatch(
                                    "userovoAlerts/table/fetchAll"
                                );
                            });
                    });
            },
    ...
    var AlertsHomeViewComponent = userovoVue.views.BaseView.extend({
        template: "#alerts-home",
        mixins: [
            userovoVue.mixins.hasDrawers("home"),
            userovoVue.mixins.auth(ALERTS_FEATURE_NAME),
        ],
        components: {
        ...
        computed: {
            countData: function() {
                var count = this.$store.getters["userovoAlerts/table/count"];
                return [
        ...
        data: function() {
            return {
                canCreate: userovoAuth.validateCreate(ALERTS_FEATURE_NAME),
            };
        },
        beforeCreate: function() {
            this.$store.dispatch("userovoAlerts/initialize");
        },
        methods: {
            createAlert: function() {
                var config = userovoAlerts.defaultDrawerConfigValue();
                this.openDrawer("home", config);
            },
        },
    });
    var alertsView = new userovoVue.views.BackboneWrapper({
        component: AlertsHomeViewComponent,
        vuex: [
            {
                clyModel: userovoAlerts,
            },
        ],
