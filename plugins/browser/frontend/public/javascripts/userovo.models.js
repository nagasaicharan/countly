/*global UserovoHelpers, jQuery, userovoBrowser, userovoCommon, _*/
UserovoHelpers.createMetricModel(window.userovoBrowser = window.userovoBrowser || {}, {name: "browser", estOverrideMetric: "browser"}, jQuery);

userovoBrowser.getBrowserData = function() {
    var chartData = userovoCommon.extractTwoLevelData(userovoBrowser.getDb(), userovoBrowser.getMeta("browser"), userovoBrowser.clearObject, [
        {
            name: "browser",
    ...
    chartData.chartData = userovoCommon.mergeMetricsByName(chartData.chartData, "browser");
...
userovoBrowser.getSegmentedData = function(browser) {
    browser = browser.toLowerCase();
    var metric = "browser_version";
    var versionData = {};
    versionData = userovoCommon.extractTwoLevelData(userovoBrowser.getDb(), userovoBrowser.getMeta(metric), userovoBrowser.clearObject, [
        {
            name: metric,
            func: function(rangeArr) {
    ...
    versionData.chartData = userovoCommon.mergeMetricsByName(versionData.chartData, metric);
...
userovoBrowser.fixBrowserVersion = function(val, data) {
    var browsers = data || userovoBrowser.getMeta("browser");
...
userovoBrowser.getBrowserVersionList = function(name, data) {
...
userovoBrowser.getBrowserVersionData = function(browser) {
    browser = browser.toLowerCase();
    var data = userovoBrowser.getData(true, false, "browser_version").chartData;
    var ret = [];
