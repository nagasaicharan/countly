/*global userovoCommon,UserovoHelpers, jQuery, CV, userovoGlobal, userovoVue, userovoTaskManager */

(function(userovoConsentManager) {
    UserovoHelpers.createMetricModel(userovoConsentManager, {name: "consents", estOverrideMetric: "consents"}, jQuery);
    userovoConsentManager.service = {
        ...
        deleteUserdata: function(query, callback) {
            CV.$.ajax({
                type: "POST",
                url: userovoCommon.API_PARTS.data.w + "/app_users/delete",
                data: {
        ...
        deleteExport: function(eid, callback) {
            CV.$.ajax({
                type: "POST",
                url: userovoCommon.API_PARTS.data.w + "/app_users/deleteExport/appUser_" + userovoCommon.ACTIVE_APP_ID + "_" + eid,
                data: {
                    "app_id": userovoCommon.ACTIVE_APP_ID,
                },
                success: function(result) {
        ...
        exportUser: function(query, callback) {
            CV.$.ajax({
                type: "POST",
                url: userovoCommon.API_PARTS.data.w + "/app_users/export",
                data: {
                    "app_id": userovoCommon.ACTIVE_APP_ID,
                    "query": query
                },
        ...
        userData: function(data, path, type) {
            ...
            var apiQueryData = CV.$.ajax({
                type: type,
                url: userovoCommon.API_PARTS.data.r + '/app_users/consents' + path,
                data: data,
                dataType: "json",
            });
            return apiQueryData;
        ...
        consentHistoryTable: function(data, path, type) {
            ...
            var apiQueryData = CV.$.ajax({
                type: type,
                url: userovoCommon.API_PARTS.data.r + '/consent/search' + path,
                data: data,
                dataType: "json",
            });
            return apiQueryData;
        ...
        common: function(data, path, type) {
            ...
            var apiQueryData = CV.$.ajax({
                type: type,
                url: userovoCommon.API_PARTS.data.r + '/consent' + path,
                data: data,
                dataType: "json",
            });
            return apiQueryData;
    ...
    var userDataResource = userovoVue.vuex.ServerDataTable("userDataResource", {
        ...
        onRequest: function(context) {
            ...
            return {
                type: "POST",
                url: userovoCommon.API_PARTS.data.r + '/app_users/consents',
                data: data
            };
        },
        onReady: function(context, rows) {
            for (var k = 0; k < rows.length; k++) {
                rows[k].optin = [];
                rows[k].optout = [];
                rows[k].time = userovoCommon.formatTimeAgoText(rows[k].lac || 0).text;
    ...
    var exportHistoryDataResource = userovoVue.vuex.ServerDataTable("exportHistoryDataResource", {
        ...
        onReady: function(context, rows) {
            for (var k = 0; k < rows.length; k++) {
                rows[k].time = userovoCommon.formatTimeAgoText(rows[k].ts || 0).text;
                var row = rows[k];
                var ret = "<p>" + ((jQuery.i18n.map["systemlogs.action." + row.a]) ? jQuery.i18n.map["systemlogs.action." + row.a] : row.a) + "</p>";
                if (typeof row.i === "object") {
                    if (typeof row.i.app_id !== "undefined" && userovoGlobal.apps[row.i.app_id]) {
                        ret += "<p title='" + row.i.app_id + "'>" + jQuery.i18n.map["systemlogs.for-app"] + ": " + userovoGlobal.apps[row.i.app_id].name + "</p>";
                    }
    ...
    var consentHistoryResource = userovoVue.vuex.ServerDataTable("consentHistoryResource", {
        columns: ['_id', 'type', 'optin', 'optout', 'av', 'ts' ],
        loadedData: {},
        // eslint-disable-next-line
        onRequest: function(context) {
            context.rootState.userovoConsentManager.consentHistoryLoading = true;
            var data = {
                app_id: userovoCommon.ACTIVE_APP_ID,
                period: userovoCommon.getPeriodForAjax()
            };
            ...
            if (context.rootState.userovoConsentManager.uid !== "") {
                filter.uid = context.rootState.userovoConsentManager.uid;
            }

            data.filter = JSON.stringify(filter);
            return {
                type: "POST",
                url: userovoCommon.API_PARTS.data.r + '/consent/search',
                data: data
            };
        },
        onReady: function(context, rows) {
            ...
            context.rootState.userovoConsentManager.consentHistoryLoading = false;
            return rows;
        }
    });

    var consentHistoryUserResource = userovoVue.vuex.ServerDataTable("consentHistoryUserResource", {
        columns: ['_id', 'type', 'optin', 'optout', 'av', 'ts' ],
        // eslint-disable-next-line
        onRequest: function(context) {
            context.rootState.userovoConsentManager.isLoading = true;
            var data = {
                app_id: userovoCommon.ACTIVE_APP_ID
            };
            if (context.rootState.userovoConsentManager.uid) {
                data.query = JSON.stringify({ uid: context.rootState.userovoConsentManager.uid });
            }
            return {
                type: "POST",
                url: userovoCommon.API_PARTS.data.r + '/consent/search',
        ...
        onReady: function(context, rows) {
            ...
            context.rootState.userovoConsentManager.isLoading = false;
            return rows;
        }
    });
    userovoConsentManager.getVuexModule = function() {
        ...
        _consentManagerDbModule.actions._ePData = function(context) {
            var data = userovoCommon.getDashboardData(userovoConsentManager.getDb(), ["e", "p"], [], {}, userovoConsentManager.service.clearObj);
        ...
        _consentManagerDbModule.actions._purgeDP = function(context) {
            ...
            return context.commit("_purgeDP", userovoCommon.extractChartData(userovoConsentManager.getDb(), userovoConsentManager.service.clearObj, chartData, dataProps));
        };
        _consentManagerDbModule.actions._consentDP = function(context, payload) {
            ...
            return context.commit("_consentDP", userovoCommon.extractChartData(userovoConsentManager.getDb(), userovoConsentManager.service.clearObj, chartData, dataProps, payload.segment));
        };
        _consentManagerDbModule.actions._exportDP = function(context, payload) {
            ...
            var data = userovoCommon.extractChartData(userovoConsentManager.getDb(), userovoConsentManager.service.clearObj, chartData, dataProps, payload.segment);
            return context.commit("_exportDP", data);
        };
        _consentManagerDbModule.actions._bigNumberData = function(context, payload) {
            var data = userovoCommon.getDashboardData(userovoConsentManager.getDb(), ["i", "o"], [], {}, userovoConsentManager.service.clearObj, payload.segment);
        ...
        return userovoVue.vuex.Module("userovoConsentManager", _module);


    };
})(window.userovoConsentManager = window.userovoConsentManager || {});