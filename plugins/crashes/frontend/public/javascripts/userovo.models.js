/* globals app, userovoCrashSymbols, jQuery, userovoCommon, userovoAuth, userovoGlobal, userovoVue, userovoCrashesEventLogs, userovoSession, CV, $ */
...
(function(userovoCrashes) {
    var _list = {};
    var FEATURE_NAME = 'crashes';

    userovoCrashes.getVuexModule = function() {
        ...
        _overviewSubmodule.getters.dashboardData = function(state) {
            ...
            if ("data" in state.rawData) {
                dashboard = userovoCommon.getDashboardData(state.filteredData.data, ["cr", "crnf", "crf", "cru", "cruf", "crunf", "crru", "crau", "crauf", "craunf", "crses", "crfses", "crnfses", "cr_s", "cr_u"], ["cru", "crau", "cruf", "crunf", "crauf", "craunf", "cr_u"], null, userovoCrashes.clearObject);
            }
            else {
                return dashboard;
            }
            ...
            return function(metric, name) {
                ...
                if (typeof metricChartConfig !== "undefined") {
                    chartData = [
                        ...
                        {data: [], label: jQuery.i18n.map[metricChartConfig.labelKey], color: userovoCommon.GRAPH_COLORS[metricChartConfig.colorIndex]}
                    ];
                }
                else {
                    chartData = [
                ...
                var results = userovoCommon.extractChartData(state.filteredData.data, userovoCrashes.clearObject, chartData, dataProps);
            ...
            if ("crashes" in state.rawData) {
                ...
                Object.keys(state.rawData.crashes.os).forEach(function(platform) {
                    statistics.topPlatforms[platform] = {
                        count: userovoCommon.getShortNumber(state.rawData.crashes.os[platform]),
        ...
        _overviewSubmodule.actions.refresh = function(context, forceLoading) {
            ...
            if (isFiltered) {
                ajaxPromises.push(userovoVue.$.ajax({
                    type: "GET",
            ...
            ajaxPromises.push(userovoSession.initialize().then(function() {
                context.state.realSession = userovoSession.getSessionData();
            }));
        ...
        _crashgroupSubmodule.getters.mobileMetrics = function(state) {
            if (userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].type !== "web" && typeof state.crashgroup._id !== "undefined" && "root" in state.crashgroup) {
                return {
        ...
        _crashgroupSubmodule.getters.chartData = function(state) {
            ...
            return function(chartBy) {
                var mapObj = userovoCommon.dot(state.crashgroup, chartBy) || {};
        ...
        _crashgroupSubmodule.getters.crashEventLog = function(state) {
            return function(crashId) {
                ...
                return userovoCrashesEventLogs.formatEventLog(state.crashgroup.data[crashIndex].eventlog);
            };
        };
                    ...
                    success: function(crashgroupJson) {
                        if (crashgroupJson.data && crashgroupJson.data.length > 0) {
                            ...
                            crashgroupJson.data.forEach(function(crash, crashIndex) {
                                // unescape logs
                                crashgroupJson.data[crashIndex].logs = userovoCommon.unescapeHtml(crash.logs);
                            ...
                            if (userovoAuth.validateRead('users')) {
                            ...
                            if (typeof userovoCrashSymbols !== "undefined") {
                                ...
                                var ajaxPromise = userovoCrashSymbols.fetchSymbols(true);
                                ajaxPromises.push(ajaxPromise);
                                ajaxPromise.then(function(fetchSymbolsResponse) {
                                    crashes.forEach(function(crash, crashIndex) {
                                        var symbol_id = userovoCrashSymbols.canSymbolicate(crash, fetchSymbolsResponse.symbolIndexing);
        ...
        _crashgroupSubmodule.actions.generateEventLogs = function(context, crashIds) {
            return new Promise(function(resolve, reject) {
                userovoVue.$.ajax({
                    type: "GET",
                    url: userovoCommon.API_PARTS.data.w + "/crashes-event-logs",
                    data: {
                        "app_id": userovoCommon.ACTIVE_APP_ID,
                        "args": JSON.stringify({
                            "crashes": crashIds
        ...
        _crashgroupSubmodule.actions.symbolicate = function(context, crash) {
            return new Promise(function(resolve, reject) {
                if (typeof userovoCrashSymbols === "undefined") {
                    ...
                    userovoCrashSymbols.fetchSymbols(true).then(function(fetchSymbolsResponse) {
                        var symbol_id = userovoCrashSymbols.canSymbolicate(crash, fetchSymbolsResponse.symbolIndexing) || crash.symbol_id;
                        userovoCrashSymbols.symbolicate(crash._id, symbol_id)
                            .then(function(json) {
                                resolve(json);
                            })
                            .catch(function(xhr) {
        ...
        _crashgroupSubmodule.actions.markResolved = function(context) {
            return userovoCrashes.manipulateCrashgroup(context.state.crashgroup._id, "resolve").then(function() {
        ...
        _crashgroupSubmodule.actions.markResolving = function(context) {
            return userovoCrashes.manipulateCrashgroup(context.state.crashgroup._id, "resolving").then(function() {
        ...
        _crashgroupSubmodule.actions.markUnresolved = function(context) {
            return userovoCrashes.manipulateCrashgroup(context.state.crashgroup._id, "unresolve").then(function() {
                context.state.crashgroup.is_resolved = false;
            });
        };

        _crashgroupSubmodule.actions.markSeen = function(context) {
            return userovoCrashes.manipulateCrashgroup(context.state.crashgroup._id, "view").then(function() {
                context.state.crashgroup.is_new = false;
            });
        };

        _crashgroupSubmodule.actions.share = function(context) {
            return userovoCrashes.manipulateCrashgroup(context.state.crashgroup._id, "share");
        };

        _crashgroupSubmodule.actions.unshare = function(context) {
            return userovoCrashes.manipulateCrashgroup(context.state.crashgroup._id, "unshare");
        };

        _crashgroupSubmodule.actions.show = function(context) {
            return userovoCrashes.manipulateCrashgroup(context.state.crashgroup._id, "show").then(function() {
                context.state.crashgroup.is_hidden = false;
            });
        };

        _crashgroupSubmodule.actions.hide = function(context) {
            return userovoCrashes.manipulateCrashgroup(context.state.crashgroup._id, "hide").then(function() {
                context.state.crashgroup.is_hidden = true;
            });
        };

        _crashgroupSubmodule.actions.delete = function(context) {
            return userovoCrashes.manipulateCrashgroup(context.state.crashgroup._id, "delete");
        };

        _crashgroupSubmodule.actions.modifyShare = function(context, args) {
            return userovoCrashes.manipulateCrashgroup(context.state.crashgroup._id, "modify_share", args);
        };

        _crashgroupSubmodule.actions.addComment = function(context, body) {
            var originalPromise = userovoCrashes.manipulateCrashgroup(context.state.crashgroup._id, "add_comment", {text: body});
        ...
        _crashgroupSubmodule.actions.editComment = function(context, payload) {
            return userovoCrashes.manipulateCrashgroup(context.state.crashgroup._id, "edit_comment", payload);
        };

        _crashgroupSubmodule.actions.deleteComment = function(context, commentId) {
            return userovoCrashes.manipulateCrashgroup(context.state.crashgroup._id, "delete_comment", {comment_id: commentId});
        };
        ...
        var crashgroupsResource = userovoVue.vuex.ServerDataTable("crashgroups", {
        ...
        var _module = {
            ...
            mutations: {},
            submodules: [
                userovoVue.vuex.Module("overview", _overviewSubmodule),
                userovoVue.vuex.Module("crashgroup", _crashgroupSubmodule),
                userovoVue.vuex.Module("crash", _crashSubmodule),
                crashgroupsResource
            ]
        };

        return userovoVue.vuex.Module("userovoCrashes", _module);
    };

    userovoCrashes.manipulateCrashgroup = function(id, path, args) {
        args = args || {};
        ...
        if (typeof args.app_id === "undefined") {
            args.app_id = userovoCommon.ACTIVE_APP_ID;
        }

        return new Promise(function(resolve) {
            userovoVue.$.ajax({
                type: "GET",
                url: userovoCommon.API_PARTS.data.w + "/crashes/" + path,
                data: {
                    app_id: userovoCommon.ACTIVE_APP_ID,
    ...
    userovoCrashes.clearObject = function(obj) {
    ...
    userovoCrashes.generateBadges = function(crash) {
        var badges = [];
    ...
    userovoCrashes.loadList = function(id) {
        $.ajax({
            type: "GET",
            url: userovoCommon.API_PARTS.data.r,
            data: {
    ...
    userovoCrashes.userCrashes = function(userId) {
        return new Promise(function(resolve) {
            $.ajax({
                type: "GET",
                url: userovoCommon.API_PARTS.data.r,
    ...
    if (userovoCommon.ACTIVE_APP_ID !== 0 && userovoAuth.validateRead(FEATURE_NAME)) {
        userovoCrashes.loadList(userovoCommon.ACTIVE_APP_ID);
    }

    app.addAppSwitchCallback(function(appId) {
        if (app._isFirstLoad !== true && userovoAuth.validateRead(FEATURE_NAME)) {
            userovoCrashes.loadList(appId);
        }
    });

    userovoCrashes.getCrashName = function(id) {
        if (_list[id]) {
            return _list[id];
        }
        return id;
    ...
    userovoCrashes.modifyQueries = function(inpQuery) {
        var resultQuery = {};
    ...
    userovoCrashes.modifyExistsQueries = function(inpQuery) {
        var resultQuery = {};
        var existsGroups = {};
...
}(window.userovoCrashes = window.userovoCrashes || {}));
