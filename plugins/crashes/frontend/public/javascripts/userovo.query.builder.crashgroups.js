/* global userovoQueryBuilder, jQuery, _ */
    ...
    jQuery(function() {
        if (window.userovoQueryBuilder) {
            ...
            var importSubquery = function(query, conjunction) {
                var rawRows = [];
                conjunction = conjunction || userovoQueryBuilder.RowConj.AND;
                            ...
                            _.flatten(
                                query[key].map(function(andBranch) {
                                    return importSubquery(
                                        andBranch,
                                        userovoQueryBuilder.RowConj.AND
                                    );
                                })
                            )
                        );
                        ...
                        rawRows = rawRows.concat(
                            _.flatten(
                                query[key].map(function(orBranch, idx) {
                                    ...
                                    return importSubquery(
                                        orBranch,
                                        userovoQueryBuilder.RowConj.OR
                                    );
                                })
                            )
                        );
            ...
            userovoQueryBuilder.registerQueryFormat("crashgroups", {
                import: function(value) {
                    if (!value.query && !value.byVal) {
                        return new userovoQueryBuilder.FrozenQuery({});
                    }
                    ...
                    return new userovoQueryBuilder.FrozenQuery({
                        rows: importSubquery(value.query).map(function(
                            ...
                            row.id = idx;
                            return new userovoQueryBuilder.FrozenRow(row);
                        }),
                ...
                export: function(currentQuery) {
                    ...
                    Object.keys(grouped).forEach(function(key) {
                        if (key === userovoQueryBuilder.NO_GROUP) {
                            mainObject = exportOrGroup(
                                grouped[userovoQueryBuilder.NO_GROUP]
                            );
                        }
                        else {
