/*global app, userovoVue, userovoDashboards, userovoGlobal, CV, _, groupsModel, Backbone, GridStack, UserovoHelpers, $, screenfull, userovoCommon */

(function() {
    var AUTHENTIC_GLOBAL_ADMIN = (userovoGlobal.member.global_admin && ((userovoGlobal.member.restrict || []).indexOf("#/manage/configurations") < 0));
    ...
    var WidgetsMixin = {
        computed: {
            __widgets: function() {
                var w = (userovoVue.container.dict && userovoVue.container.dict["/custom/dashboards/widget"] && userovoVue.container.dict["/custom/dashboards/widget"].data) || [];
                ...
                w = w.reduce(function(acc, component) {
                    ...
                    if (!widgetType) {
                        allowed = false;
                        userovoDashboards.factory.log("Widget type is mandatory!");
                    }

                    if (!featureName) {
                        allowed = false;
                        userovoDashboards.factory.log("Feature name is mandatory!");
                    }
        ...
        methods: {
            ...
            widgetSettingsGetter: function(widget, def) {
                ...
                if (!widget.widget_type) {
                    userovoDashboards.factory.log("Widget type is not defined");
                    return false;
                }
                ...
                if (!registrations) {
                    userovoDashboards.factory.log("Soooo, unfortunately, we don't have any widget settings for " + widget.widget_type);
                    userovoDashboards.factory.log("Possible reason is - The widget wasn't registered correctly in the UI.");
                    userovoDashboards.factory.log("Please check the widget registration. Thanks :)");
                    userovoDashboards.factory.log("Also it could be that the plugin associated with the widget is disabled.");
                ...
                if (!setting) {
                    userovoDashboards.factory.log("No setting found for the " + widget.widget_type + " widget type based on the widget getter. Please register the widget settings correctly.");
                    return def ? defaultSetting : false;
                }
            ...
            widgetSettingsPrimary: function(widget) {
                ...
                if (!setting) {
                    userovoDashboards.factory.log("No primary widget found for " + widget.widget_type + " !. Please set primary to true in the widget registration.");
                }
        ...
        methods: {
            ...
            returnDimensions: function(settings, widget) {
                ...
                if (newDimensions) {
                    ...
                    userovoDashboards.factory.log("No dimensions were found for the widget!");
                }
        ...
        methods: {
            ...
            calculateWidth: function(settings, widget, width) {
                ...
                if (!w || w < minWidth) {
                     ...
                     */
                    w = minWidth;
                    userovoDashboards.factory.log("Width should be atleast equal to " + minWidth + "! Old width = " + width + ", New width = " + w);
                }
            ...
            calculateHeight: function(settings, widget, height) {
                ...
                if (!h || h < minHeight) {
                     ...
                     */
                    h = minHeight;
                    userovoDashboards.factory.log("Height should be atleast equal to " + minHeight + "! Old height = " + height + ", New height = " + h);
                }
            ...
            isWidgetDisabled: function(widget) {
                ...
                if (widget.isPluginWidget) {
                    ...
                    disabled = userovoGlobal.plugins.map(function(p) {
    ...
    var DashboardMixin = {
        methods: {
            addDashboard: function() {
                var empty = userovoDashboards.factory.dashboards.getEmpty();
                empty.__action = "create";
                this.$store.dispatch("userovoDashboards/requests/drawerOpenStatus", true);
                this.openDrawer("dashboards", empty);
            }
        }
    };

    var DisabledWidget = userovoVue.views.BaseView.extend({
    ...
    var InvalidWidget = userovoVue.views.BaseView.extend({
    ...
    var DashboardDrawer = userovoVue.views.create({
        ...
        data: function() {
            return {
                title: "",
                saveButtonLabel: "",
                sharingAllowed: userovoGlobal.sharing_status || AUTHENTIC_GLOBAL_ADMIN,
                groupSharingAllowed: userovoGlobal.plugins.indexOf("groups") > -1 && AUTHENTIC_GLOBAL_ADMIN,
                sharedEmailEdit: [],
                sharedEmailView: [],
                sharedGroupEdit: [],
                sharedGroupView: [],
        ...
        computed: {
            constants: function() {
                ...
                var allowPublicDashboards = userovoGlobal.allow_public_dashboards !== false;
                return {
        ...
        methods: {
            onSubmit: function(doc) {
                var action = "userovoDashboards/create";
                var __action = doc.__action;

                if (__action === "edit") {
                    action = "userovoDashboards/update";
                }

                if (__action === "duplicate") {
                    action = "userovoDashboards/duplicate";
                }
            ...
            onClose: function() {
                this.$store.dispatch("userovoDashboards/requests/drawerOpenStatus", false);
            },
        },
        mounted: function() {
            if (this.groupSharingAllowed) {
    ...
    var NoWidget = userovoVue.views.BaseView.extend({
    ...
    var NoDashboard = userovoVue.views.BaseView.extend({
        template: '#dashboards-nodashboard',
        mixins: [userovoVue.mixins.hasDrawers("dashboards"), DashboardMixin],
        components: {
            "dashboards-drawer": DashboardDrawer
        }
    });

    var WidgetComponent = userovoVue.views.BaseView.extend({
        ...
        computed: {
            canUpdateGrid: function() {
                var dashboard = this.$store.getters["userovoDashboards/selected"];
    ...
    var WidgetDrawer = userovoVue.views.create({
        ...
        methods: {
            onSubmit: function(doc) {
                var action = "userovoDashboards/widgets/create";
                var __action = doc.__action;
                var self = this;
                var isEdited = __action === "edit";

                if (isEdited) {
                    action = "userovoDashboards/widgets/update";
                }

                var setting = this.widgetSettingsGetter(doc);

                if (!setting) {
                    userovoDashboards.factory.log("Well this is very strange.");
                    userovoDashboards.factory.log("On widget submit, atleast one of the widget registrations should be returned.");
                    userovoDashboards.factory.log("Kindly check your widget getter, maybe something is wrong there.");
                ...
                this.$store.dispatch(action, {id: doc._id, settings: obj}).then(function(id) {
                    if (id) {
                        if (isEdited) {
                            self.$store.dispatch("userovoDashboards/requests/isProcessing", true);
                            self.$store.dispatch("userovoDashboards/widgets/get", doc._id).then(function() {
                                self.$store.dispatch("userovoDashboards/requests/isProcessing", false);
                            });
                            UserovoHelpers.notify({
            ...
            onCopy: function(doc) {
                ...
                if (!setting) {
                    userovoDashboards.factory.log("Well this is very strange.");
                    userovoDashboards.factory.log("On widget onCopy, atleast one of the widget registrations should be returned.");
                    userovoDashboards.factory.log("Kindly check your widget getter, maybe something is wrong there.");
    ...
    var GridComponent = userovoVue.views.BaseView.extend({
        template: '#dashboards-grid',
        mixins: [userovoVue.mixins.hasDrawers("widgets"), WidgetsMixin, WidgetValidationMixin, DimensionsMixin],
        props: {
            loading: {
                type: Boolean,
                default: true
        ...
        computed: {
            allWidgets: function() {
                var allWidgets = JSON.parse(JSON.stringify(this.$store.getters["userovoDashboards/widgets/all"]));
        ...
        methods: {
            onWidgetAction: function(command, data) {
                ...
                if (!setting && command !== "delete") {
                    userovoDashboards.factory.log("Well this is very strange.");
                    userovoDashboards.factory.log("On widget action, atleast one of the widget registrations should be returned.");
                    userovoDashboards.factory.log("Kindly check your widget getter, maybe something is wrong there.");
                    return;
                }
                ...
                case "delete":
                    d.__action = "delete";
                    UserovoHelpers.confirm(this.i18nM("dashboards.delete-widget-text"), "danger", function(result) {
                        if (!result) {
                            return false;
                        }

                        self.$store.dispatch("userovoDashboards/widgets/delete", d._id).then(function(res) {
                            if (res) {
                                var node = document.getElementById(d._id);
                                self.removeGridWidget(node);
                                self.$store.dispatch("userovoDashboards/widgets/remove", d._id);
                                UserovoHelpers.notify({
            ...
            addWidget: function(widget) {
                ...
                if (!setting) {
                    userovoDashboards.factory.log("This can never happen! \
            ...
            initGrid: function() {
                ...
                this.grid.on("resizestart dragstart", function() {
                    self.$nextTick(function() {
                        self.$store.dispatch("userovoDashboards/requests/gridInteraction", true);
                        self.$store.dispatch("userovoDashboards/requests/markSanity", false);
                    });
                });
                ...
                this.grid.on("resizestop dragstop", function() {
                    ...
                    self.$nextTick(function() {
                         ...
                         */
                        self.$store.dispatch("userovoDashboards/requests/gridInteraction", false);
                    });
                });
                ...
                this.grid.on("added", function(event, element) {
                    ...
                    if (node && node.new) {
                        ...
                        self.$store.dispatch("userovoDashboards/widgets/update", {id: widgetId, settings: {position: p, size: s}}).then(function(res) {
                            if (res) {
                                self.$store.dispatch("userovoDashboards/widgets/get", widgetId).then(function() {
            ...
            updateWidgetGeography: function(widgetId, settings) {
                ...
                this.$store.dispatch("userovoDashboards/widgets/syncGeography", {_id: widgetId, settings: settings});
                this.$nextTick(() => {
                    setTimeout(function() {
                        self.$store.dispatch("userovoDashboards/widgets/update", {id: widgetId, settings: settings});
                    }, 100);
                });
            },
            /**
    ...
    var HomeComponent = userovoVue.views.BaseView.extend({
        template: "#dashboards-main",
        mixins: [userovoVue.mixins.hasDrawers("dashboards"), userovoVue.mixins.hasDrawers("widgets"), WidgetsMixin],
        components: {
        ...
        computed: {
            noSelectedDashboard: function() {
                var selected = this.$store.getters["userovoDashboards/selected"];
                return !(selected.id && selected.data);
            },
            noWidgets: function() {
                return !this.$store.getters["userovoDashboards/widgets/all"].length;
            },
            dashboard: function() {
                var selected = this.$store.getters["userovoDashboards/selected"];
                ...
                dashboard.name = userovoCommon.unescapeHtml(dashboard.name);
            ...
            isInitLoad: function() {
                var isInit = this.$store.getters["userovoDashboards/requests/isInitializing"];
                return isInit;
            },
            isRefreshing: function() {
                var isRefreshing = this.$store.getters["userovoDashboards/requests/isRefreshing"];
                return isRefreshing;
            },
            isDrawerOpen: function() {
                var isOpen = this.$store.getters["userovoDashboards/requests/drawerOpenStatus"];
                return isOpen;
            },
            isWidgetProcessing: function() {
                var isProcessing = this.$store.getters["userovoDashboards/requests/isProcessing"];
                return isProcessing;
            },
            isGridInteraction: function() {
                var isInteraction = this.$store.getters["userovoDashboards/requests/gridInteraction"];
                return isInteraction;
            },
            isRequestSane: function() {
                var isSane = this.$store.getters["userovoDashboards/requests/isSane"];
                return isSane;
            }
        },
        created: function() {
        ...
        methods: {
            refresh: function(forceRefresh) {
                ...
                if (!isRequestSane) {
                    ...
                    if (!isRefreshing && !isInitializing && !isGridInteraction && !isWidgetProcessing) {
                         ...
                         */
                        this.$store.dispatch("userovoDashboards/requests/markSanity", true);
                    }
            ...
            dateChanged: function(isRefresh) {
                var self = this;
                this.$store.dispatch("userovoDashboards/requests/isRefreshing", isRefresh);

                this.$store.dispatch("userovoDashboards/getDashboard", {id: this.dashboardId, isRefresh: isRefresh}).then(function() {
                    self.$store.dispatch("userovoDashboards/requests/isRefreshing", false);
                });
            },
            onDashboardAction: function(command, data) {
                ...
                case "delete":
                    d.__action = "delete";
                    UserovoHelpers.confirm(this.i18n("dashboards.delete-dashboard-text", d.name), "danger", function(result) {
                        if (!result) {
                            return false;
                        }

                        self.$store.dispatch("userovoDashboards/delete", d._id).then(function(res) {
                            if (res) {
                                UserovoHelpers.notify({
        ...
        beforeMount: function() {
            ...
            this.$store.dispatch("userovoDashboards/setDashboard", {id: this.dashboardId, isRefresh: false}).then(function() {
                self.$store.dispatch("userovoDashboards/requests/isInitializing", false);
            });
        },
        beforeDestroy: function() {
            this.$store.dispatch("userovoDashboards/requests/reset");
        }
    });
    ...
    var DashboardsMenu = userovoVue.views.create({
        template: CV.T('/dashboards/templates/dashboards-menu.html'),
        mixins: [userovoVue.mixins.hasDrawers("dashboards"), DashboardMixin, userovoVue.mixins.commonFormatters],
        components: {
            "dashboards-drawer": DashboardDrawer
        },
        data: function() {
        ...
        computed: {
            ...
            allDashboards: function() {
                ...
                var dashboards = (this.$store.getters["userovoDashboards/all"] || []).slice();
        ...
        methods: {
            onDashboardMenuItemClick: function(dashboard) {
                this.$store.dispatch("userovoSidebar/updateSelectedMenuItem", {menu: "dashboards", item: dashboard});
            },
            identifySelected: function() {
                var dashboards = this.$store.getters["userovoDashboards/all"];
                ...
                if (/^\/custom/.test(currLink) === false) {
                    ...
                    if (selected.menu === "dashboards") {
                         ...
                         * anything.
                         */
                        this.$store.dispatch("userovoSidebar/updateSelectedMenuItem", { menu: "dashboards", item: {} });
                    }
                 ...
                 */
                this.$store.dispatch("userovoSidebar/updateSelectedMenuItem", {menu: "dashboards", item: currMenu || {}});
            }
        },
        beforeCreate: function() {
            this.module = userovoDashboards.getVuexModule();
            CV.vuex.registerGlobally(this.module);
        },
        beforeMount: function() {
            var self = this;
            this.$store.dispatch("userovoDashboards/getAll", {just_schema: true}).then(function() {
                self.identifySelected();
            });
        }
    });

    userovoVue.container.registerData("/sidebar/menu/main", {
        name: "dashboards",
    ...
    userovoVue.container.registerMixin("/manage/export/export-features", {
        pluginName: "dashboards",
        beforeCreate: function() {
            var self = this;
            this.$store.dispatch("userovoDashboards/getAll", {just_schema: true}).then(function(res) {
                if (res) {
                    var dashboards = [];
                    ...
                    if (dashboards.length) {
                        ...
                        self.$store.dispatch("userovoConfigTransfer/addConfigurations", selectItem);
                    }
                }
            });
        }
