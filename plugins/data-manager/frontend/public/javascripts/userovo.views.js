/*global app, userovoAuth, userovoVue, CV, userovoDataManager, userovoCommon, userovoEvent, moment, userovoGlobal, UserovoHelpers, _ */
...
(function() {
    ...
    var EXTENDED_VIEWS = userovoDataManager.extended && userovoDataManager.extended.views || {};
    ...
    var ManageCategoryInput = userovoVue.views.create({
    ...
    var ManageCategory = userovoVue.views.create({
    ...
    var EventsDrawer = userovoVue.views.create({
        ...
        data: function() {
            return {
                isDrill: userovoGlobal.plugins.indexOf("drill") > -1,
                isOpened: false,
        ...
        computed: {
            ...
            categories: function() {
                var cats = this.$store.getters["userovoDataManager/categories"] || [];
                return [{ label: CV.i18n('data-manager.uncategorized'), value: null }].concat(cats.map(function(ev) {
                    return {
                        label: userovoCommon.unescapeHtml(ev.name),
                        value: ev._id
                    };
                }));
            },
            ...
            onSubmit: function(doc) {
                if (doc.isEditMode) {
                    if (!doc.status || doc.status === 'unplanned') {
                        if (!doc.is_visible) {
                            UserovoHelpers.notify({message: CV.i18n('data-manager.error.event-visibility-error'), sticky: false, type: 'error'});
                        }
                        doc.is_visible = true;
                    }
                    if (!(_.isEqual(doc, this.$refs.eventDrawer.initialEditedObject))) {
                        this.$store.dispatch('userovoDataManager/editEvent', doc);
                    }
                }
                else {
                    this.$store.dispatch('userovoDataManager/saveEvent', doc);
                }
            },
        }
    });

    var EventGroupDrawer = userovoVue.views.create({
        ...
        computed: {
            ...
            eventList: function() {
                var events = this.$store.getters["userovoDataManager/events"] || [];
                return events.map(function(ev) {
                    return {
                        label: userovoCommon.unescapeHtml(ev.name || ev.key || ev.e),
                        value: ev.key || ev.e
                    };
                });
            },
        ...
        methods: {
            ...
            onSubmit: function(doc) {
                if (doc.isEditMode) {
                    doc.app_id = userovoCommon.ACTIVE_APP_ID;
                    if (!(_.isEqual(doc, this.$refs.eventGroupDrawer.initialEditedObject))) {
                        this.$store.dispatch('userovoDataManager/editEventGroups', doc);
                    }
                }
                else {
                    doc.app_id = userovoCommon.ACTIVE_APP_ID;
                    this.$store.dispatch('userovoDataManager/saveEventGroups', doc);
                }
            },
            onCopy: function() {
            },
    ...
    var EventGroupDetailView = userovoVue.views.create({
        template: CV.T('/data-manager/templates/event-group-detail.html'),
        mixins: [
            userovoVue.mixins.hasDrawers(["eventgroup"]),
        ...
        computed: {
            eventGroup: function() {
                var self = this;
                var eventGroup = {};
                var eventGroups = this.$store.getters["userovoDataManager/eventGroups"];
        ...
        methods: {
            initialize: function() {
                this.$store.dispatch('userovoDataManager/loadEventGroups');
            },
            handleEdit: function() {
                var doc = this.eventGroup;
                doc.name = userovoCommon.unescapeHtml(doc.name);
                doc.description = userovoCommon.unescapeHtml(doc.description);
                if (Array.isArray(doc.source_events)) {
                    doc.source_events = doc.source_events.map(function(val) {
                        return userovoCommon.unescapeHtml(val);
                    });
                }
            ...
            submitDeleteForm: function() {
                this.$store.dispatch("userovoDataManager/deleteEventGroups", [this.deleteElement._id]);
                this.showDeleteDialog = false;
    ...
    var EventsDefaultTabView = userovoVue.views.create({
        ...
        data: function() {
            return {
                isDrill: userovoGlobal.plugins.indexOf("drill") > -1,
                eventsTablePersistKey: "dm_events_table_" + userovoCommon.ACTIVE_APP_ID,
        ...
        computed: {
            hasCreateRight: function() {
                return userovoAuth.validateCreate(FEATURE_NAME);
            },
            hasDeleteRight: function() {
                return userovoAuth.validateDelete(FEATURE_NAME);
            },
            isLoading: {
                get: function() {
                    return this.$store.getters["userovoDataManager/isLoading"];
                },
                cache: false
            },
            ...
            categories: function() {
                return this.$store.getters["userovoDataManager/categories"];
            },
            limits: function() {
                var eventLimit = {};
                var limits = this.$store.getters["userovoDataManager/limits"];
            ...
            categoriesMap: function() {
                return this.$store.getters["userovoDataManager/categoriesMap"];
            },
            eventTransformationMap: function() {
                if (this.isDrill) {
                    return this.$store.getters["userovoDataManager/eventTransformationMap"];
                }
                else {
                    return null;
                }
            ...
            events: function() {
                var self = this;
                var isEventCountAvailable = this.$store.getters["userovoDataManager/isEventCountAvailable"];
                var eventCount = this.$store.getters["userovoDataManager/eventCount"];
                return this.$store.getters["userovoDataManager/events"]
                    .filter(function(e) {
                        ...
                        if (!e.status || e.status === "unplanned") {
                            // var config = userovoPlugins.getConfigsData()['data-manager'] || {};
                    ...
                    .map(function(e) {
                        ...
                        if (isEventCountAvailable) {
                            ...
                            e.totalCountFormatted = userovoCommon.formatNumber(e.totalCount);
                        }
                        return e;
                    });
            },
                ...
                if (this.isDrill) {
                    return [{
                        ...
                        options: [
                            ...
                            return {value: c.name, label: userovoCommon.unescapeHtml(c.name)};
                        })),
                        default: "all",
                        action: true
                    },
        ...
        methods: {
            ...
            handleChangeCategory: function(cat, rows) {
                this.$store.dispatch("userovoDataManager/changeCategory", {
                    category: cat,
                    events: rows.map(function(ev) {
                        return userovoCommon.unescapeHtml(ev.key);
                    })
                });
            ...
            handleChangeVisibility: function(command, rows) {
                ...
                this.$store.dispatch('userovoDataManager/changeVisibility', { events: events, isVisible: isVisible }).then(function() {
                    userovoEvent.refreshEvents();
                });
                this.unpatchSelectedEvents();
            },
            handleChangeStatus: function(command, rows) {
                ...
                this.$store.dispatch('userovoDataManager/updateEventStatus', { events: events, status: command });
                this.unpatchSelectedEvents();
            },
            ...
            onSaveCategories: function() {
                ...
                if (newCatgories.length) {
                    this.$store.dispatch('userovoDataManager/saveCategories', newCatgories);
                }
                if (editedCategories.length) {
                    // var self = this;
                    this.$store
                        .dispatch('userovoDataManager/editCategories', editedCategories)
                        .then(function(res) {
                            if (res === 'Error') {
                ...
                if (this.deletedCategories && this.deletedCategories.length) {
                    this.$store.dispatch('userovoDataManager/deleteCategories', this.deletedCategories.map(function(cat) {
                        return cat._id;
                    }));
                }
            },
            onCloseCategories: function() {
                if (this.categoryDialogVisible) {
                    this.$store.dispatch('userovoDataManager/loadCategories');
            ...
            submitDeleteForm: function() {
                ...
                this.unpatchSelectedEvents();
                this.$store.dispatch('userovoDataManager/deleteEvents', events).then(function() {
                    userovoEvent.refreshEvents();
                });
                this.deleteQueue = null;
                this.showDeleteDialog = false;
    ...
    var EventsGroupsTabView = userovoVue.views.create({
        ...
        data: function() {
            return {
                eventsGroupTablePersistKey: "dm_event_groups_table_" + userovoCommon.ACTIVE_APP_ID,
                selectedFilter: 'all',
                trackedFields: ['isSelected'],
                eventGroupFilters: [
        ...
        computed: {
            ...
            eventGroups: function() {
                var self = this;
                var eventGroup = this.$store.getters["userovoDataManager/eventGroups"];
        ...
        methods: {
            ...
            handleChangeVisibility: function(command, rows) {
                ...
                this.$store.dispatch('userovoDataManager/changeEventGroupsVisibility', { events: events, isVisible: isVisible });
            ...
            handleDelete: function(rows) {
                ...
                this.$store.dispatch('userovoDataManager/deleteEventGroups', events);
            },
    ...
    var EventsView = userovoVue.views.create({
        template: CV.T('/data-manager/templates/events.html'),
        mixins: [
            userovoVue.mixins.auth(FEATURE_NAME),
            userovoVue.mixins.hasDrawers(["events", "transform", "segments", "eventgroup", "regenerate"]),
            userovoVue.container.tabsMixin({
                ...
                localTabs.push(
                    {
                        ...
                        route: "#/" + userovoCommon.ACTIVE_APP_ID + "/manage/data-manager/events/events"
                    }
                );
                localTabs.push({
                    priority: 3,
                    ...
                    route: "#/" + userovoCommon.ACTIVE_APP_ID + "/manage/data-manager/events/event-groups"
                });
            }
            return {
                isDrill: userovoGlobal.plugins.indexOf("drill") > -1,
        ...
        computed: {
            ...
            canUserCreateTransform: function() {
                return userovoAuth.validateCreate(SUB_FEATURE_TRANSFORMATIONS);
            },
        ...
        methods: {
            initialize: function() {
                this.$store.dispatch('userovoDataManager/loadEventsData');
                this.$store.dispatch('userovoDataManager/loadEventGroups');
                this.$store.dispatch('userovoDataManager/loadCategories');
                this.$store.dispatch('userovoDataManager/fetchLimits');
                if (this.isDrill) {
                    this.$store.dispatch('userovoDataManager/loadTransformations');
                    this.$store.dispatch('userovoDataManager/loadSegmentsMap');
                    this.$store.dispatch('userovoDataManager/loadValidations');
                    this.$store.dispatch('userovoDataManager/loadInternalEvents');
                    if (userovoGlobal.plugins.indexOf("views") > -1) {
                        this.$store.dispatch('userovoDataManager/loadViews');
                    }
                }
            },
            ...
            handleMetaCommands: function(event) {
                if (event === 'regnerate') {
                    ...
                    this.$store.dispatch('userovoDataManager/exportSchema');
                }
            ...
            onSaveImport: function() {
                ...
                this.$store.dispatch('userovoDataManager/importSchema', files[0])
                    .then(function() {
                        self.initialize();
                    });
            },
        ...
        mounted: function() {
            ...
            this.$root.$on('dm-open-edit-event-drawer', function(data) {
                ...
                if (self.isDrill) {
                            ...
                            .forEach(function(segment) {
                                if (segment) {
                                    try {
                                        ...
                                        sg.name = userovoCommon.unescapeHtml(segment);
                                        segments.push(sg);
                                    }
                                    catch (e) {
                                        // supress create mode
                    ...
                    data.segments = data.segments.map(function(seg) {
                        return {
                            name: userovoCommon.unescapeHtml(seg)
                        };
                    });
                }
                data.isEditMode = true;
                data.is_visible = data.is_visible === undefined ? true : data.is_visible;
                data.description = userovoCommon.unescapeHtml(data.description);
                data.e = userovoCommon.unescapeHtml(data.e);
                if (data.key) {
                    data.key = userovoCommon.unescapeHtml(data.key);
                }
                data.categoryName = userovoCommon.unescapeHtml(data.categoryName);
                data.name = userovoCommon.unescapeHtml(data.name);
                if (data.sg) {
                    Object.keys(data.sg).forEach(function(key) {
                        var decodedKey = userovoCommon.unescapeHtml(key);
                        if (data.sg[key].name) {
                            data.sg[key].name = userovoCommon.unescapeHtml(data.sg[key].name);
                        }
                        if (decodedKey !== key) {
            ...
            this.$root.$on('dm-open-edit-transform-drawer', function(doc) {
                ...
                doc.name = userovoCommon.unescapeHtml(doc.name);
                doc.transformResult = userovoCommon.unescapeHtml(doc.transformResult);
                if (doc.actionType === 'value') {
                    doc.actionType = 'change-value';
                }
                doc.isEditMode = true;
                if (doc.parentEvent) {
                    doc.parentEvent = userovoCommon.unescapeHtml(doc.parentEvent);
    ...
    var MainView = userovoVue.views.create({
        template: CV.T('/data-manager/templates/main.html'),
        mixins: [
            userovoVue.container.mixins(["/manage/data-manager"]),
            userovoVue.container.tabsMixin({
        ...
        data: function() {
            var localTabs = [];
            if (userovoAuth.validateRead(FEATURE_NAME) || userovoAuth.validateRead(SUB_FEATURE_TRANSFORMATIONS)) {
                localTabs.push({
                    priority: 1,
                    title: this.i18n('data-manager.events'),
                    name: "events",
                    component: EventsView,
                    route: "#/" + userovoCommon.ACTIVE_APP_ID + "/manage/data-manager/events",
                });
            }
            return {
    ...
    var EventDetailView = userovoVue.views.create({
        template: CV.T('/data-manager/templates/event-detail.html'),
        mixins: [
            userovoVue.mixins.hasDrawers(["events", "segments"]),
            userovoVue.mixins.auth(FEATURE_NAME),
            userovoVue.mixins.commonFormatters,
        ],
        components: {
        ...
        computed: {
            ...
            segments: function() {
                var self = this;
                var segments = [];
                var segmentsMap = this.$store.getters["userovoDataManager/segmentsMap"];
                if (segmentsMap) {
            ...
            isLoading: function() {
                return this.$store.getters["userovoDataManager/segmentsMapLoading"];
            },
            categoriesMap: function() {
                return this.$store.getters["userovoDataManager/categoriesMap"];
            },
        ...
        methods: {
            ...
            submitDeleteForm: function() {
                this.$store.dispatch('userovoDataManager/deleteEvents', [this.deleteElement]).then(function() {
                    userovoEvent.refreshEvents();
                });
                this.showDeleteDialog = false;
    ...
    app.route("/manage/data-manager/events/events/*query", 'data-manager-event-detail', function(query) {
        ...
        var queryUrlParameter = query && UserovoHelpers.isJSON(query) ? JSON.parse(query) : query;
        detailView.params = {
