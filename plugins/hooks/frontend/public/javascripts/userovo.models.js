...
(function(hooksPlugin, jQuery) {
    ...
    var userovoCommon = window.userovoCommon;
    ...
    hooksPlugin.getVuexModule = function() {
        ...
        var actions = {
                ...
                return CV.$.ajax({
                    ...
                    success: function(data) {
                        if (data.hooksList && data.hooksList.length === 1) {
                            ...
                            record._canUpdate = userovoAuth.validateUpdate(FEATURE_NAME, userovoGlobal.member, record.apps[0]),
                            record._canDelete = userovoAuth.validateDelete(FEATURE_NAME, userovoGlobal.member, record.apps[0]),
            ...
            refresh: function(context) {
                context.dispatch("userovoHooks/table/fetchAll", null, {root: true});
            },
            saveHook: function(context, record) {
                ...
                return new Promise(function(resolve, reject) {
                    CV.$.ajax({
                        type: 'POST',
                        url: userovoCommon.API_PARTS.data.w + '/hook/save?' + 'app_id=' + record.apps[0],
                        data: {
                        ...
                        success: function() {
                            context.dispatch('userovoHooks/table/fetchAll', null, {root: true});
                            context.dispatch('userovoHooks/initializeDetail', record._id, {root: true});
                            resolve();
                        },
                        error: function(err) {
                            ...
                            UserovoHelpers.notify({type: 'error', message: msg});
                            reject(err);
                        },
                    });
                });
            ...
            deleteHook: function(context, id) {
                return CV.$.ajax({
                    type: "POST",
                    url: userovoCommon.API_PARTS.data.w + "/hook/delete" + "?app_id=" + userovoCommon.ACTIVE_APP_ID,
                    data: {
                        "hookID": id
                    },
                    dataType: "json",
            ...
            testHook: function(context, hookConfig) {
                ...
                return CV.$.ajax({
                    type: "get",
                    url: userovoCommon.API_PARTS.data.w + "/hook/test" + "?app_id=" + userovoCommon.ACTIVE_APP_ID,
                    data: {
        ...
        var tableResource = userovoVue.vuex.Module("table", {
            ...
            actions: {
                updateStatus: function(context, status) {
                    return CV.$.ajax({
                        type: "post",
                        url: userovoCommon.API_PARTS.data.w + "/hook/status" + "?app_id=" + userovoCommon.ACTIVE_APP_ID,
                        data: {
                ...
                fetchAll: function(context) {
                    ...
                    return CV.$.ajax({
                        type: "GET",
                        url: userovoCommon.API_PARTS.data.r + "/hook/list" + "?app_id=" + userovoCommon.ACTIVE_APP_ID,
                        ...
                        for (var i = 0; i < hookList.length; i++) {
                            ...
                            if (hookList[i].apps) {
                                appNameList = _.map(hookList[i].apps, function(appID) {
                                    return userovoGlobal.apps[appID] && userovoGlobal.apps[appID].name;
                                });
                            }
                            ...
                            tableData.push({
                                ...
                                _canUpdate: userovoAuth.validateUpdate(FEATURE_NAME, userovoGlobal.member, hookList[i].apps[0]),
                                _canDelete: userovoAuth.validateDelete(FEATURE_NAME, userovoGlobal.member, hookList[i].apps[0]),
                            });
                        }
        ...
        return userovoVue.vuex.Module("userovoHooks", {
