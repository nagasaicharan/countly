/*global
   CV, _,  userovoVue, $, userovoCommon, jQuery,userovoGlobal, app, hooksPlugin, moment, UserovoHelpers,  userovoEvent, userovoAuth
 */
(function() {
    ...
    var TableView = userovoVue.views.BaseView.extend({
        ...
        computed: {
            tableRows: function() {
                var rows = this.$store.getters["userovoHooks/table/all"];
            ...
            initialized: function() {
                var result = this.$store.getters["userovoHooks/table/getInitialized"];
                return result;
            },
        ...
        data: function() {
            ...
            for (var id in userovoGlobal.apps) {
                appsSelectorOption.push({label: userovoGlobal.apps[id].name, value: id, image: "background-image:url(" + userovoGlobal.apps[id].image + ")"});
            }
            ...
            return {
                ...
                tablePersistKey: "hooks_table_" + userovoCommon.ACTIVE_APP_ID,
        ...
        methods: {
            handleHookEditCommand: function(command, scope) {
                if (command === "edit-comment") {
                    ...
                    this.$store.dispatch("userovoHooks/resetTestResult");
                    ...
                    UserovoHelpers.confirm(deleteMessage, "red", function(result) {
                        ...
                        self.$store.dispatch("userovoHooks/deleteHook", self.deleteElement._id);
            ...
            updateStatus: function(scope) {
                ...
                var self = this;
                this.$store.dispatch("userovoHooks/table/updateStatus", status).then(function() {
                    return self.$store.dispatch("userovoHooks/table/fetchAll");
                });
            },
            refresh: function() {
            },
    ...
    var HTTPEffect = userovoVue.views.BaseView.extend({
    ...
    var EmailEffect = userovoVue.views.BaseView.extend({
    ...
    var CustomCodeEffect = userovoVue.views.BaseView.extend({
    ...
    var EffectViews = userovoVue.views.BaseView.extend({
    ...
    var IncomingDataTrigger = userovoVue.views.BaseView.extend({
        ...
        methods: {
            ...
            getEventOptions: function() {
                ...
                userovoEvent.getEventsForApps(apps, function(events) {
                    events = events.map(function(e) {
                        e.label = e.name; return e;
                    });
                    $.ajax({
                        type: "GET",
                        url: userovoCommon.API_URL + '/o/internal-events',
                        data: {
                            app_id: apps[0],
                        },
    ...
    var InternalEventTrigger = userovoVue.views.BaseView.extend({
        ...
        methods: {
            getCohortOptioins: function() {
                var apps = [this.$props.app];
                var data = {
                    "app_id": apps[0], //userovoCommon.ACTIVE_APP_ID,
                    "method": "get_cohorts",
            ...
            getHookOptions: function() {
                ...
                $.ajax({
                    type: "GET",
                    url: userovoCommon.API_PARTS.data.r + '/hook/list',
            ...
            getAlertOptions: function() {
                ...
                $.ajax({
                    type: "GET",
                    url: userovoCommon.API_PARTS.data.r + '/alert/list',
                    data: {app_id: this.$props.app},
                    dataType: "json",
                    success: function(data) {
    ...
    var ScheduledTrigger = userovoVue.views.BaseView.extend({
        template: '#hooks-ScheduledTrigger',
        data: function() {
            var zones = [];
            /* eslint-disable  no-loop-func */
            for (var country in userovoGlobal.timezones) {
                var c = userovoGlobal.timezones[country];
                c && c.z && c.z.forEach(function(item) {
                    for (var zone in item) {
                        zones.push({value: item[zone], label: userovoGlobal.timezones[country].n + ' ' + zone, image: "background-image:url(" + userovoGlobal.path + "/images/flags/" + country.toLowerCase() + ".png)"});
                    }
                });
            }
    ...
    var APIEndPointTrigger = userovoVue.views.BaseView.extend({
        ...
        methods: {
            copyURL: function() {
                ...
                UserovoHelpers.notify({clearAll: true, type: 'green', title: jQuery.i18n.map['hooks.copy-notify-title'], message: jQuery.i18n.map['hooks.copy-notify-message'], info: "", delay: 2000, sticky: false});
            }
        }
    });

    var TriggerViews = userovoVue.views.BaseView.extend({
    ...
    var HookDrawer = userovoVue.views.BaseView.extend({
        ...
        data: function() {
            ...
            for (var id in userovoGlobal.apps) {
                var item = {label: userovoGlobal.apps[id].name, value: id, image: "background-image:url(" + userovoGlobal.apps[id].image + ")"};
                if (userovoAuth.validateCreate(FEATURE_NAME, userovoGlobal.member, id)) {
                    appsSelectorOption.push(item);
                }
                if (userovoAuth.validateUpdate(FEATURE_NAME, userovoGlobal.member, id)) {
        ...
        computed: {
            testResult: function() {
                var testResult = this.$store.getters["userovoHooks/testResult"];
        ...
        methods: {
            onSubmit: function(doc, callback) {
                this.$store.dispatch("userovoHooks/saveHook", doc)
                    .then(function() {
                        callback();
                    })
                    .catch(function(err) {
            ...
            testHook: function() {
                ...
                this.$store.dispatch("userovoHooks/testHook", hookData);
            },
        }
    });

    var HooksHomeViewComponent = userovoVue.views.BaseView.extend({
        template: "#hooks-home",
        mixins: [
        ...
        beforeCreate: function() {
            this.$store.dispatch("userovoHooks/initialize");
        },
        methods: {
            createHook: function() {
                this.$store.dispatch("userovoHooks/resetTestResult");
    ...
    var DetailErrorsTableView = userovoVue.views.BaseView.extend({
        template: '#hooks-detail-errors-table-view',
        computed: {
            tableRows: function() {
                var hookDetail = this.$store.getters["userovoHooks/hookDetail"];
            ...
            detailLogsInitialized: function() {
                var result = this.$store.getters["userovoHooks/getDetailLogsInitialized"];
                return result;
            },
            hookDetail: function() {
                var hookDetail = this.$store.getters["userovoHooks/hookDetail"];
        ...
        methods: {
            refresh: function() {
            // this.$store.dispatch("userovoHooks/table/fetchAll");
            },
    ...
    var HooksDetailComponent = userovoVue.views.BaseView.extend({
        template: "#hooks-detail-view",
        mixins: [
            userovoVue.mixins.hasDrawers("detail"),
            userovoVue.mixins.auth(FEATURE_NAME),
        ],
        components: {
        ...
        computed: {
            ...
            detailLogsInitialized: function() {
                return this.$store.getters["userovoHooks/getDetailLogsInitialized"];
            }
        },
        methods: {
            handleHookEditCommand: function(command, scope) {
        ...
        beforeCreate: function() {
            this.$store.dispatch("userovoHooks/initializeDetail", this.$route.params.id, {root: true});
        }
    });


    var hooksView = new userovoVue.views.BackboneWrapper({
    ...
    var hooksDetailView = new userovoVue.views.BackboneWrapper({
