/*global moment, userovoVue, app, userovoLogger, userovoCommon, CV */
(function() {
    ...
    var ReadableDateComponent = userovoVue.components.BaseComponent.extend({
        props: ['timestamp'],
        computed: {
            date: function() {
    ...
    var DetailsComponent = userovoVue.components.BaseComponent.extend({
    ...
    var InfoComponent = userovoVue.components.BaseComponent.extend({
        props: ['info', 'filter'],
        computed: {
            logInfo: function() {
                if (this.filter === 'all') {
    ...
    var LoggerView = userovoVue.views.BaseView.extend({
        ...
        data: function() {
            return {
                ...
                collectionInfo: '',
                tablePersistKey: 'requestLogsTable_' + userovoCommon.ACTIVE_APP_ID,
                defaultFilters: [{
                    value: 'all',
        ...
        methods: {
            formatExportFunction: function() {
                    ...
                    try {
                        ...
                        Object.keys(queryObject).forEach(key => {
                            if (typeof queryObject[key] === 'string') {
                                ...
                                queryObject[key] = userovoCommon.unescapeHtml(queryObject[key]);
                            }
                                ...
                                Object.keys(queryObject[key]).forEach(nestedKey => {
                                    if (typeof queryObject[key][nestedKey] === 'string') {
                                        queryObject[key][nestedKey] = userovoCommon.unescapeHtml(queryObject[key][nestedKey]);
                                    }
                                });
                            }
                        });
                ...
                for (var i = 0; i < tableData.length; i++) {
                    var item = {};
                    item[CV.i18n('logger.requests').toUpperCase()] = userovoCommon.formatTimeAgoText(tableData[i].reqts).text;
        ...
        mounted: function() {
            ...
            userovoLogger.getCollectionInfo()
                .then(function(info) {
    ...
    var logger = new userovoVue.views.BackboneWrapper({
