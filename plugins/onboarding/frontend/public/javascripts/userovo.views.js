/*globals $,store,userovoGlobal,_,userovoVersionHistoryManager,T*/
...
function whatsNewPopup() {
     ...
     */
    if (userovoVersionHistoryManager) {
        $.when(userovoVersionHistoryManager.initialize()).then(function() {

            var versionData = _.sortBy(userovoVersionHistoryManager.getData(), "updated");
            if (versionData && versionData.length) {
                ...
                if (currentVersionData && currentVersionData.version && currentVersionData.version.split) {
                    ...
                    // If Drill is present it should be a Userovo Enterprise instance
                    var isEE = (userovoGlobal.plugins.indexOf("drill") !== -1);
                    ...
                    var localStorageData = store.get("userovo_onboard_whatsnew");
                    ...
                    if (userovoGlobal &&
                        ...
                        userovoGlobal.member.created_at &&
                        currentVersionData.updated < (userovoGlobal.member.created_at * 1000)
                    ) {
                    ...
                    // installed Userovo so we won't show a what's new popup
                    ...
                    if (!displayParams.disabled && !(userovoGlobal && userovoGlobal.ssr)) {
                        ...
                        if (localStorageData && localStorageData.version && localStorageData.seen && localStorageData.version.indexOf(currentVersion) === 0) {
                            ...
                            store.set("userovo_onboard_whatsnew", {
                        ...
                        $.when($.get(userovoGlobal.path + '/onboarding/data/versions/' + currentVersion + '/features.json', function(featuresJSON) {
