/*global userovoAuth, _,$,userovoPlugins,jQuery,userovoGlobal,app,userovoCommon,UserovoHelpers,userovoVue,CV */

(function() {
    var PluginsView = userovoVue.views.create({
        ...
        beforeCreate: function() {
            var self = this;
            return $.when(userovoPlugins.initialize()).then(
                function() {
                    try {
                        self.pluginsData = JSON.parse(JSON.stringify(userovoPlugins.getData()));
                    }
                    catch (ex) {
                        self.pluginsData = [];
                    }
        ...
        methods: {
            refresh: function() {
                ...
                return $.when(userovoPlugins.initialize()).then(
                    function() {
                        try {
                            var plugins = JSON.parse(JSON.stringify(userovoPlugins.getData()));
            ...
            formatRow: function(row) {
                ...
                row.deps = Object.keys(row.dependents).map(function(item) {
                    return userovoPlugins.getTitle(item);
                }).join(", ");
            },
            ...
            checkProcess: function() {
                ...
                $.ajax({
                    type: "GET",
                    url: userovoCommon.API_URL + "/o/plugins-check?app_id=" + userovoCommon.ACTIVE_APP_ID,
            ...
            togglePlugin: function(plugins) {
                ...
                userovoPlugins.toggle(plugins, function(res) {
            ...
            showPluginProcessMessage: function(title, message, info, delay, sticky, type, reload) {
                UserovoHelpers.notify({clearAll: true, type: type, title: title, message: message, info: info, delay: delay, sticky: sticky});
                if (reload) {
                    setTimeout(function() {
            ...
            onToggle: function(scope, row) {
                ...
                var enabledDescendants = _.intersection(userovoPlugins.getRelativePlugins(plugin, "down"), plugins),
                    disabledAncestors = _.difference(userovoPlugins.getRelativePlugins(plugin, "up"), plugins, ["___CLY_ROOT___"]);
                ...
                if (row.enabled && enabledDescendants.length > 0) {
                    ...
                    this.dialog = {
                        ...
                        text: CV.i18n("plugins.disable-descendants", userovoPlugins.getTitle(plugin), enabledDescendants.map(function(item) {
                            return userovoPlugins.getTitle(item);
                        }).join(", "))
                    };
                }
                    ...
                    this.dialog = {
                        ...
                        cancelButtonLabel: CV.i18n('common.no-dont-continue'),
                        title: CV.i18n('plugins.indirect-status-change'),
                        text: CV.i18n("plugins.enable-ancestors", userovoPlugins.getTitle(plugin), disabledAncestors.map(function(item) {
                            return userovoPlugins.getTitle(item);
                        }).join(", "))
                    };
                }
    ...
    var ConfigurationsView = userovoVue.views.create({
        ...
        computed: {
            selectedConfigSearchBar: {
                ...
                set: function(value) {
                    ...
                    this.diff_ = {};
                    try {
                        this.configsData = JSON.parse(JSON.stringify(userovoPlugins.getConfigsData()));
                    }
                    catch (ex) {
                        this.configsData = {};
                    }
        ...
        beforeCreate: function() {
            ...
            return $.when(userovoPlugins.initializeConfigs())
                .then(function() {
                    try {
                        self.configsData = JSON.parse(JSON.stringify(userovoPlugins.getConfigsData()));
                    ...
                    delete self.configsData.frontend.userovo_tracking;
                    ...
                    for (var key in self.configsData) {
                        if (self.coreDefaults.indexOf(key) === -1 && userovoGlobal.plugins.indexOf(key) !== -1) {
                            plugins.push(key);
                        }
        ...
        methods: {
            ...
            onChange: function(key, value, config) {
                var configsData = userovoPlugins.getConfigsData();
            ...
            getLabelName: function(id, ns) {
                ...
                if (this.coreDefaults.indexOf(ns) === -1 && ns !== "feedback" && userovoGlobal.plugins.indexOf(ns) === -1) {
                    return null;
                }
            ...
            getWarningTags(configGroup, key) {
                var warnings = userovoPlugins.getConfigWarnings(configGroup, key);
                var tooltipColors = userovoPlugins.getTooltipColors();
                return warnings.map(function(warning) {
                    return {
                        ...
                        label: userovoPlugins.getTooltipLabel(warning.type)
                    };
                });
            },
            checkIfOverwritten: function(id, ns) {
                ns = ns || this.selectedConfig;
                var configsData = userovoPlugins.getConfigsData();
                var plugs = userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].plugins;
                ...
                //check if value is overwritten on user level
                    var sets = userovoGlobal.member.settings;
            ...
            save: function() {
                ...
                userovoPlugins.updateConfigs(changes, function(err) {
                    if (err) {
                        UserovoHelpers.notify({
            ...
            onEnterSearch: function() {
                ...
                if (self.searchQuery && self.searchQuery !== "") {
                    ...
                    for (var config in self.predefinedStructure) {
                        if (config.toLowerCase().includes(self.searchQuery) && UserovoHelpers.isPluginEnabled(config.toLowerCase())) {
                            ...
                            self.predefinedStructure[config].groups.map(function(group) {
                                if (UserovoHelpers.isPluginEnabled(config.toLowerCase())) {
    ...
    var AccountView = userovoVue.views.create({
        ...
        data: function() {
            return {
                ...
                userData: userovoGlobal.member,
                userConfigs: {},
                avatar: this.setDefaultAvatar(userovoGlobal.member.member_image),
                initials: this.updateInitials(userovoGlobal.member.full_name),
                predefinedLabels: app.configurationsView.predefinedLabels,
                predefinedInputs: app.configurationsView.predefinedInputs,
                selectedConfig: "frontend",
                security: userovoGlobal.security
            };
        },
        beforeCreate: function() {
            var self = this;
            return $.when(userovoPlugins.initializeUserConfigs())
                .then(function() {
                    try {
                        self.userConfigs = JSON.parse(JSON.stringify(userovoPlugins.getUserConfigsData()));
                    }
                    catch (ex) {
                        self.userConfigs = {};
                    }
        ...
        methods: {
            onChange: function(id, key, value) {
                ...
                var configsData = userovoPlugins.getUserConfigsData();
            ...
            passwordDialog: function() {
                ...
                if (old_pwd.length > 0 && new_pwd.length > 0 && re_new_pwd.length > 0) {
                    var data = {
                        ...
                        full_name: userovoGlobal.member.full_name,
            ...
            submitDeleteDialog: function() {
                ...
                if (pv === "") {
                    ...
                    UserovoHelpers.notify(msg1);
                }
                else {
                    userovoPlugins.deleteAccount({password: pv}, function(err, msg) {
                        if (msg === true || msg === 'true') {
                            ...
                            UserovoHelpers.notify({title: jQuery.i18n.map["common.error"], message: jQuery.i18n.map["user-settings." + msg], sticky: true, clearAll: true, type: "error"});
                        }
                        else if (err === true) {
                            var msg2 = {title: jQuery.i18n.map["common.error"], message: msg, sticky: true, clearAll: true, type: "error"};
                            UserovoHelpers.notify(msg2);
                        }
                    });
                }
            },
                    ...
                    success: function(result) {
                        if (result === "username-exists") {
                            ...
                            if (!isNaN(parseInt(result))) {
                                userovoGlobal.member.password_changed = parseInt(result);
                            }
                            ...
                            userovoGlobal.member.full_name = doc.full_name;
                            userovoGlobal.member.username = doc.username;
                            userovoGlobal.member.api_key = doc.api_key;
                        }
                        if (Object.keys(self.changes).length) {
                            userovoPlugins.updateUserConfigs(self.changes, function(err) {
                                if (err) {
                                    UserovoHelpers.notify({
                                        title: jQuery.i18n.map["configs.not-saved"],
                                        message: jQuery.i18n.map["configs.not-changed"],
            ...
            handleAvatarSuccess: function(url) {
                userovoGlobal.member.member_image = url;
                this.avatar = this.setDefaultAvatar(userovoGlobal.member.member_image);
            ...
            deleteAvatar: function() {
                ...
                $.ajax({
                    type: "POST",
                    url: userovoCommon.API_URL + "/user/settings",
                    data: {
                        username: userovoGlobal.member.username,
                        api_key: userovoGlobal.member.api_key,
                        member_image: "delete",
                    ...
                    success: function() {
                        userovoGlobal.member.member_image = "";
                        self.avatar = self.setDefaultAvatar(userovoGlobal.member.member_image);
                        self.updateGlobalObject();
                    },
                    error: function() {
                        UserovoHelpers.notify({ type: "error", message: jQuery.i18n.map['configs.delete_avatar_failed']});
                    }
                });
            },
            setDefaultAvatar: function(image) {
                if (image) {
                    ...
                    var defaultAvatarSelector = (userovoGlobal.member.created_at || Date.now()) % 10 * -100;
    ...
    app.configurationsView.registerInput("api.send_test_email", {
        ...
        click: function() {
            ...
            $.ajax({
                type: "GET",
                url: userovoCommon.API_URL + "/o/email_test",
                data: {},
                success: function() {
                    self.attrs.disabled = false;
                    UserovoHelpers.notify({ type: "ok", message: jQuery.i18n.map['configs.help.api-send_test_email_delivered']});

                },
                error: function() {
                    self.attrs.disabled = false;
                    UserovoHelpers.notify({ type: "error", message: jQuery.i18n.map['configs.help.api-send_test_email_failed']});
                }
            });
        }
    });
    ...
    for (var a in userovoGlobal.apps) {
        appList.push({value: userovoGlobal.apps[a].key, label: userovoGlobal.apps[a].name});
    }
    ...
    if (userovoAuth.validateGlobalAdmin()) {
        ...
        if (userovoGlobal.plugins.includes("logger")) {
        ...
        userovoPlugins.initializeConfigs().always(function() {
            var pluginsData = userovoPlugins.getConfigsData();
            ...
            app.configurationsView.registerInput("frontend.theme", {input: "el-select", attrs: {}, list: userovoPlugins.getThemeList()});
        });
    }
    ...
    if (userovoAuth.validateGlobalAdmin()) {
        if (!userovoGlobal.myuserovo && userovoGlobal.plugins.indexOf('my-userovo') === -1) {
    ...
    if (userovoAuth.validateGlobalAdmin()) {
        ...
        var isGlobalDomainHasValue = userovoGlobal.domain === "" || typeof userovoGlobal.domain === "undefined" ? false : true;
        if (!isCurrentHostnameIP && !isGlobalDomainHasValue) {
            userovoPlugins.updateConfigs({"api": {"domain": window.location.protocol + "//" + window.location.hostname}}, function(err) {
                if (err) {
                    // throw err
                }
            });
