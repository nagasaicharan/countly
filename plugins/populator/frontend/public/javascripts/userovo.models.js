/*global chance, UserovoHelpers, userovoAuth, userovoGlobal, userovoCommon, userovoCohorts, userovoFunnel, $, app, moment, CV*/
(function(userovoPopulator) {
    var metric_props = {
    ...
    var jsErrors = [
        ...
        },
        {
            error: "Error: SyntaxError\n" +
                "    Hello Userovo (at bundle.js:1:138)\n" +
        ...
        },
        {
            ...
            '    "Userovo".reverse is not a function\n' +
    ...
    function getPageTemplates(populatorType) {
        return [
            "/populator/" + userovoCommon.ACTIVE_APP_KEY + "/demo-" + populatorType + ".html",
            "/populator/" + userovoCommon.ACTIVE_APP_KEY + "/demo-" + populatorType + "-1.html",
            "/populator/" + userovoCommon.ACTIVE_APP_KEY + "/demo-" + populatorType + "-2.html"
        ];
    }
    ...
    function getUserProperties(templateUp) {
        ...
        if (userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID] && userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].type === "web" && (Math.random() > 0.5)) {
    ...
    function User() {
        ...
        this.getUserFromTemplate = function(templateUp, index = 0) {
            ...
            if (userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID] && userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].type === "web") {
                this.platform = this.getProp("_os_web");
            }
            else if (userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID] && userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].type === "desktop") {
            ...
            if (userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID] && userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].type && metric_props[userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].type]) {
                m_props = metric_props[userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].type];
            }
            for (var mPropsIndex = 0; mPropsIndex < m_props.length; mPropsIndex++) {
                if (m_props[mPropsIndex] !== "_os") {
                    //handle specific cases
                    if (m_props[mPropsIndex] === "_store" && userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID] && userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].type === "web") {
        ...
        this.getError = function() {
            var errors = [];
            if (userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].type === "web") {
            ...
            else if (userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].type === "mobile") {
                errors = [{ // android
        ...
        this.getEvent = function(id, eventTemplate, ts, isRandom) {
            ...
            if (eventTemplate && eventTemplate.segmentations && eventTemplate.segmentations.length) {
                ...
                event.segmentation = {};
                var populatorType = userovoPopulator.getSelectedTemplate().substr(7).toLowerCase();
                Object.keys(viewSegments).forEach(function(key) {
                    var values = [];
                    if (userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].type === "web" && key === "name") {
        ...
        this.getFeedbackEvents = function() {
            ...
            if (userovoGlobal.plugins.indexOf("surveys") !== -1 && userovoAuth.validateCreate("surveys")) {
        ...
        this.reportConversion = function(uid, campaingId, deviceId) {
            $.ajax({
                type: "GET",
                url: userovoCommon.API_URL + "/i",
                data: {
                    campaign_id: uid,
                    campaign_user: campaingId,
                    app_key: userovoCommon.ACTIVE_APP_KEY,
                    device_id: deviceId,
        ...
        this.saveEnvironment = function(environmentUserList, setEnviromentInformationOnce) {
            ...
            return new Promise((resolve, reject) => {
                $.ajax({
                    type: "POST",
                    url: userovoCommon.API_URL + "/i/populator/environment/save",
                    data: data,
                    success: function() {
                        resolve(true);
                    },
    ...
    function flushAllRequests(req) {
        return new Promise((resolve, reject) => {
            if (generating) {
                $.ajax({
                    type: "POST",
                    url: userovoCommon.API_URL + "/i/bulk",
                    data: {
                        app_key: userovoCommon.ACTIVE_APP_KEY,
    ...
    function createCampaign(id, name, cost, type, callback) {
        $.ajax({
            type: "GET",
            url: userovoCommon.API_URL + "/i/campaign/create",
            data: {
                args: JSON.stringify({
                    "_id": id + userovoCommon.ACTIVE_APP_ID,
                    "name": name,
                    "link": "http://count.ly",
                    "cost": cost,
                    "costtype": type,
    ...
    function createMessage(data, callback) {
        ...
        if (data.triggers) {
            $.ajax({
                type: "POST",
                url: userovoCommon.API_URL + "/i/push/message/create",
                data: data,
            ...
            $.ajax({
                type: "POST",
                url: userovoCommon.API_URL + "/i/pushes/create",
                data: {
    ...
    function addParameter(parameter_key, description, default_value, callback) {
        ...
        return $.ajax({
            type: "GET",
            url: userovoCommon.API_URL + "/i/remote-config/add-parameter",
            data: {
    ...
    function addExperiment(name, callback) {
        ...
        return $.ajax({
            type: "GET",
            url: userovoCommon.API_PARTS.data.w + "/ab-testing/add-experiment",
            data: {
                app_id: userovoCommon.ACTIVE_APP_ID,
    ...
    function createFeedbackWidget(popup_header_text, popup_comment_callout, popup_email_callout, popup_button_callout, popup_thanks_message, trigger_position, trigger_bg_color, trigger_font_color, trigger_button_text, target_devices, target_pages, target_page, is_active, hide_sticker, callback) {
        return $.ajax({
            type: "GET",
            url: userovoCommon.API_URL + "/i/feedback/widgets/create",
            data: {
    ...
    function createNPSWidget(name, followUpType, mainQuestion, followUpPromoter, followUpPassive, followUpDetractor, followUpAll, thanks, style, show, color, callback) {
        return $.ajax({
            type: "GET",
            url: userovoCommon.API_URL + "/i/surveys/nps/create",
            data: {
                status: true,
                name: name,
    ...
    function createSurveyWidget(name, questions, thanks, position, show, color, logo, exitPolicy, callback) {
        return $.ajax({
            type: "GET",
            url: userovoCommon.API_URL + "/i/surveys/survey/create",
            data: {
                status: true,
                name: name,
    ...
    function generateWidgets(done) {
        ...
        function generateRatingWidgets(callback) {
            if (userovoGlobal.plugins.indexOf("star-rating") !== -1 && userovoAuth.validateCreate("star-rating") && _featuresToPopulate.includes("star-rating")) {
                createFeedbackWidget("What's your opinion about this page?", "Add comment", "Contact me by e-mail", "Send feedback", "Thanks for feedback!", "mleft", "#fff", "#ddd", "Feedback", {phone: true, tablet: false, desktop: true}, ["/"], "selected", true, false, function() {
                    createFeedbackWidget("Leave us a feedback", "Add comment", "Contact me by e-mail", "Send feedback", "Thanks!", "mleft", "#fff", "#ddd", "Feedback", {phone: true, tablet: false, desktop: false}, ["/"], "selected", true, false, function() {
                        createFeedbackWidget("Did you like this web page?", "Add comment", "Contact me by e-mail", "Send feedback", "Thanks!", "bright", "#fff", "#ddd", "Feedback", {phone: true, tablet: false, desktop: false}, ["/"], "selected", true, false, function() {
                            $.ajax({
                                type: "GET",
                                url: userovoCommon.API_URL + "/o/feedback/widgets",
                                data: {
                                    app_id: userovoCommon.ACTIVE_APP_ID
                                },
                                success: function(json) {
        ...
        function generateNPSWidgets(callback) {
            if (userovoGlobal.plugins.indexOf("surveys") !== -1 && userovoAuth.validateCreate("surveys") && _featuresToPopulate.includes("surveys")) {
        ...
        function generateSurveyWidgets1(callback) {
            createSurveyWidget("Customer support example", [
                ...
                $.ajax({
                    type: "GET",
                    url: userovoCommon.API_URL + "/o/surveys/survey/widgets",
                    data: {
                        app_id: userovoCommon.ACTIVE_APP_ID
                    },
                    success: function(json) {
    ...
    function generateAbTests(callback) {
        addParameter(abExampleName, "Test Pricing", "5000/month", function() {
            addExperiment(abExampleName, function(json) {
                $.ajax({
                    type: "GET",
                    url: userovoCommon.API_PARTS.data.w + "/ab-testing/start-experiment",
                    data: {
                        app_id: userovoCommon.ACTIVE_APP_ID,
                        "experiment_id": json
                    },
    ...
    function clickCampaign(name) {
        ...
        $.ajax({
            type: "GET",
            url: userovoCommon.API_URL + "/i/campaign/click/" + name + userovoCommon.ACTIVE_APP_ID,
    ...
    function generateCampaigns(callback) {
        if (!UserovoHelpers.isPluginEnabled("attribution") || typeof userovoAttribution === "undefined") {
            callback();
            return;
        }
    ...
    //Public Methods
    userovoPopulator.setStartTime = function(time) {
        startTs = time;
    };
    userovoPopulator.getStartTime = function() {
        return startTs;
    };
    userovoPopulator.setEndTime = function(time) {
        endTs = time;
    };
    userovoPopulator.getEndTime = function() {
        return endTs;
    };
    userovoPopulator.getUserAmount = function() {
        return userAmount;
    };

    userovoPopulator.generateUsers = function(run, template, environment) {
        ...
        async function getUsers() {
            ...
            async function createAndProcessUsers() {
                ...
                if (environment.length < template.uniqueUserCount) { // if environment users are less than template user count(in case of stop generate or error), complete the rest with new users in next run
                    UserovoHelpers.notify({type: 'warning', message: CV.i18n("populator.warning-environment-users", environment.length, template.uniqueUserCount, template.uniqueUserCount - environment.length), sticky: true});
        ...
        **/
        ...
        if (environment && environment.length) {
            ...
            generateWidgets(function() {
                generateCampaigns(async function() {
                    ...
                    if (userovoGlobal.plugins.indexOf("ab-testing") !== -1 && userovoAuth.validateCreate("ab-testing") && _featuresToPopulate.includes("ab-testing")) {
        ...
        if (userovoGlobal.plugins.indexOf("systemlogs") !== -1) {
            let data = {
                app_id: userovoCommon.ACTIVE_APP_ID,
            ...
            $.ajax({
                type: "POST",
                url: userovoCommon.API_URL + "/i/systemlogs",
                data: {
    ...
    userovoPopulator.stopGenerating = function(ensureJobs, callback) {
        stopCallback = callback;
        generating = false;

        if (ensureJobs) {
            userovoPopulator.ensureJobs();
        }
        if (stopCallback) {
            stopCallback(true);
        }
    ...
    userovoPopulator.isGenerating = function() {
        return generating;
    };

    userovoPopulator.ensureJobs = function() {
        messages.forEach(function(m) {
            m.apps = [userovoCommon.ACTIVE_APP_ID];
        });

        var template = this.currentTemplate || {};

        if (UserovoHelpers.isPluginEnabled("cohorts") && typeof userovoCohorts !== "undefined" && userovoAuth.validateCreate('cohorts') && _featuresToPopulate.includes("cohorts")) {
        ...
        if (UserovoHelpers.isPluginEnabled("funnels") && typeof userovoFunnel !== "undefined" && userovoAuth.validateCreate('funnels') && _featuresToPopulate.includes("funnels")) {

            let pages = userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].type === "mobile" ? viewSegments.name : getPageTemplates(userovoPopulator.getSelectedTemplate().substr(7).toLowerCase());
            let page1 = pages[getRandomInt(0, pages.length - 1)];
            let page2 = pages[getRandomInt(0, pages.length - 1)];

            userovoFunnel.createFunnel({
        ...
        if (userovoGlobal.plugins.indexOf('crash_symbolication') !== -1 && userovoAuth.validateCreate('crash_symbolication') && _featuresToPopulate.includes("crashes")) {
            ...
            for (let i = 0; i < crashPlatforms.length; i++) {
                ...
                for (let j = 0; j < platformVersions.length; j++) {
                    ...
                    form_data.append('app_key', userovoCommon.ACTIVE_APP_KEY);
                    form_data.append("app_id", userovoCommon.ACTIVE_APP_ID);
                    form_data.append('populator', true);

                    $.ajax({
                        url: userovoCommon.API_URL + "/i/crash_symbols/add_symbol",
                        data: form_data,
                        processData: false,
                        contentType: false,
                        type: "POST",
        ...
        if (userovoGlobal.plugins.indexOf('drill') !== -1) {
            $.ajax({
                type: "GET",
                url: userovoCommon.API_URL + "/i/drill/regenerate_meta",
                data: {
                    app_id: userovoCommon.ACTIVE_APP_ID,
                ...
                error: function() {
                    UserovoHelpers.notify({message: CV.i18n("populator.failed-message-regenerate-meta"), type: "error"});
                }
            });
        }

        if (UserovoHelpers.isPluginEnabled("push") && _featuresToPopulate.includes("push")) {
    ...
    userovoPopulator.getSelectedTemplate = function() {
        return _templateType;
    };

    userovoPopulator.getCompletedRequestCount = function() {
        return completedRequestCount;
    };

    userovoPopulator.setSelectedTemplate = function(value) {
        _templateType = value;
    };

    userovoPopulator.setSelectedFeatures = function(value) {
        _featuresToPopulate = (value === "all") ? _allFeatures : value;
    };

    userovoPopulator.getTemplate = function(templateId, callback) {
        ...
        if (typeof foundDefault !== "undefined") {
            ...
            $.ajax({
                type: "GET",
                url: userovoCommon.API_URL + "/o/populator/templates",
                data: {template_id: templateId, app_id: userovoCommon.ACTIVE_APP_ID},
                success: callback,
                error: function() {
                    UserovoHelpers.notify({message: CV.i18n("populator.failed-to-fetch-template", templateId), type: "error"});
                }
            });
        }
    };

    userovoPopulator.getTemplates = function(platformType, callback) {
        const data = {app_id: userovoCommon.ACTIVE_APP_ID};
        if (platformType) {
        ...
        $.ajax({
            ...
            error: function() {
                UserovoHelpers.notify({message: CV.i18n("populator.failed-to-fetch-templates"), type: "error"});
            }
        });
    };

    userovoPopulator.createTemplate = function(template, callback) {
        template.app_id = userovoCommon.ACTIVE_APP_ID;

        $.ajax({
            type: "POST",
            url: userovoCommon.API_URL + "/i/populator/templates/create",
    ...
    userovoPopulator.editTemplate = function(templateId, newTemplate, callback) {
        ...
        if (typeof foundDefault !== "undefined") {
            ...
            newTemplate.app_id = userovoCommon.ACTIVE_APP_ID;
            $.ajax({
                type: "POST",
                url: userovoCommon.API_URL + "/i/populator/templates/edit",
    ...
    userovoPopulator.removeTemplate = function(templateId, callback) {
        var foundDefault = defaultTemplates.find(function(template) {
            return template._id === templateId;
        });
        ...
        if (typeof foundDefault !== "undefined") {
            ...
            $.ajax({
                type: "GET",
                url: userovoCommon.API_URL + "/i/populator/templates/remove",
                data: {template_id: templateId, app_id: userovoCommon.ACTIVE_APP_ID},
                success: callback,
                error: function() {
                    UserovoHelpers.notify({message: CV.i18n("populator.failed-to-remove-template", templateId), type: "error"});
                }
            });
        }
    };

    userovoPopulator.checkEnvironment = function(environmentName, callback) {
        const data = {app_id: userovoCommon.ACTIVE_APP_ID, environment_name: environmentName};
        $.ajax({
            type: "GET",
            url: userovoCommon.API_URL + "/o/populator/environment/check",
            data: data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            ...
            error: function() {
                UserovoHelpers.notify({message: CV.i18n("populator.failed-to-fetch-environment"), type: "error"});
            }
        });
    };

    userovoPopulator.getEnvironments = function(callback) {
        const data = {app_id: userovoCommon.ACTIVE_APP_ID};
        $.ajax({
            type: "GET",
            url: userovoCommon.API_URL + "/o/populator/environment/list",
            data: data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            ...
            error: function() {
                UserovoHelpers.notify({message: CV.i18n("populator.failed-to-fetch-environments"), type: "error"});
            }
        });
    };

    userovoPopulator.getEnvironment = function(templateId, environmentId, callback) {
        const data = { app_id: userovoCommon.ACTIVE_APP_ID, template_id: templateId, environment_id: environmentId };
        $.ajax({
            type: "GET",
            url: userovoCommon.API_URL + "/o/populator/environment/get",
            data: data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function(enviroment) {
    ...
    userovoPopulator.removeEnvironment = function(templateId, environmentId, callback) {
        const data = { app_id: userovoCommon.ACTIVE_APP_ID, template_id: templateId, environment_id: environmentId };
        $.ajax({
            type: "GET",
            url: userovoCommon.API_URL + "/o/populator/environment/remove",
            data: data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function(enviroment) {
            ...
            error: function() {
                UserovoHelpers.notify({message: CV.i18n("populator.failed-to-delete-environment"), type: "error"});
            }
        });
    };

    userovoPopulator.defaultTemplates = defaultTemplates;
}(window.userovoPopulator = window.userovoPopulator || {}));
