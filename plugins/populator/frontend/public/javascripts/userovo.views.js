/* global app, userovoAuth, userovoVue, userovoPopulator, UserovoHelpers, CV, userovoCommon, userovoGlobal, Vue, moment */
(function() {
    var FEATURE_NAME = 'populator';

    var PopulatorView = userovoVue.views.create({
        template: userovoVue.T("/populator/templates/populator.html"),
        data: function() {
            return {
        ...
        computed: {
            ...
            availableFeatures: function() {
                var plugins = [
                    ...
                    {value: "crashes", label: CV.i18n(userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID] && userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].type === "web" ? "web.crashes.title" : "crashes.title")},
                ...
                if (userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID] && userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].type === "mobile") {
                    plugins.push({value: "push", label: CV.i18n("push-notification.title")});
                }
                return plugins.filter(function(plugin) {
                    return UserovoHelpers.isPluginEnabled(plugin.value);
                }).sort(function(a, b) {
        ...
        methods: {
            ...
            unescapeCondition: function(prmConditions) {
                ...
                conditions.forEach(function(condition) {
                    condition.selectedKey = userovoCommon.unescapeHtml(condition.selectedKey);
                    condition.selectedValue = userovoCommon.unescapeHtml(condition.selectedValue);
                    condition.values.forEach(function(value) {
                        value.key = userovoCommon.unescapeHtml(value.key);
                    });
                });
                return conditions;
            },
            unescapSegmentations: function(prmSegmentations) {
                ...
                segmentations.forEach(function(segmentation) {
                    segmentation.key = userovoCommon.unescapeHtml(segmentation.key);
            ...
            decodeHtmlEntities: function(obj) {
                ...
                if (obj.users) {
                    obj.users.forEach(function(user) {
                        user.key = userovoCommon.unescapeHtml(user.key);
                        if (user.values) {
                ...
                if (obj.events) {
                    obj.events.forEach(function(event) {
                        event.key = userovoCommon.unescapeHtml(event.key);
                ...
                if (obj.views) {
                    obj.views.forEach(function(view) {
                        view.key = userovoCommon.unescapeHtml(view.key);
                ...
                if (obj.sequences) {
                    obj.sequences.forEach(function(sequence) {
                        sequence.steps.forEach(function(step) {
                            step.value = userovoCommon.unescapeHtml(step.value);
                        });
                    });
                }
            ...
            submitConfirmDialog: function() {
                ...
                if (this.dialog.type === "deleteTemplate") {
                    userovoPopulator.removeTemplate(this.dialog.params.templateId, function(res) {
                        if (res.result) {
                    ...
                    if (this.isOpen) {
                        userovoPopulator.checkEnvironment(this.environmentName, function(res) {
                            if (res.errorMsg) {
                                UserovoHelpers.notify({type: "error", title: CV.i18n("common.error"), message: res.errorMsg, sticky: false, clearAll: true});
            ...
            redirectHomePage: function() {
                ...
                var self = this;
                userovoPopulator.stopGenerating(false, function() {
            ...
            moveProgressBar: function(template) {
                ...
                self.progressBar = setInterval(function() {
                    if (userovoPopulator.isGenerating()) {
                        self.percentage = userovoPopulator.getCompletedRequestCount() / (template.uniqueUserCount) * 100;
                        self.progressBarWidth = userovoPopulator.getCompletedRequestCount() + " / " + template.uniqueUserCount;
                    }
                    else {
                        self.percentage = 100;
                        userovoPopulator.stopGenerating(true);
                        window.clearInterval(self.progressBar);
                        self.generateDataModal = { showDialog: false };
            ...
            startPopulate: function() {
                ...
                userovoPopulator.setStartTime(userovoCommon.periodObj.start / 1000);
                userovoPopulator.setEndTime(userovoCommon.periodObj.end / 1000);
                ...
                if (this.currentPopulateTab === 'pop-with-env') { // populate with environment selected
                    ...
                    userovoPopulator.getEnvironment(templateId, self.selectedEnvironment, function(env) {
                        if (env && env.aaData && env.aaData.length) {
                            env = env.aaData.map(environmentName => {
                                return { ...environmentName, name: name };
                            });
                            userovoPopulator.generateUsers(self.selectedRunCount, self.selectedTemplateInformation, env);
                    ...
                    userovoPopulator.setSelectedTemplate(self.selectedTemplate);
                    userovoPopulator.setSelectedFeatures(this.selectedFeatures);
                    ...
                    userovoPopulator.generateUsers(self.selectedRunCount, this.selectedTemplateInformation);
            ...
            stopPopulate: function() {
                ...
                this.description = CV.i18n('populator.warning3');
                var self = this;
                userovoPopulator.stopGenerating(true, function() {
                    window.clearInterval(self.progressBar);
                    self.generateDataModal = { showDialog: false };
                    self.dialog = {type: '', showDialog: false, saveButtonLabel: '', cancelButtonLabel: '', title: '', text: ''};
                    if (self.isOpen) {
            ...
            openDialog: function() {
                if (userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].salt || userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].checksum_salt) {
                    UserovoHelpers.notify({type: 'error', message: CV.i18n("populator.error-salt"), sticky: true});
                    return;
                }
                var self = this;
                ...
                userovoPopulator.getTemplate(selectedTemplateId, function(template) {
                    ...
                    const selectedDayCount = parseInt((userovoCommon.periodObj.end / 1000 - userovoCommon.periodObj.start / 1000) / 3600, 10);
                    ...
                    self.dialog = {
                        ...
                        title: CV.i18n('populator.warning1', UserovoHelpers.appIdsToNames([userovoCommon.ACTIVE_APP_ID])),
                    ...
                    if (self.isOpen) {
                        userovoPopulator.checkEnvironment(self.environmentName, function(res) {
                            if (res.errorMsg) {
                                UserovoHelpers.notify({type: "error", title: CV.i18n("common.error"), message: res.errorMsg, sticky: false, clearAll: true});
                                self.environmentName = '';
                                self.isOpen = false;
            ...
            getTemplateList: function(force) {
                ...
                self.templates = [];
                userovoPopulator.getEnvironments(function(environments) {
                    ...
                    userovoPopulator.getTemplates(userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].type, function(templates) {
        ...
        mixins: [
            userovoVue.mixins.hasDrawers("populatorTemplate"),
            userovoVue.mixins.auth(FEATURE_NAME)
        ]
    });

    var EnvironmentDetail = userovoVue.views.create({
        data: function() {
            var self = this;
            var tableStore = userovoVue.vuex.getLocalStore(userovoVue.vuex.ServerDataTable("environmentUsersTable", {
            ...
            return {
                ...
                tableStore,
                remoteTableDataSource: userovoVue.vuex.getServerDataSource(tableStore, "environmentUsersTable")
            };
        },
        computed: {
        ...
        methods: {
            ...
            submitConfirmDialog: function() {
                var self = this;
                userovoPopulator.removeEnvironment(this.templateId, this.environmentId, function(res) {
                    if (res.result) {
                        UserovoHelpers.notify({type: "ok", title: CV.i18n("common.success"), message: CV.i18n('populator-success-delete-environment'), sticky: true, clearAll: true});
                        self.dialog.showDialog = false;
                        app.navigate("/manage/populate", true);
                    }
                    else {
                        UserovoHelpers.notify({type: "error", title: CV.i18n("common.error"), message: CV.i18n('populator.failed-to-delete-environment', self.environmentId), sticky: false, clearAll: true});
                    }
                });
            },
            openDialog: function() {
        ...
        template: userovoVue.T("/populator/templates/environment_detail.html"),
        created: function() {
            var self = this;
            this.templateId = this.$route.params.id;
            userovoPopulator.getEnvironments(function(envs) {
                self.filterByEnvironmentOptions = envs.filter(x => x.templateId === self.templateId)
                    .map(x => ({value: x._id, label: userovoCommon.unescapeHtml(x.name)}));
        ...
        beforeCreate: function() {
            var self = this;
            userovoPopulator.getTemplate(this.$route.params.id, function(template) {
    ...
    var AppLockedView = userovoVue.views.create({
        template: userovoVue.T("/populator/templates/application-lock.html"),
        methods: {
    ...
    app.route("/manage/populate*state", "populate", function() {
        if (userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].locked) {
            this.renderWhenReady(getAppLockedView());
        }
        else if (userovoAuth.validateRead("populator")) {
    ...
    Vue.component("cly-populator-template-drawer", userovoVue.views.create({
        props: {
        ...
        methods: {
            ...
            onSubmit: function(editedObject) {
                ...
                if (validationMessages && validationMessages.length) {
                    UserovoHelpers.notify({type: "error", title: CV.i18n("common.error"), message: validationMessages, sticky: true, clearAll: true});
                ...
                if (duplicatedValues && duplicatedValues.length) {
                    UserovoHelpers.notify({type: "error", title: CV.i18n("common.error"), message: duplicatedValues, sticky: true, clearAll: true});
                    this.$refs.populatorDrawer.disableAutoClose = true;
                    return;
                }
                if (isEdit && !isDuplicate) {
                    userovoPopulator.editTemplate(editedObject._id, editedObject, function(res) {
                        if (res && res.result) {
                            UserovoHelpers.notify({type: "ok", title: CV.i18n("common.success"), message: CV.i18n("populator-success-edit-template"), sticky: false, clearAll: true});
                            self.$emit('refresh-table', true, true);
                        }
                        else if (res && res.err) {
                            UserovoHelpers.notify({type: "error", title: CV.i18n("common.error"), message: res.err, sticky: true, clearAll: true});
                        }
                    });
                }
                else {
                    userovoPopulator.createTemplate(editedObject, function(res) {
                        if (res && res.result) {
                            UserovoHelpers.notify({type: "ok", title: CV.i18n("common.success"), message: CV.i18n("populator-success-create-template"), sticky: false, clearAll: true});
                            self.$emit('refresh-table', true, true);
                        }
                        else if (res && res.err) {
                            UserovoHelpers.notify({type: "error", title: CV.i18n("common.error"), message: res.err, sticky: true, clearAll: true});
        ...
        template: userovoVue.T("/populator/templates/template_form.html"),
    }));
})();