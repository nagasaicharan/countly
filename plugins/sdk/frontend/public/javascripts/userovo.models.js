/*global userovoCommon, userovoVue, CV, UserovoHelpers, jQuery, $, userovoTotalUsers, _ */

(function(userovoSDK) {

    UserovoHelpers.createMetricModel(userovoSDK, {name: "sdks", estOverrideMetric: "sdks"}, jQuery);

    userovoSDK.clearRequestObject = function(obj) {
        if (obj) {
            if (!obj.r) {
                obj.r = 0;
            }
    ...
    userovoSDK.getSDKVersionData = function(sdk) {
        sdk = sdk.toLowerCase();
        var data = userovoSDK.getData(true, false, "sdk_version").chartData;
        var ret = [];
    ...
    userovoSDK.loadListOfSDKs = function(app_id, callback) {
        $.ajax({
            type: "GET",
    ...
    userovoSDK.getRequestData = function(clean, join, metric1) {
        var chartData = {};
        var i = 0;
        chartData = userovoCommon.extractTwoLevelData(userovoSDK.getDb(), userovoSDK.getMeta(metric1), userovoSDK.clearRequestObject, [
            {
                name: metric1,
        ...
        chartData.chartData = userovoCommon.mergeMetricsByName(chartData.chartData, metric1);
    ...
    userovoSDK.getChartData = function(sdk, metric, displayType) {
        if (!metric) {
            metric = "u";
        }
        ...
        var dd = userovoCommon.extractChartData(userovoSDK.getDb(), userovoSDK.clearObject, chartData, dataProps, "", true);
    ...
    userovoSDK.getRequestChartData = function(metric, segment, displayType) {
        if (!metric) {
            metric = "r";
        }
        var isPercentage = displayType === "percentage";
        var data = userovoSDK.getRequestData(true, false, segment).chartData;
        var chartData = [];
        var dataProps = [];
        for (let i = 0; i < data.length; i++) {
        ...
        var dd = userovoCommon.extractChartData(userovoSDK.getDb(), userovoSDK.clearRequestObject, chartData, dataProps, "", true);
        var series = dd.chartDP;
        var totals = [];
        var percent = [];
        var labels = [];
    ...
    userovoSDK.getRequestTotals = function() {
        return userovoCommon.getDashboardData(userovoSDK.getDb(), ["r", "c", "q"], [], {}, userovoSDK.clearRequestObject);
    };
    userovoSDK.getHealthCheckTotals = function() {
        return userovoCommon.getDashboardData(userovoSDK.getDb(), ["hc_hc", "hc_el", "hc_wl"], [], {}, userovoSDK.clearRequestObject);
    };

    userovoSDK.getHealthCheckData = function(clean, join, metric1) {
        var chartData = {};
        var i = 0;
        chartData = userovoCommon.extractTwoLevelData(userovoSDK.getDb(), userovoSDK.getMeta(metric1), userovoSDK.clearRequestObject, [
            {
    ...
    userovoSDK.getHealthCheckChartData = function(metric, segment, displayType) {
        if (!metric) {
            metric = "r";
        }
        var isPercentage = displayType === "percentage";
        var data = userovoSDK.getHealthCheckData(true, false, segment).chartData;
        var chartData = [];
        var dataProps = [];
        for (let i = 0; i < data.length; i++) {
            chartData.push({ data: [], label: data[i][segment] });
    ...
    userovoSDK.getDelayTotals = function() {
        var data = userovoSDK.getDb(),
            properties = ["d_total", "d_count", "d_min", "d_max"],
            unique = [];
        var estOverrideMetric = {},
            clearObject = userovoSDK.clearRequestObject,
            segment = "";
        ...
        for (i = 0; i < properties.length; i++) {
            ...
            dataArr[properties[i]] = {
                "total": userovoCommon.formatSecond(current[properties[i]]),
                "prev-total": userovoCommon.formatSecond(previous[properties[i]]),
        ...
        change.d = userovoCommon.getPercentChange(Math.round(previous.d_total / Math.max(previous.d_count, 1)), Math.round(current.d_total / Math.max(current.d_count, 1)));
        dataArr.d = {
            "total": userovoCommon.formatSecond(Math.round(current.d_total / Math.max(current.d_count, 1))),
            "prev-total": userovoCommon.formatSecond(Math.round(previous.d_total / Math.max(previous.d_count, 1))),
    ...
    userovoSDK.getRequestTimeData = function() {
        var cData = [
                { data: [], label: "Received", color: userovoCommon.GRAPH_COLORS[0] },
                { data: [], label: "Canceled", color: userovoCommon.GRAPH_COLORS[1] },
                { data: [], label: "Queued", color: userovoCommon.GRAPH_COLORS[2] }
            ],
            dataProps = [
                { name: "r" },
                { name: "c" },
        ...
        var data = userovoCommon.extractChartData(userovoSDK.getDb(), userovoSDK.clearRequestObject, cData, dataProps);
    ...
    userovoSDK.getHealthCheckTimeData = function() {
        var cData = [
                { data: [], label: "Health Checks", color: userovoCommon.GRAPH_COLORS[0] },
                { data: [], label: "Error Logs", color: userovoCommon.GRAPH_COLORS[1] },
                { data: [], label: "Warning Logs", color: userovoCommon.GRAPH_COLORS[2] }
            ],
            dataProps = [
                { name: "hc_hc" },
                { name: "hc_el" },
    ...
    userovoSDK.getRequestDelayData = function() {
        var cData = [
                { data: [], label: "Minimum Delay", color: userovoCommon.GRAPH_COLORS[0] },
                { data: [], label: "Average Delay", color: userovoCommon.GRAPH_COLORS[1] },
                { data: [], label: "Maximum Delay", color: userovoCommon.GRAPH_COLORS[2] },
            ],
            dataProps = [
                { name: "d_min" },
                {
        ...
        return {
            ...
            yAxis: yAxis,
            valFormatter: function(val) {
                return userovoCommon.formatSecond(val);
            }
        };
    };
    ...
    userovoSDK.service = {
        ...
        fetchSDKStats: function() {
            return $.when(userovoSDK.initialize(), userovoTotalUsers.initialize("sdks"));
        },
        updateParameter: function(parameter) {
            return CV.$.ajax({
                type: "POST",
                url: userovoCommon.API_PARTS.data.w + "/sdk-config/update-parameter",
                data: {
                    app_id: userovoCommon.ACTIVE_APP_ID,
        ...
        calculate: function() {
            var chartData = userovoCommon.extractTwoLevelData(userovoSDK.getDb(), userovoSDK.getMeta("sdks"), userovoSDK.clearObject, [
                {
                    name: "sdks",
                    func: function(rangeArr) {
                        return rangeArr;
            ...
            chartData.chartData = userovoCommon.mergeMetricsByName(chartData.chartData, "sdks");
            ...
            for (var i = 0; i < chartData.chartData.length; i++) {
                var tmpVersion = userovoSDK.getSDKVersionData(chartData.chartData[i].sdks);
    ...
    userovoSDK.getVuexModule = function() {
        var actions = {
            initialize: function(context) {
                return userovoSDK.service.initialize().then(function(sdk) {
                    context.dispatch("userovoSDK/sdk/all", sdk, {root: true});
                });
            },
            fetchSDKStats: function(context) {
                context.dispatch('onFetchInit', "sdks");
                userovoSDK.service.fetchSDKStats().then(function() {
                    var sdks = userovoSDK.service.calculate();
                    if (!context.state.stats.selectedSDK) {
                        context.dispatch('onSetSelectedSDK', sdks.versions[0].label);
                    }
                    context.commit('stats/setSDKChartData', userovoSDK.getChartData(context.state.stats.selectedSDK, context.state.stats.selectedProperty, context.state.stats.selectedDisplay));
                    context.commit('stats/setRequestChartData', {
                        received: userovoSDK.getRequestChartData("r", "type", context.state.stats.selectedDisplay),
                        canceled: userovoSDK.getRequestChartData("c", "reason", context.state.stats.selectedDisplay),
                        timeseries: userovoSDK.getRequestTimeData(),
                        timedelays: userovoSDK.getRequestDelayData(),
                        totals: userovoSDK.getRequestTotals(),
                        delays: userovoSDK.getDelayTotals()
                    });
                    context.commit('stats/setHCChartData', {
                        statusCodes: userovoSDK.getHealthCheckChartData("hc_sc", "status", context.state.stats.selectedDisplay),
                        errorMessages: userovoSDK.getHealthCheckChartData("hc_em", "error", context.state.stats.selectedDisplay),
                        timeseries: userovoSDK.getHealthCheckTimeData(),
                        totals: userovoSDK.getHealthCheckTotals()
                    });
        ...
        var sdkResource = userovoVue.vuex.Module("sdk", {
            state: function() {
                return {
                    all: {},
                    isTableLoading: true
            ...
            actions: {
                ...
                update: function(context, parameter) {
                    ...
                    return userovoSDK.service.updateParameter(updateParameter);
                },
        ...
        var sdkStats = userovoVue.vuex.Module("stats", {
            state: function() {
                return {
                    sdk: {"chart": {}, "table": []},
                    chartData: {},
        ...
        return userovoVue.vuex.Module("userovoSDK", {
            actions: actions,
            submodules: [sdkResource, sdkStats]
        });
    };

})(window.userovoSDK = window.userovoSDK || {});