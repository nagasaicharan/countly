/*global app, CV, userovoVue, userovoCommon, userovoSources, userovoGlobal, $, userovoGraphNotesCommon */
(function() {
    var FEATURE_NAME = "sources";

    var SourcesDetailTableContainer = userovoVue.views.create({
    ...
    var SourcesTabContainer = userovoVue.views.create({
        ...
        methods: {
            sourcesDetailDataMap: function() {
                var self = this;
                var cleanData = userovoSources.getData(true).chartData;
                var source;
                for (var i in cleanData) {
                    source = userovoSources.getSourceName(cleanData[i].sources);
            ...
            chartsDataPrepare: function(sourcesData) {
                ...
                self.pieSourcesTotalSessions.series[0].label = {
                    formatter: "{a|" + CV.i18n('common.table.total-sessions') + "}\n" + userovoCommon.getShortNumber(sumOfTotalSession || 0),
                    textAlign: "center",
                    fontWeight: 500,
                    fontSize: 16,
                    lineHeight: 24,
                ...
                self.pieSourcesNewUsers.series[0].label = {
                    formatter: "{a|" + CV.i18n('common.table.new-users') + "}\n" + userovoCommon.getShortNumber(sumOfNewUsers || 0),
                    textAlign: "center",
                    fontWeight: 500,
                    fontSize: 16,
                    lineHeight: 24,
            ...
            refresh: function(force) {
                ...
                $.when(userovoSources.initialize(true))
                    .then(function() {
                        self.sourcesData = userovoSources.getData();
    ...
    var KeywordsTabContainer = userovoVue.views.create({
        ...
        methods: {
            getTop3: function(data) {
                ...
                return [
                    {percentage: Math.round((data[totalsArray[0][0]].t / sum) * 100), label: data[totalsArray[0][0]]._id, value: userovoCommon.getShortNumber(totalsArray[0][1] || 0)},
                    {percentage: Math.round((data[totalsArray[1][0]].t / sum) * 100), label: data[totalsArray[1][0]]._id, value: userovoCommon.getShortNumber(totalsArray[1][1] || 0)},
                    {percentage: Math.round((data[totalsArray[2][0]].t / sum) * 100), label: data[totalsArray[2][0]]._id, value: userovoCommon.getShortNumber(totalsArray[2][1] || 0)}
                ];
            },
            refresh: function() {
                var self = this;
                $.when(userovoSources.initializeKeywords())
                    .then(function() {
                        self.searchedTermsData = userovoSources.getKeywords();
    ...
    var SourcesContainer = userovoVue.views.create({
        template: CV.T("/sources/templates/main.html"),
        mixins: [],
        data: function() {
            return {
                tab: (this.$route.params && this.$route.params.tab) || 'sources',
                isWeb: userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].type === "web",
    ...
    var SourcesView = new userovoVue.views.BackboneWrapper({
    ...
    var WidgetComponent = userovoVue.views.create({
        ...
        mixins: [userovoVue.mixins.customDashboards.global,
            userovoVue.mixins.customDashboards.widget,
            userovoVue.mixins.customDashboards.apps,
    ...
    var KeywordsDashboardWidget = userovoVue.views.create({
        ...
        mounted: function() {
            var self = this;
            $.when(userovoSources.initializeKeywords()).then(function() {
        ...
        methods: {
            ...
            calculateAllData: function() {
                var data = userovoSources.getKeywords();
                var totalsArray = [];
                var sum = 0;
                for (var i = 0; data.length > i; i++) {
                    totalsArray.push([i, data[i].t]);
                ...
                for (var z = 0; z < 3; z++) {
                    if (totalsArray[z]) {
                        totalsData.push({percentage: Math.round((data[totalsArray[z][0]].t / sum) * 100), label: data[totalsArray[z][0]]._id, value: userovoCommon.getShortNumber(totalsArray[z][1] || 0)});
                    }
                    else {
    ...
    var SourcesDashboardWidget = userovoVue.views.create({
        ...
        mounted: function() {
            ...
            $.when(userovoSources.initialize(true)).then(function() {
        ...
        methods: {
            ...
            calculateAllData: function() {
                var blocks = [];
                var data = userovoSources.getData() || {};
                data = data.chartData || [];
                ...
                for (var k = 0; k < values.length; k++) {
                    ...
                    blocks.push({
                        "name": values[k].name,
                        "value": userovoCommon.getShortNumber(values[k].t || 0),
                        "percent": percent,
