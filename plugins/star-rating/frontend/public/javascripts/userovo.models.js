/*global userovoCommon, app, jQuery $*/
(function(starRatingPlugin) {
    ...
    starRatingPlugin.bulkUpdateWidgetStatus = (payload) => {
        return $.ajax({
            type: 'POST',
            url: `${userovoCommon.API_URL}/i/feedback/widgets/status`,
            data: {
    ...
    starRatingPlugin.requestPlatformVersion = function(isRefresh) {
        var periodString = userovoCommon.getPeriodForAjax();
        //returning promise
        return $.ajax({
            type: "GET",
    ...
    starRatingPlugin.requestFeedbackData = function(filterObj) {
        var data = {app_id: userovoCommon.ACTIVE_APP_ID};
    ...
    starRatingPlugin.requestSingleWidget = function(id, callback) {
        ...
        return $.ajax({
            type: "GET",
            url: userovoCommon.API_URL + "/o/feedback/widget",
            data: {
                widget_id: id,
                app_id: userovoCommon.ACTIVE_APP_ID
            },
    ...
    starRatingPlugin.editFeedbackWidget = function(feedbackWidget, callback) {
        ...
        return $.ajax({
            type: "POST",
            url: userovoCommon.API_URL + "/i/feedback/widgets/edit",
            data: data,
            success: function(json, textStatus, xhr) {
                callback(json, xhr.status);
            }
    ...
    starRatingPlugin.removeFeedbackWidget = function(widget_id, with_data, callback) {
        return $.ajax({
            type: "POST",
            url: userovoCommon.API_URL + "/i/feedback/widgets/remove",
            data: {
                app_id: userovoCommon.ACTIVE_APP_ID,
                widget_id: widget_id,
                with_data: true
            ...
            success: function(json) {
                ...
                json.forEach(row => {
                    if (typeof row?.internalName === "string") {
                        row.internalName = userovoCommon.unescapeHtml(row.internalName);
                    }
                    if (typeof row?.targeting?.user_segmentation === "string") {
                        row.targeting.user_segmentation = userovoCommon.unescapeHtml(row.targeting.user_segmentation);
                    }
                });
                _fwd = json;
            }
