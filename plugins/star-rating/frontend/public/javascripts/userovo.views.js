/*global $, userovoReporting, userovoGlobal, UserovoHelpers, starRatingPlugin, app, jQuery, userovoPlugins, userovoCommon,  CV, userovoVue, moment, userovoCohorts*/
(function() {
    ...
    */
    ...
    var consentLink = userovoVue.views.create({
    ...
    var Drawer = userovoVue.views.create({
        ...
        data: function() {
            return {
                ...
                logoDropzoneOptions: {
                    ...
                    url: "/i/feedback/logo" + "?api_key=" + userovoGlobal.member.api_key + "&app_id=" + userovoCommon.ACTIVE_APP_ID,
                    paramName: "logo",
                    params: { _csrf: userovoGlobal.csrf_token, identifier: '' }
                },
                logoFile: "",
                stamp: 0,
                cohortsEnabled: userovoGlobal.plugins.indexOf('cohorts') > -1
            };
        },
        watch: {
            imageSource: {
    ...
    var CommentsTable = userovoVue.views.create({
        mixins: [userovoVue.mixins.commonFormatters],
        ...
        computed: {
            preparedRows: function() {
                ...
                return this.comments.map(function(comment) {
                    comment.cd = userovoCommon.formatTimeAgoText(comment.cd).tooltip;
        ...
        data: function() {
            var self = this;
            var tableStore = userovoVue.vuex.getLocalStore(userovoVue.vuex.ServerDataTable("commentsTable", {
                columns: ['comment', 'cd', 'email', 'rating'],
                onRequest: function() {
                    const data = {app_id: userovoCommon.ACTIVE_APP_ID, period: userovoCommon.getPeriodForAjax()};
                    ...
                    return {
                        type: "GET",
                        url: userovoCommon.API_URL + "/o/feedback/data",
                        data: data
                    };
                },
                ...
                onReady: function(context, rows) {
                    rows.forEach(function(row) {
                        row.cd = userovoCommon.formatTimeAgoText(row.cd).tooltip;
            ...
            return {
                ...
                commentsTablePersistKey: 'comments_table_' + userovoCommon.ACTIVE_APP_ID,
                remoteTableDataSource: userovoVue.vuex.getServerDataSource(tableStore, "commentsTable"),
            };
        }
    });

    var RatingsTable = userovoVue.views.create({
        ...
        data: function() {
            return {
                ratingsTablePersistKey: 'ratings_table_' + userovoCommon.ACTIVE_APP_ID
            };
        }
    });

    var WidgetsTable = userovoVue.views.create({
        mixins: [
        ...
        data() {
            return {
                ...
                persistKey: 'ratingsWidgetsTable_' + userovoCommon.ACTIVE_APP_ID,
        ...
        methods: {
            bulkUpdateTableData(tableScope) {
                const diff = tableScope?.diff || [];

                if (diff.length > 0) {
                    this.loading = true;
                    const requests = diff.reduce((acc, item) => {
            ...
            goWidgetDetail(row) {
                window.location.hash = `#/${userovoCommon.ACTIVE_APP_ID}/feedback/ratings/widgets/${row._id}`;
            },
            ...
            parseTargetingForExport(targeting) {
                const segmentationDescription = userovoCohorts.getSegmentationDescription(targeting);
    ...
    var RatingsTab = userovoVue.views.create({
        ...
        methods: {
            ...
            calCumulativeData: function() {
                ...
                var periodArray = userovoCommon.getPeriodObj().currentPeriodArr;
    ...
    var WidgetsTab = userovoVue.views.create({
        ...
        mixins: [
            userovoVue.mixins.hasDrawers("widget"),
            userovoVue.mixins.auth(FEATURE_NAME)
        ],
        data: function() {
            return {
        ...
        methods: {
            ...
            // TODO: localizations
                ...
                if ((userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID] && userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].plugins && userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].plugins.feedbackApp) || userovoPlugins.getConfigsData().feedback) {
                    var feedbackApp = userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID] && userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].plugins && userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].plugins.feedbackApp ? userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].plugins.feedbackApp : null;
                    var feedback = userovoPlugins.getConfigsData().feedback;
                    if (feedbackApp && feedbackApp.main_color) {
                        trigger_bg_color = feedbackApp.main_color;
                    }
                    else if (userovoPlugins.getConfigsData().feedback) {
                    ...
                    else if (userovoPlugins.getConfigsData().feedback && userovoPlugins.getConfigsData().feedback.feedback_logo) {
    ...
    var RatingsMain = userovoVue.views.create({
                ...
                tabs: [
                    {
                        ...
                        route: '#/' + userovoCommon.ACTIVE_APP_ID + '/feedback/ratings/ratings',
                    ...
                    },
                    {
                        ...
                        route: '#/' + userovoCommon.ACTIVE_APP_ID + '/feedback/ratings/widgets',
    ...
    var WidgetDetail = userovoVue.views.create({
        ...
        methods: {
            backToWidgets: function() {
                window.location.hash = '#/' + userovoCommon.ACTIVE_APP_ID + '/feedback/ratings/widgets';
            },
            calCumulativeData: function() {
                var self = this;
                // reset values
            ...
            handleCommand: function(command) {
                ...
                switch (command) {
                case 'delete-widget':
                    UserovoHelpers.confirm(CV.i18n('feedback.delete-a-widget-description'), "red", function(result) {
                        if (!result) {
                            return true;
                        }
            ...
            fetch: function(force) {
                ...
                starRatingPlugin.requestSingleWidget(this.$route.params.id, function(widget) {
                    ...
                    self.widget.created_at = userovoCommon.formatTimeAgoText(self.widget.created_at).text;
    ...
    var UserFeedbackRatingsTable = userovoVue.views.create({
        mixins: [userovoVue.mixins.commonFormatters],
                ...
                starRatingPlugin.requestFeedbackData({uid: this.uid, period: "noperiod"})
                    .then(function() {
                        ...
                        self.ratingsData.map(function(rating) {
                            rating.ts = userovoCommon.formatTimeAgo(rating.ts);
                        });
                        self.isLoading = false;
                    });
            }
    ...
    var RatingsMainView = new userovoVue.views.BackboneWrapper({
    ...
    var WidgetDetailView = new userovoVue.views.BackboneWrapper({
    ...
    app.addPageScript("/manage/reports", function() {
        userovoReporting.addMetric({name: jQuery.i18n.map["reports.star-rating"], pluginName: "star-rating", value: "star-rating"});
    });
...
app.addPageScript("/drill#", function() {
    ...
    var record_star_rating = userovoGlobal.record_star_rating;

    if (userovoGlobal.apps && userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID] && userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].plugins && userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].plugins.drill && typeof userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].plugins.drill.record_star_rating !== "undefined") {
        record_star_rating = userovoGlobal.apps[userovoCommon.ACTIVE_APP_ID].plugins.drill.record_star_rating;
    }
...
*/
    ...
    if (app.configurationsView && !app.configurationsView.predefinedLabels["feedback.main_color"]) {
        ...
        app.configurationsView.registerInput("feedback.feedback_logo", {
            ...
            attrs: {
                name: "feedback_logo",
                action: userovoGlobal.path + "/i/feedback/upload",
                data: {auth_token: userovoGlobal.auth_token}
            },
            errorMessage: "",
            success: function() {
            ...
            data: function() {
                return {
                    id: userovoCommon.ACTIVE_APP_ID,
                };
            },
    ...
    if (app.appManagementViews && !app.appManagementViews.feedbackApp) {
        ...
        var feedbackId = userovoCommon.ACTIVE_APP_ID;
        if (manageIndex !== -1) {
        ...
        app.addAppManagementInput("feedbackApp", CV.i18n("feedback.title"),
            {
                ...
                "feedbackApp.feedback_logo": {
                    ...
                    before: function(file) {
                        ...
                        feedbackId = segments[manageIndex + 1] || userovoCommon.ACTIVE_APP_ID;
                        var type = file.type;
                        if (type !== "image/png" && type !== "image/gif" && type !== "image/jpeg") {
