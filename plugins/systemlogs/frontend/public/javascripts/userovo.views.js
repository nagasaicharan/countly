/*global userovoSystemLogs, userovoAttribution, userovoCrashes, moment, userovoCommon, userovoGlobal, app, $, jQuery, userovoVue, CV, userovoAuth */

(function() {
    var FEATURE_NAME = "systemlogs";
    var SystemLogsViewExpanded = userovoVue.views.create({
    ...
    var SystemLogsView = userovoVue.views.create({
        ...
        data: function() {
            var self = this;
            var tableStore = userovoVue.vuex.getLocalStore(userovoVue.vuex.ServerDataTable("systemLogsTable", {
                columns: ['_id', "ts", "u", "ip"],
                onRequest: function() {
                    self.loaded = false;
                    return {
                    ...
                    for (var i = 0; i < rows.length; i++) {
                        ...
                        if (typeof row.i === "object") {
                            if (typeof row.i.app_id !== "undefined") {
                                if (typeof userovoGlobal.apps[row.i.app_id] !== "undefined") {
                                    row.app_name = jQuery.i18n.map["systemlogs.for-app"] + ": " + userovoGlobal.apps[row.i.app_id].name;
                                }
                                else {
                            ...
                            if (typeof row.i.campaign_id !== "undefined" && typeof userovoAttribution !== "undefined") {
                                row.campaign_id = jQuery.i18n.map["systemlogs.for-campaign"] + ": " + userovoAttribution.getCampaignName(row.i.campaign_id);
                            }
                            if (typeof row.i.crash_id !== "undefined" && typeof userovoCrashes !== "undefined") {
                                row.crash_id = jQuery.i18n.map["systemlogs.for-crash"] + ": " + userovoCrashes.getCrashName(row.i.crash_id);
                            }
            ...
            return {
                ...
                remoteTableDataSource: userovoVue.vuex.getServerDataSource(tableStore, "systemLogsTable")
            };
        },
        props: {
            query: {
        ...
        beforeCreate: function() {
            var self = this;
            return $.when(userovoSystemLogs.initialize())
                .then(function() {
                // get fetched sources datas
                    var meta = userovoSystemLogs.getMetaData();
                    if (meta.action) {
        ...
        methods: {
            ...
            getExportAPI: function() {
                ...
                requestPath = '/o?api_key=' + userovoGlobal.member.api_key +
                "&app_id=" + userovoCommon.ACTIVE_APP_ID + "&method=systemlogs&iDisplayStart=0&export=true" +
