...
const demoPageElements = {
    ...
    ADD_COMMENT_CHECKBOX: '#userovo-feedback-show-comment',
    ADD_COMMENT_LABEL: '#cf-comment-text',
    ADD_COMMENT_INPUT: '#userovo-feedback-comment-textarea',
    CONTACT_VIA_CHECKBOX: '#userovo-feedback-show-email',
    CONTACT_VIA_LABEL: '#cf-email-text',
    CONTACT_VIA_INPUT: '#userovo-feedback-contact-me-email',
    CONSENT: '#consent',
    SUBMIT_BUTTON: '#cf-submit-button',
    LOGO_IMAGE: '#powered-by-userovo',
        ...
        .then((response) => {
            for (const key in response.admin_of) {
                if (response.admin_of[key].name === appName) {
                    ...
                    cy.visit(`/feedback/rating?widget_id=${widgetID.trim()}&device_id=test&app_key=${appKey}&custom={"tc":1,"rw":1,"xb":1}`);
                }
            }
        });
};
